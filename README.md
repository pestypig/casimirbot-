...
| Theory badge graph | Canonical typed theory architecture from first principles through laws, derived relations, calculator payloads, runtime evidence, and claim boundaries. | `docs/architecture/theory-badge-graph-contract.md`, `shared/theory/helix-theory-badge-graph.ts`, `shared/contracts/theory-badge-graph.v1.ts` |
| Evidence tree + DAG | Legacy and audit compatibility structure for math, citations, manifests, root-to-leaf parity, and replay checks. | `AUDIT_TREE.json`, `MATH_GRAPH.json`, `configs/physics-root-leaf-manifest.v1.json`, `docs/warp-tree-dag-inventory.md` |
