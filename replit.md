...
- **Mission Start Portal**: Feather-light entry page at root `/` route with 4 mission profiles (Radiant Optimist, Engineer, Diplomat, Strategist). Progressive disclosure interface showing profile icons/images initially, then revealing zen quotes, physics focus areas, and navigation on selection. Main application moved to `/bridge` route for clean URL structure. Supports both emoji and custom image profiles with enhanced hover effects and accessibility (January 2025).
