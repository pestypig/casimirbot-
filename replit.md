...
- **Annotation-Only SliceViewer Design**: Switched to cleaner annotation-only approach for equatorial curvature slice visualization. Removed interactive control panel in favor of overlay annotations explaining axes (X-axis: Radial ρ in meters, Y-axis: Curvature κ(ρ) in 1/m²) and dynamic grid scale based on hull dimensions. Maintains physics-driven preferences system while emphasizing visual clarity over user controls.
