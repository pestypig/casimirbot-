...
- **Enhanced SliceViewer with Dynamic Annotations**: Completely redesigned SliceViewer with annotation-only approach featuring dynamic axis overlays, physics-accurate equatorial curvature rendering, and intelligent grid scaling. Includes SVG-based axis labels (X: forward/aft, Z: port/starboard), automatic "nice step" tick spacing, unit-aware formatting (m/km), colored centerlines (cyan for x=0, orange for z=0), and grid scale legend. Maintains authentic Natário amplitude calculations with exposure controls while providing clean visual presentation without interactive panels.
