...
function makeRouteId(): string {
  return `zen-graph-situation-reflection:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 8)}`;
}
...
function detectRouteGuards(text: string): RouteGuard[] {
  ...
  if (/\b(label|diagnose|classify)\b.*\b(real person|person|character|personality)\b/.test(normalized)) {
    guards.push({
      ...
      reason: "ZenGraph character presets are perspective bindings and cannot classify a real person's character.",
    });
  }
  ...
  if (/\b(evil|bad person|good person|morally guilty|moral verdict|final verdict)\b/.test(normalized)) {
    guards.push({
      ...
      reason: "ZenGraph can provide reflection evidence, but it cannot issue terminal moral verdicts.",
    });
  }
  ...
  if (/\b(bypass|override|without confirmation|without consent|execute anyway|run command|commit code|send message)\b/.test(normalized)) {
    guards.push({
      ...
      reason: "ZenGraph recommendations cannot bypass confirmation, consent, or action admission.",
    });
  }
  ...
  if (/\b(legal|medical|financial|sue|diagnosis|diagnose|investment|bankruptcy)\b/.test(normalized)) {
    guards.push({
      ...
      reason: "ZenGraph reflection is evidence-only and cannot become legal, medical, or financial authority.",
    });
  }
...
function buildGuardAdmission(params: {
  ...
  const actions: HelixRecommendedActionAdmissionEntryV1[] = params.guards.map((guard) => ({
    actionId: `zen-graph.${guard.actionType}`,
    panelId: "zen-graph",
    label: guard.label,
    ...
    source: {
      ...
      tool: "zen-graph-situation-reflection-prototype",
      artifact_type: "zen_graph_situation_reflection_route",
      artifact_id: params.routeId,
    },
    display_policy: "hidden",
    evidenceRefs: params.refs,
    reasonCodes: ["zen_graph_reflection", guard.code, "evidence_only_authority"],
  }));
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    evidenceRefs: params.refs,
    reasonCodes: ["zen_graph_reflection", "route_guard", "evidence_only_authority"],
    actions,
  });
}
...
function buildAgentEvidence(response: ZenGraphReflectionToolResponseV1, routeId: string) {
  return {
...
zenGraphPrototypeRouter.post("/zen-graph/reflection/prototype", async (req, res) => {
  const parsed = requestSchema.safeParse(req.body ?? {});
  if (!parsed.success) {
    res.status(400).json({
      error: "invalid_zen_graph_reflection_request",
  ...
  try {
    ...
    const toolResponse = reflectWithZenGraphToolV1(graph, {
      reflectionId: `zen-graph-reflection:${routeId}`,
      loopDepth: 0,
    ...
    res.status(500).json({
      error: "zen_graph_reflection_failed",
