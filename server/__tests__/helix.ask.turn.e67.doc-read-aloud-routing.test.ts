...
describe("helix ask E67 docs read-aloud routing", () => {
  ...
  it("routes named whitepaper read-aloud prompts through docs acquisition and read start", async () => {
    ...
    const response = await request(app)
      .post("/api/agi/ask/turn")
      .send({
        question: "Ok read Nhm2 Current Status Whitepaper 2026 05 02 to me",
        mode: "read",
        debug: true,
    ...
    expect(serialized).toContain("docs/research/nhm2-current-status-whitepaper-2026-05-02.md");
  }, 90000);
});
