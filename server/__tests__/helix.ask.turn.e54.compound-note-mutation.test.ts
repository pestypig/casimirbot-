...
describe("helix ask E54 compound note mutation", () => {
  it("completes create-note turns through a current-turn note update receipt", async () => {
    const app = createApp();
    const activePath =
    ...
    expect(response.body?.canonical_goal_frame?.required_terminal_kind).toBe("note_update_receipt");
