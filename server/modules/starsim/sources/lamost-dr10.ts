...
const LAMOST_DR10_ADAPTER_VERSION = "lamost_dr10.live/2";
const FIXTURE_ROOT = path.resolve(process.cwd(), "tests", "fixtures", "starsim", "sources", "lamost-dr10");
    ...
    const fixtures = await Promise.all(
      jsonFiles.map(async (fileName) => {
        ...
        return {
          ...parsed,
          catalog: "lamost_dr10",
          adapter_version: LAMOST_DR10_ADAPTER_VERSION,
          fetch_mode: "fixture",
...
const matchesIdentifiers = (record: StarSimSourceRecord, identifiers: StarSimSourceIdentifiers): boolean =>
  Boolean(
    (identifiers.lamost_obsid
      && record.identifiers.lamost_obsid
      && normalize(record.identifiers.lamost_obsid) === normalize(identifiers.lamost_obsid))
...
const findFixture = async (args: {
  ...
  if (fixtures.length === 0) {
    return {
      ...
      attempted: true,
      record: null,
      reason: "lamost_dr10_fixture_unavailable",
    };
  }
  ...
  return {
    ...
    attempted: true,
    record: null,
    reason: "lamost_dr10_record_not_found",
  };
};
...
const buildNormalizedLiveRecord = (args: {
  ...
  const obsid = String(args.payload.lamost_obsid ?? args.payload.obsid ?? "");
  const gaiaSourceId = String(args.payload.gaia_dr3_source_id ?? "");
  if (!obsid && !gaiaSourceId) {
    return null;
  }
  ...
  const metallicity = maybeNumber(args.payload.metallicity_feh ?? args.payload.feh);
  ...
  return {
    ...
    fetched_at_iso: args.fetched_at_iso,
    query_metadata: args.query_metadata,
    record_id: `lamost_dr10:${obsid || gaiaSourceId}`,
    identifiers: {
      gaia_dr3_source_id: gaiaSourceId || undefined,
      lamost_obsid: obsid || undefined,
    },
    aliases: [args.request.target?.name ?? "", obsid, gaiaSourceId].filter(Boolean),
    quality_flags: [
      ...(Array.isArray(args.payload.quality_flags) ? args.payload.quality_flags.filter((flag): flag is string => typeof flag === "string") : []),
      "live_fetch",
    ],
    quality_score: maybeNumber(args.payload.quality_score) ?? 65,
    notes: ["LAMOST DR10 live fetch"],
    target: {
...
const fetchLiveRecord = async (args: {
  ...
  const runtime = buildStarSimSourceAdapterRuntime("lamost_dr10");
  if (!runtime.endpoint) {
    return {
      catalog: "lamost_dr10",
      attempted: true,
  ...
  const lamostObsid = args.identifiers.lamost_obsid?.trim();
  const gaiaSourceId = args.identifiers.gaia_dr3_source_id?.trim();
  if (!lamostObsid && !gaiaSourceId && !args.request.target?.name?.trim()) {
    return {
      catalog: "lamost_dr10",
      attempted: true,
      record: null,
  ...
  try {
    ...
    if (lamostObsid) {
      url.searchParams.set("lamost_obsid", lamostObsid);
    }
    if (gaiaSourceId) {
...
export const resolveLamostDr10Source = async (args: {
  request: StarSimRequest;
  identifiers: StarSimSourceIdentifiers;
}): Promise<StarSimSourceAdapterResult> => {
  const runtime = buildStarSimSourceAdapterRuntime("lamost_dr10");
...
export const getLamostDr10AdapterVersion = (): string => LAMOST_DR10_ADAPTER_VERSION;
