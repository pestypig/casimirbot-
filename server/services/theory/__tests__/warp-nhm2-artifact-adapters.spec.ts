...
describe("read-only Warp/NHM2 artifact adapters", () => {
  it("produces a completed receipt from pass-like full-solve artifacts", async () => {
  ...
  it("surfaces receipts in Theory Run evidence rows", async () => {
    ...
    expect(withReceipts.rows.some((row) => row.runtimeReceiptV1?.status === "completed")).toBe(true);
