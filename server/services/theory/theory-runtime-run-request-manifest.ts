...
function requestPath(projectRoot: string | undefined, requestId: string): string {
...
export async function createTheoryRuntimeRunRequestManifest(
  ...
  if (!entrypoint) {
    throw new Error(`Runtime ${input.runtimeId} is not registered in THEORY_RUNTIME_ENTRYPOINTS.`);
  }
