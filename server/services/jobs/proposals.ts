...
const interestingSegments = ["lib/", "hooks/", "store/", "physics/", "constants/", "services/", "workers/", "shared/", "modules/", "server/", "datasets/", "docs/"];
const globPatterns = [
...
let cachedProposals: DesktopPanelProposal[] = [];
let lastScan = 0;

export function discoverPanelProposals(limit = 8): DesktopPanelProposal[] {
  const now = Date.now();
  if (cachedProposals.length && now - lastScan < ttlMs) {
    return cachedProposals.slice(0, limit);
  }
  ...
  const candidates = files
    .map((file) => buildProposalForFile(file, registered))
    .filter((entry): entry is { proposal: DesktopPanelProposal; score: number } => !!entry)
    .sort((a, b) => (b.score === a.score ? a.proposal.title.localeCompare(b.proposal.title) : b.score - a.score))
    .map((entry) => entry.proposal);
  cachedProposals = candidates;
  lastScan = now;
...
function buildProposalForFile(filePath: string, registered: Set<string>): { proposal: DesktopPanelProposal; score: number } | null {
  ...
  const proposal: DesktopPanelProposal = {
    id: makeId(relPath),
    title,
    summary,
...
function toKebab(value: string): string {
  return toHuman(value).toLowerCase().replace(/\s+/g, "-");
}
...
function buildSummary(componentName: string, relPath: string, panelId: string, dataSources: string[]): string {
  ...
  if (dataSources.length) {
    ...
    return `Essence spotted ${pretty} (${relPath}) pulling from ${list}. Promote it as \`${panelId}\` to surface those parallels in Helix.`;
  }
