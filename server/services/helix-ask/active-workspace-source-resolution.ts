...
const naturalActiveDocContinuationRe =
  /(?:\bwhat(?:'s|\s+is)\s+(?:the\s+)?(?:main|central|core)\s+(?:idea|claim|argument|point|thesis)\b|\bwhat(?:'s|\s+is)\s+(?:it|this|that|the\s+(?:doc(?:ument)?|paper|whitepaper|source|study))\s+(?:mainly\s+)?about\b|\bwhat\s+(?:is|was)\s+(?:it|this|that|the\s+(?:doc(?:ument)?|paper|whitepaper|source|study))\s+(?:really\s+)?(?:trying|attempting|intended)\s+to\s+(?:establish|show|prove|demonstrate|argue|claim|test)\b|\b(?:explain|summarize|describe|outline)\s+(?:it|this|that|the\s+(?:doc(?:ument)?|paper|whitepaper|source|study))\b|\b(?:correct|revise|qualify|tighten)\s+(?:it|this|that|the\s+(?:claim|answer|summary|interpretation))\b|\bwhat\s+(?:the\s+)?(?:paper|doc(?:ument)?|whitepaper|source|study|it|this|that)\s+(?:does|do|can|could|would)\s+not\s+(?:prove|show|support|establish|demonstrate|imply|claim|argue|mean)\b|\bhow\s+(?:does|do|is|are)\s+(?:it|this|that|the\s+(?:doc(?:ument)?|paper|whitepaper|source|study|idea|claim|argument))\s+(?:connect(?:ed)?|relate(?:d)?|compare(?:d)?|apply|fit|matter)\b|\b(?:does|do|can|could|would)\s+(?:it|this|that|the\s+(?:doc(?:ument)?|paper|whitepaper|source|study|author))\s+(?:actually\s+)?(?:prove|show|support|establish|demonstrate|imply|claim|argue|mean)\b)/i;

const isActiveDocEvidenceFollowup = (prompt: string): boolean => {
  const unquoted = unquotePrompt(prompt);
  if (!unquoted) return false;
  if (
    /\b(?:do\s+not|don'?t|without|avoid|ignore|disregard|no\s+need\s+to)\b[\s\S]{0,120}\b(?:quote|cite|extract|explain|summarize|describe|interpret|correct|revise|qualify|tighten|check|verify|read|use|connect|relate|compare|prove|show|support)\b/i.test(unquoted) ||
    /^(?:if|when|once|after)\b[\s\S]{0,180}\b(?:later|eventually|next\s+time|in\s+the\s+future|tomorrow)\b/i.test(unquoted) ||
    /\b(?:later|eventually|next\s+time|in\s+the\s+future|tomorrow|previously|earlier|historically)\b[\s\S]{0,140}\b(?:quot(?:e|ed|ing)|cit(?:e|ed|ing)|extract(?:ed|ing)?|explain(?:ed|ing)?|summar(?:y|ize|ized|izing)|describ(?:e|ed|ing)|interpret(?:ed|ing)?|correct(?:ed|ing)?|revis(?:e|ed|ing)|qualif(?:y|ied|ying)|tighten(?:ed|ing)?|check(?:ed|ing)?|verif(?:y|ied|ying)|read|connect(?:ed|ing)?|relat(?:e|ed|ing)|compar(?:e|ed|ing)|prov(?:e|ed|ing)|show(?:n|ed|ing)?|support(?:ed|ing)?)\b/i.test(unquoted) ||
