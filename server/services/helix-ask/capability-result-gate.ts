...
const isEvidenceKind = (kind: string): boolean =>
  /^(?:doc_search_results|doc_candidate_validation|doc_location_matches|doc_evidence_location|doc_location_result|situation_context_pack|procedure_epoch_replay|visual_scene_comparison_result|repo_code_evidence_answer|scholarly_research_observation|scholarly_research_answer|process_graph_overview|source_binding_status|source_binding_repair_candidate|reasoning_context|validation)$/.test(kind);
