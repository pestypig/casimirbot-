...
const RAW_AUDIT_PROMPT_RE =
  /\b(?:audit|debug|provenance|raw\s+refs?|lookup\s+keys?|evidence\s+depth|sidecar(?:\s+id)?|source\s+(?:id|image\s+hash|hash)|crop\s+ref|bbox|active\s+promoted\s+row\s+blockers|historical\s+non-promoted\s+row\s+blockers|report\s+only|tell\s+me\s+which\s+promoted\s+page-grounded\s+equation\s+row|which\s+promoted\s+page-grounded\s+equation\s+row|which\s+paper,\s*page,\s*equation)\b/i;
...
export const finalizeScientificWorkflowAnswer = (input: ScientificWorkflowFinalizerInput): string => {
  ...
  const promotedCount = sidecar?.exact_equation_summary.promoted_row_count ?? 0;
  const partialCount = sidecar?.exact_equation_summary.partial_row_count ?? 0;
  ...
  if (promotedCount > 0) {
    return [
      ...
      "",
      `Evidence state: ${evidenceDepth}; promoted exact rows: ${promotedCount}; partial rows: ${partialCount}.`,
      "Next step: use the promoted row as the cited evidence target, then keep the answer framed as relevance/adjacency unless a derivation chain is added.",
    ].join("\n");
  }

  if (evidenceDepth === "exact_row_partial" || partialCount > 0 || workflowStatus?.promotedRowState === "partial") {
    return [
      ...
      "",
      `Evidence state: ${evidenceDepth}; promoted exact rows: 0; partial rows: ${partialCount}.`,
