...
describe("Helix workstation tool gateway provider parity", () => {
  it("exposes the same read/observe manifest to Helix and Codex providers", () => {
  ...
  it.each([
  ...
  ])("keeps %s non-terminal for Helix and Codex", async (capabilityId, args) => {
    ...
    for (const result of [helixResult, codexResult]) {
  ...
  it("blocks unavailable mutating capabilities the same way for Helix and Codex", async () => {
    const helixResult = await callWorkstationGatewayCapability({
      agentRuntime: "helix",
