...
const rules: CueRule[] = [
  ...
  },
  {
    ...
    cues: [
      { label: "compare_to_named_prior_scene", pattern: /\bcompare\b[\s\S]{0,120}\b(?:last|previous)\s+[A-Za-z0-9 _.-]{2,80}\s+(?:folder\s+)?scene\b/i },
      { label: "find_named_prior_scene", pattern: /\bfind\b[\s\S]{0,80}\bscene\b[\s\S]{0,80}\b(?:folder|camera\s+roll|soho|sun|audio|task\s+manager)\b/i },
      { label: "semantic_scene_memory", pattern: /\b(?:camera\s+roll|soho|sun\s+folder|audio\s+files?|task\s+manager)\b[\s\S]{0,120}\b(?:scene|compare|changed|previous|last)\b/i },
    ],
  },
  {
    target: "procedure_memory",
