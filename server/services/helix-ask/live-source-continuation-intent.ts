...
const hasExplicitLiveSourceMailCue = (text: string): boolean =>
  ...
  /\blive_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)\b/i.test(text) ||
...
const hasContextualLiveSourceMailCue = (text: string): boolean =>
  /["'`][^"'`]*(?:live_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)|read\s+live\s+source\s+mail|live[-\s]?source\s+mailbox|observer\s+mailbox|stage\s+play\s+mail|interpreter\s+profile|profile\s+note)[^"'`]*["'`]/i.test(text) ||
  /\b(?:in\s+the\s+future|future|later|eventually|if|when|before|after|would|could|might|hypothetically)\b[\s\S]{0,140}\b(?:live_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)|read\s+live\s+source\s+mail|live[-\s]?source\s+mailbox|observer\s+mailbox|stage\s+play\s+mail|visual\s+summary\s+mail|interpreter\s+profile|profile\s+note)\b/i.test(text) ||
  /\b(?:screen|page|button|label|ui|text)\b[\s\S]{0,90}\b(?:says|shows|reads|contains|labeled|labelled|called|named)\b[\s\S]{0,120}\b(?:live_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)|read\s+live\s+source\s+mail|live[-\s]?source\s+mailbox|observer\s+mailbox|stage\s+play\s+mail|visual\s+summary\s+mail|interpreter\s+profile|profile\s+note)\b/i.test(text) ||
  /\b(?:do\s+not|don't|dont|without|not\s+asking\s+to|for\s+now)\b[\s\S]{0,120}\b(?:run|execute|use|read|check|process|create|configure|apply|pause|open|compile)?\b[\s\S]{0,120}\b(?:live_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)|read\s+live\s+source\s+mail|live[-\s]?source\s+mailbox|observer\s+mailbox|stage\s+play\s+mail|visual\s+summary\s+mail|interpreter\s+profile|profile\s+note)\b/i.test(text) ||
  /\b(?:explain|describe|what\s+does|what\s+is|what\s+are)\b[\s\S]{0,120}\b(?:live_env\.(?:read_live_source_mail|record_live_source_mail_decision|configure_interpreter_profile|compare_mail_to_interpreter_profile)|read\s+live\s+source\s+mail|live[-\s]?source\s+mailbox|observer\s+mailbox|stage\s+play\s+mail|visual\s+summary\s+mail|interpreter\s+profile|profile\s+note)\b[\s\S]{0,80}\b(?:mean|means|for|do|does|is|are)\b/i.test(text);
