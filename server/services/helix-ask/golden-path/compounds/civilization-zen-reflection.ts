...
const buildPromptZenGraphReflectionToolResult = (promptText: string): RecordLike => {
  const lower = promptText.toLowerCase();
  ...
  const reflection = {
    reflectionId: "zen-graph:prompt-produced",
    input: {
      summary: promptText,
...
export const buildHelixAskGoldenPathCivilizationBoundsZenReflectionCompoundPayload = (args: {
  ...
  const promptProducedZenResult = buildPromptZenGraphReflectionToolResult(promptText);
  ...
  const zenResult = readCompactZenGraphReflectionToolResult(args.body) ?? promptProducedZenResult;
  ...
  const civilizationReflectionLayers = buildHelixReflectionObservationLayers({
    ...
    compoundHandoffNotes: {
      zen_graph_reflection: "Apply ideology guidance after the civilization bounds have limited feasibility and readiness claims.",
  ...
  const zenReflectionLayers = buildHelixReflectionObservationLayers({
    promptText,
    selectedNodes: [
      reflection,
    ...
    locatorRationale:
      "The zen graph reflection was selected to constrain the final wording and normative interpretation after the civilization-bounds location is known.",
    supportRefs: zenEvidenceRefs,
    constraintsIntroduced: [
    ...
    missingEvidence: zenRefs.length > 0 ? [] : ["ideology_context_support_refs"],
    practicalFraming:
