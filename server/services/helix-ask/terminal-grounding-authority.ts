...
export const buildHelixTerminalGroundingAuthority = (input: {
  ...
  const poisonAudit = readRecord(payload.poison_audit) ?? readRecord(debug?.poison_audit);
  ...
  const evidenceRequiredDeclaration =
    typeof evidenceReentryGate?.required === "boolean"
      ...
      : null;
  const admittedRouteTools = Array.isArray(routeEvidenceAuthority?.admitted_tools)
  ...
  const routeDeclaresModelOnly =
    ...
    admittedRouteTools.every((entry) =>
  ...
  const routeGroundingEvidenceRefs = readStringArray(
    ...
    /:(?:observation|workstation_gateway|gateway_call|codex_normalized):/i.test(ref));
  const hasGroundingSignals =
    selectedEvidenceRefs.length > 0 ||
    routeGroundingEvidenceRefs.length > 0;
  ...
  const artifactBindingOk =
    ...
    declaredArtifactRefs.every((declaredRef) => declaredRef === terminalArtifactRef);
  const artifactKindBindingOk = unique([
  ...
  const answerSourceBindingOk = unique([
    finalAnswerSource,
    readString(terminalAuthority?.final_answer_source),
    readString(terminalPresentation?.final_answer_source),
  ...
  const poisonAuditOk =
    solverTrace?.poison_audit_ok === true &&
    poisonAudit?.ok !== false;
  ...
  const providerSupportedRefs = new Set(unique([
    ...readStringArray(providerBridge?.successful_gateway_observation_refs),
    ...readStringArray(providerBridge?.successful_capability_lane_observation_refs),
  ...
  for (const rawEntry of currentTurnLedger) {
    ...
    if (!entry || (readString(entry.turn_id) && readString(entry.turn_id) !== turnId)) continue;
  ...
  const providerBridgeReentryValid =
    ...
    readString(providerReentry?.schema) === "helix.provider_reasoning_reentry.v1" &&
  ...
  const evidenceReentryAuthorityValid =
    evidenceReentryGate?.reentry_authority === "runtime_event_log"
      ...
      : evidenceReentryGate?.reentry_authority === "provider_terminal_authority_bridge"
  ...
  const failureCodes = unique([
    ...
    terminalArtifactRef && (!artifactBindingOk || !artifactKindBindingOk || !answerSourceBindingOk)
    ...
    groundingRequired && solverArtifactAudit?.ok === false
      ...
      : null,
  ].filter((entry): entry is HelixTerminalGroundingAuthorityFailureCode => Boolean(entry))) as
    HelixTerminalGroundingAuthorityFailureCode[];
  ...
  return {
    ...
    runtime_lifecycle_verified:
      groundingRequired && evidenceReentryGate?.runtime_lifecycle_verified === true,
