...
const inferRouteFamily = (payload: RecordLike, capability: string): HelixPostToolAuthorityBridge["route_family"] => {
  ...
  if (/dottie|situation-room|minecraft|live_pipeline|stage_play_badge_graph|reflect_stage_play_context/i.test(haystack)) return "situation_room_setup";
...
export function buildPostToolAuthorityBridge(input: {
  ...
  const toolObservationRefs = artifactRefs(input.payload, /agent_step_observation_packet|runtime_tool_observation|live_environment_tool_observation|workspace_action_receipt|calculator_receipt|dottie_|voice_delivery|workstation_tool_evaluation|stage_play_badge_graph/);
