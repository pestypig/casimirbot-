...
): HelixLiveAcceptanceGate => helixLiveAcceptanceGateSchema.parse({
  gate_id: gateId,
  verdict: "pass",
...
const readSnapshot = async (input: {
  ...
  const { rows: readRows } = await input.client.query<Snapshot["fresh_reads"][number]>(
  ...
  const counts = control ? await input.client.query<{
  ...
  return {
    ...
    active_binding: identity?.binding_status === "active" &&
      identity?.private_only === true,
...
export const readRobinhoodLiveAcceptanceReadiness = async (input: {
  ...
  const deploymentEnabled = input.deploymentEnabled ??
    process.env.ENABLE_ROBINHOOD_LIVE_EQUITY_EXECUTION === "1";
  const supervisorEnabled = input.supervisorEnabled ??
    process.env.ENABLE_ROBINHOOD_LIVE_SUPERVISOR === "1";
  return withSharedRealtimeRoomTransaction(async (client: Queryable) => {
    const snapshot = await readSnapshot({ client, ...input, now });
    const freshReadNames = new Set(snapshot.fresh_reads.map((row) =>
      row.upstream_tool));
    const readsComplete = REQUIRED_READ_TOOLS.every((tool) =>
      freshReadNames.has(tool));
    const contractFresh = snapshot.contract_verdict === "pass" &&
    ...
    const supervisorFresh = snapshot.protective_exit_ready &&
      snapshot.supervisor_status === "healthy" && supervisorAge >= 0 &&
    ...
    const operatorAge = snapshot.operator_presence_at
      ...
      : Number.POSITIVE_INFINITY;
    const operatorFresh = operatorAge >= 0 && operatorAge <= OPERATOR_FRESH_MS;
    ...
    const gates: HelixLiveAcceptanceGate[] = [
      snapshot.account_selected
        ? pass("agentic_account_selected", "agentic_account_selected",
          "Exactly one encrypted Agentic account selection is recorded.")
        : pending("agentic_account_selected", "agentic_account_not_selected",
          "Complete Agentic account discovery before live acceptance."),
      snapshot.private_room && snapshot.active_binding
        ? pass("owner_private_room_binding", "owner_private_room_active",
          "The active binding is restricted to its owner-only room.")
        : pending("owner_private_room_binding", "private_room_binding_required",
          "An active owner-only room binding is required."),
      readsComplete
        ? pass("required_read_receipts_fresh", "required_reads_fresh",
          "All five required sanitized read receipts are fresh.",
          snapshot.fresh_reads.map((row) => row.output_hash),
          snapshot.fresh_reads.reduce<Date | string | null>((latest, row) =>
            !latest || new Date(row.observed_at) > new Date(latest)
              ? row.observed_at : latest, null))
        : pending("required_read_receipts_fresh", "required_reads_missing_or_stale",
          `Fresh reads missing: ${REQUIRED_READ_TOOLS.filter((tool) =>
            !freshReadNames.has(tool)).join(", ")}.`),
      contractFresh
        ? pass("provider_contract_fresh_pass", "provider_contract_fresh_pass",
        ...
        : pending("provider_contract_fresh_pass",
          snapshot.contract_verdict === "fail"
            ? "latest_provider_contract_failed" : "provider_contract_pass_required",
          snapshot.contract_verdict === "fail"
    ...
    const safeToEnable = readAcceptanceComplete && snapshot.unresolved_count === 0;
    const readyToStart = safeToEnable && deploymentPairEnabled && supervisorFresh &&
      !snapshot.attention_required;
    const readyToArm = readyToStart && operatorFresh;
    const acceptanceComplete = readAcceptanceComplete &&
      snapshot.filled_entry_count > 0 && snapshot.filled_exit_count > 0 &&
      snapshot.unresolved_count === 0;
