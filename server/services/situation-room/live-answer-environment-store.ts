...
export function resolveLiveAnswerLineSchema(input: {
  ...
  const custom = Array.isArray(input.line_schema)
    ? input.line_schema.map(normalizeLineDefinition).filter((line): line is LiveAnswerLineDefinition => Boolean(line))
    : [];
  if (custom.length > 0) return custom.slice(0, 16);
  const preset = normalizePreset(input.preset);
  return LIVE_ANSWER_ENVIRONMENT_LINE_PRESETS[preset === "custom" ? "minecraft_run_monitor" : preset].map((line) => ({ ...line }));
}
...
export function createLiveAnswerEnvironment(input: {
  ...
  const environment: LiveAnswerEnvironment = {
    ...
    lines: existing?.lines?.length
      ? existing.lines
      : lineSchema.map((line) => makeLineState(line, objective, now, setupEvidence)),
  ...
  const receipt: LiveAnswerEnvironmentReceipt = {
    ...
    line_keys: lineSchema.map((line) => line.key),
...
export function getActiveLiveAnswerEnvironmentForThread(threadId: string): LiveAnswerEnvironment | null {
  return Array.from(environments.values())
    .filter((environment) => environment.thread_id === threadId && environment.status === "active")
...
export function getActiveLiveAnswerEnvironmentForRoom(roomId: string): LiveAnswerEnvironment | null {
  return Array.from(environments.values())
    .filter((environment) => environment.room_id === roomId && environment.status === "active")
    .sort((a, b) => b.updated_at.localeCompare(a.updated_at))
    .at(0) ?? null;
}

export function listLiveAnswerEnvironments(): LiveAnswerEnvironment[] {
  return Array.from(environments.values()).sort((a, b) => b.updated_at.localeCompare(a.updated_at));
}
...
export function updateLiveAnswerEnvironment(input: {
  ...
  const next: LiveAnswerEnvironment = {
    ...existing,
    lines: nextLines,
    latest_summary: normalizeString(input.latest_summary) ?? nextLines.find((line) => line.key === changedLineKeys[0])?.value ?? existing.latest_summary,
