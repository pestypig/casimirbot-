...
export function recordLiveInterpretationWorkerRun(run: HelixLiveInterpretationWorkerRun): HelixLiveInterpretationWorkerRun {
  runsByInterpretationRun.set(run.interpretation_run_id, [
    ...(runsByInterpretationRun.get(run.interpretation_run_id) ?? [])
      .filter((entry: HelixLiveInterpretationWorkerRun) => entry.interpretation_worker_run_id !== run.interpretation_worker_run_id),
    run,
  ].slice(-1000));
  return run;
}
...
export function listLiveInterpretationWorkerRuns(input: {
  ...
  return (Array.from(runsByInterpretationRun.values()).flat() as HelixLiveInterpretationWorkerRun[])
    .filter((run: HelixLiveInterpretationWorkerRun) => !input.threadId || run.thread_id === input.threadId)
    .filter((run: HelixLiveInterpretationWorkerRun) => !input.interpretationRunId || run.interpretation_run_id === input.interpretationRunId)
    .filter((run: HelixLiveInterpretationWorkerRun) => !input.situationRunId || run.situation_run_id === input.situationRunId)
    .filter((run: HelixLiveInterpretationWorkerRun) => !input.workerId || run.interpretation_worker_id === input.workerId)
    .slice(-limit);
}
