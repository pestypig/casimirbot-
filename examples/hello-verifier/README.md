...
- The workflow at `examples/hello-verifier/.github/workflows/hello-verifier.yml` is meant to live at `.github/workflows/hello-verifier.yml` in a standalone repo.
- Set `CASIMIR_TOKEN` and `CASIMIR_TENANT` secrets if auth/tenant is enabled; otherwise only `CASIMIR_PUBLIC_BASE_URL` / `SHADOW_OF_INTENT_BASE_URL` is needed.
