...
const DEFAULT_SNAPSHOT_PATH = path.join('docs', 'specs', 'data', 'warp-core4-alcubierre-1994.v1.json');
const DEFAULT_OUT_JSON = path.join(FULL_SOLVE_DIR, `warp-geometry-replay-${DATE_STAMP}.json`);
const DEFAULT_OUT_MD = path.join('docs', 'audits', 'research', `warp-geometry-replay-${DATE_STAMP}.md`);
const REQUIRED_KEYS = [
...
const findLatestGeometryPath = (): string | null => {
  try {
    const candidates = fs
      .readdirSync(FULL_SOLVE_DIR)
      .filter((entry) => /^geometry-conformance-\d{4}-\d{2}-\d{2}\.json$/.test(entry))
      .map((entry) => path.join(FULL_SOLVE_DIR, entry))
...
const loadBaselineSignature = (baselinePath: string | null): Record<string, SignatureStatus> => {
  const signature: Record<string, SignatureStatus> = {};
  if (!baselinePath || !fs.existsSync(baselinePath)) return signature;
  const baseline = readJson(baselinePath);
  const checks = Array.isArray(baseline?.checks) ? baseline.checks : [];
  for (const check of checks) {
    const id = asText(check?.id);
    if (!id) continue;
    signature[id] = parseStatus(check?.status);
  }
  return signature;
};

const renderMarkdown = (payload: any): string => {
  const rows = Object.entries(payload.geometry_signature as Record<string, SignatureEntry>)
    .map(
      ([key, value]) =>
  ...
  const blockers = (payload.blockers as string[]).length
    ...
    : '- none';
  return `# Warp Geometry Replay (${payload.work_id}, ${payload.generatedOn})

"${BOUNDARY_STATEMENT}"

## Inputs
- snapshot: \`${payload.inputs.snapshotPath}\`
- baseline: \`${payload.inputs.baselinePath ?? 'UNKNOWN'}\`
...
export const runWarpGeometryReplay = (options: {
  snapshotPath?: string;
  baselinePath?: string;
  outJsonPath?: string;
  outMdPath?: string;
  ...
  const baselinePath = options.baselinePath ?? findLatestGeometryPath();
  ...
  const externalSignature = (snapshot?.geometry_signature ?? {}) as Record<string, Record<string, unknown>>;
  ...
  for (const key of REQUIRED_KEYS) {
    ...
    if (comparison === 'pass') {
      passCount += 1;
      if (reasonCode && reasonCode !== 'reason_code_unspecified') reasonCodes.add(reasonCode);
    } else {
      failCount += 1;
  ...
  const hasHardBlocker = blockers.some((entry) => entry === 'missing_equation_anchor' || entry.startsWith('missing_snapshot_field:'));
  ...
  const payloadBase: Record<string, unknown> = {
    artifactType: 'external_warp_geometry_replay/v1',
    ...
    inputs: {
      ...
      baselinePath: baselinePath ? normalizePath(baselinePath) : null,
    },
    geometry_signature: normalizedSignature,
    comparison_result: {
    ...
    replay: {
      status: replayStatus,
      note: 'Geometry-first method replay; energetic/QEI equivalence is deferred by policy.',
    },
    provenance: {
...
if (isEntryPoint) {
  const payload = runWarpGeometryReplay({
