...
const DEFAULT_SNAPSHOT_PATH = path.join('docs', 'specs', 'data', 'gr-frame-dragging-observable.v1.json');
const DEFAULT_OUT_JSON = path.join(FULL_SOLVE_DIR, `gr-frame-dragging-replay-${DATE_STAMP}.json`);
const DEFAULT_OUT_MD = path.join('docs', 'audits', 'research', `warp-gr-frame-dragging-replay-${DATE_STAMP}.md`);
...
const renderMarkdown = (payload: any): string => {
  ...
  const signature = payload.gr_observable_signature.frame_dragging;
  return `# GR Frame-Dragging Replay (${payload.work_id}, ${payload.generatedOn})
...
## Derived Observable
- gpb_predicted_mas_per_year: \`${signature.gpb_predicted_mas_per_year}\`
- gpb_observed_mas_per_year: \`${signature.gpb_observed_mas_per_year}\`
- gpb_residual_mas_per_year: \`${signature.gpb_residual_mas_per_year}\`
- lageos_expected_ratio: \`${signature.lageos_expected_ratio}\`
- lageos_observed_ratio: \`${signature.lageos_observed_ratio}\`
- lageos_residual_ratio: \`${signature.lageos_residual_ratio}\`
...
export const runGrFrameDraggingReplay = (options: {
  snapshotPath?: string;
  outJsonPath?: string;
  outMdPath?: string;
  workId?: string;
  ...
  const workId = options.workId ?? asText(snapshot?.work_id) ?? 'EXT-GR-FD-001';
  ...
  if (gpbPredicted == null || lageosExpectedRatio == null) {
  ...
  const gpbObserved = asNumber(observed.gpb_measured_mas_per_year);
  const gpbUncertainty = asNumber(observed.gpb_uncertainty_mas_per_year) ?? 7.2;
  const lageosObservedRatio = asNumber(observed.lageos_ratio_measured);
  const lageosRatioUncertainty = asNumber(observed.lageos_ratio_uncertainty) ?? 0.1;
  const laresAccuracyPercent = asNumber(observed.lares_accuracy_percent_reported);
  if (gpbObserved == null || lageosObservedRatio == null) {
  ...
  const maxAbsGpbResidual = asNumber(acceptance.max_abs_gpb_residual_mas_per_year) ?? 14.4;
  const maxAbsLageosRatioResidual = asNumber(acceptance.max_abs_lageos_ratio_residual) ?? 0.2;

  const gpbResidual = gpbPredicted != null && gpbObserved != null ? gpbObserved - gpbPredicted : null;
  const lageosRatioResidual =
    lageosExpectedRatio != null && lageosObservedRatio != null ? lageosObservedRatio - lageosExpectedRatio : null;

  const gpbPass = gpbResidual != null ? Math.abs(gpbResidual) <= maxAbsGpbResidual : null;
  const lageosPass = lageosRatioResidual != null ? Math.abs(lageosRatioResidual) <= maxAbsLageosRatioResidual : null;

  let signatureStatus: 'pass' | 'fail' | 'unknown' = 'unknown';
  let signatureReason = 'insufficient_data';
  if (gpbPass === true && lageosPass === true) {
    signatureStatus = 'pass';
    signatureReason = 'gpb_and_lageos_within_tolerance';
  } else if (gpbPass === false || lageosPass === false) {
    signatureStatus = 'fail';
    signatureReason = gpbPass === false ? 'gpb_residual_exceeds_tolerance' : 'lageos_ratio_residual_exceeds_tolerance';
  }
  reasonCodes.add(signatureReason);

  const hasHardBlocker = blockers.some((entry) => ['missing_equation_anchor', 'missing_constants', 'missing_observed_anchor'].includes(entry));
  ...
  const payloadBase: Record<string, unknown> = {
    artifactType: 'external_gr_frame_dragging_replay/v1',
    ...
    gr_observable_signature: {
      frame_dragging: {
        ...
        max_abs_gpb_residual_mas_per_year: maxAbsGpbResidual,
        lageos_expected_ratio: lageosExpectedRatio,
        lageos_observed_ratio: lageosObservedRatio,
        lageos_residual_ratio: lageosRatioResidual,
        lageos_uncertainty: lageosRatioUncertainty,
        max_abs_lageos_ratio_residual: maxAbsLageosRatioResidual,
    ...
    replay: {
      ...
      derived: {
        gpbPredictedMasPerYear: gpbPredicted,
        gpbObservedMasPerYear: gpbObserved,
        gpbResidualMasPerYear: gpbResidual,
        lageosExpectedRatio: lageosExpectedRatio,
        lageosObservedRatio: lageosObservedRatio,
        lageosRatioResidual: lageosRatioResidual,
      },
    },
    provenance: {
...
if (isEntryPoint) {
  const payload = runGrFrameDraggingReplay({
