...
async function main(): Promise<void> {
  ...
  const firstClaimId = `claim:${paperId}:1`;
  const graphAnchorRefs = claims.map((_, idx) => `claim:${paperId}:${idx + 1}`);
  ...
  const packBase = {
    ...
    framework_deltas: {
      ...
      dag_delta: { added_nodes: [], added_edges: [], updated_edges: [] },
  ...
  const overallStatus =
    predictionStatus === "fail" || symbolStatus === "fail" || falsifierStatus === "fail"
      ? "fail"
      : maturityStatus === "warn"
        ? "warn"
        : "pass";
