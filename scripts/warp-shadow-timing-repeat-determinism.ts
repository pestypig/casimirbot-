...
const DEFAULT_PASS1_PATH = path.join(
  ...
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-recovery-${DATE_STAMP}.json`,
);
const DEFAULT_PASS1_REPEAT_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-recovery-${DATE_STAMP}-repeat.json`,
);
...
const DEFAULT_PASS2_REPEAT_PATH = path.join(
  ...
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-typed-${DATE_STAMP}-repeat.json`,
);
const DEFAULT_REPORTABLE_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-reportable-${DATE_STAMP}.json`,
);
const DEFAULT_REPORTABLE_REPEAT_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-reportable-${DATE_STAMP}-repeat.json`,
);
const DEFAULT_REPORTABLE_REFERENCE_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-reportable-reference-${DATE_STAMP}.json`,
);
const DEFAULT_REPORTABLE_REFERENCE_REPEAT_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `shadow-injection-run-ti-primary-reportable-reference-${DATE_STAMP}-repeat.json`,
);
const DEFAULT_COMPAT_PATH = path.join('artifacts', 'research', 'full-solve', `ti-compat-check-${DATE_STAMP}.json`);
const DEFAULT_COMPAT_REPEAT_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `ti-compat-check-${DATE_STAMP}-repeat.json`,
);
const DEFAULT_COMPAT_REPORTABLE_PATH = path.join(
  'artifacts',
  'research',
  'full-solve',
  `ti-compat-check-reportable-${DATE_STAMP}.json`,
);
...
export const buildTimingRepeatDeterminism = (options: {
  pass1Path?: string;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = buildTimingRepeatDeterminism({
