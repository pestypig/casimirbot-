#!/usr/bin/env python3
"""Fail-closed independent audit for one preserved H2-P8C diagnostic result.

Audit PASS authenticates evidence.  It does not imply that the numerical H2
diagnostic passed, and it cannot promote candidate or physical authority.
"""
...
EXPECTED_PROPOSAL_SHA256 = "7e8f28d755b5dea7cc212c4d0fda263a84374215680b0a94a179fbb2fbca2ace"
EXPECTED_CORRECTION_SHA256 = "aade7e5d8d384500503b4ecd1b2f04f4afcf95bccffd735da309363d01d6c32b"
EXPECTED_ARCHIVE_SHA256 = "f0a0fabf608949d6755465ddc8f35075631818f383d6ba5eb78ab297152d3c4c"
EXPECTED_MANIFEST_SHA256 = "78fdff467f3ededee3a18be0d6c2f94176a90b65b9e94da140f701f95d2fd868"
EXPECTED_BINARY_SHA256 = "7e7d78393f933ac103208476f6e8c5beefb5de66b58d93a6b2a080bdf80deb25"
...
EXPECTED_SCHEMA = "nhm2.g2h_e_s5.c08_h2_p8c_diagnostic_run.v1"
PROCESS_TIMEOUT_SECONDS = 50400
VM_RUNTIME_CEILING_SECONDS = 54000
PLANNING_RATE_USD_PER_HOUR = 0.79068
COST_CEILING_USD = 13.0
DIAGNOSTIC_MAXIMUM_BYTES = 65536
...
def terminal_record_ok(record: dict[str, object], stdout_bytes: bytes) -> bool:
    return (
        record.get("schema") == EXPECTED_SCHEMA
        and record.get("status") in {"PASS", "FAIL"}
        and record.get("phase") in {
            "scalar_initialize", "h2_initialize", "scalar_extend", "h2_extend"
        }
        and isinstance(record.get("origin_h2_passed"), bool)
        and isinstance(record.get("scalar_extension_passed"), bool)
        and isinstance(record.get("diagnostic_serialized"), bool)
        and len(stdout_bytes) <= DIAGNOSTIC_MAXIMUM_BYTES + 4096
...
def audit(capture: Path, output: Path) -> dict[str, object]:
    ...
    checks["correction_binding_exact"] = binding.get("correction_sha256") == EXPECTED_CORRECTION_SHA256
    ...
    checks["resource_metadata_parse"] = all(
        isinstance(item, dict) for item in (pre, post, disk_pre, disk_post)
    )
    ...
    finished = safe_utc(capture / "run.finished.utc")
    elapsed = (finished - started).total_seconds() if finished and started else None
    ...
    try:
        exit_code = int(safe_text(capture / "run.exit").strip()) if (capture / "run.exit").is_file() else None
    except ValueError:
        exit_code = None
    ...
    if len(lines) == 1:
        try:
            ...
            record = {}
    complete_record = len(lines) == 1 and terminal_record_ok(record, stdout_bytes)
    partial_evidence = result_kind in {"TIMEOUT", "PARTIAL"} and (
        exit_code in {None, 124, 137, 143} or disposition.get("forced_vm_stop") is True
    )
    ...
    diagnostic_pass = audit_pass and result_kind == "COMPLETE" and record.get("status") == "PASS"
    classification = (
        "P8C_DIAGNOSTIC_PASS" if diagnostic_pass
        else "P8C_DIAGNOSTIC_NUMERICAL_FAIL" if audit_pass and result_kind == "FAIL"
        else "P8C_DIAGNOSTIC_TIMEOUT_OR_PARTIAL" if audit_pass and result_kind in {"TIMEOUT", "PARTIAL"}
        else "AUDIT_FAIL"
    )
    payload: dict[str, object] = {
        "schema": "nhm2.g2h_e_s5.c08_h2_p8c_result_audit.v1",
        "audit_status": "PASS" if audit_pass else "FAIL",
        "result_classification": classification,
        "diagnostic_h2_pass": diagnostic_pass,
        ...
        "capture_sha256": {
            ...
            for name in sorted(p.name for p in capture.iterdir() if p.is_file())
        } if capture.is_dir() else {},
        ...
        "terminal_record": record if complete_record else None,
...
def make_self_test_capture(root: Path, result_kind: str) -> Path:
    ...
    capture.mkdir()
    write_json(capture / "instance-prestop.json", {
        "name": EXPECTED_VM, "zone": f"zones/{EXPECTED_ZONE}",
        "machineType": f"machineTypes/{EXPECTED_MACHINE}", "status": "RUNNING",
        "creationTimestamp": "2026-08-28T19:00:00+00:00",
    ...
    write_json(capture / "capture-disposition.json", {
        "result_kind": result_kind, "retry_count": 0, "retune_used": False,
        "additional_uploads": 0, "frozen_candidate_evaluated": False,
        "protected_roots_created": 0, "authority": authority,
        "stop_capture_utc": "2026-08-28T21:00:00Z",
        "forced_vm_stop": result_kind in {"TIMEOUT", "PARTIAL"},
    })
    (capture / "frozen-binding.txt").write_text(
        f"proposal_sha256={EXPECTED_PROPOSAL_SHA256}\n"
        f"correction_sha256={EXPECTED_CORRECTION_SHA256}\n"
        f"archive_sha256={EXPECTED_ARCHIVE_SHA256}\n"
        f"manifest_sha256={EXPECTED_MANIFEST_SHA256}\n"
        f"binary_sha256={EXPECTED_BINARY_SHA256}\n"
        f"process_name={EXPECTED_PROCESS}\n", encoding="utf-8"
    )
    (capture / "process-count.txt").write_text("1\n", encoding="utf-8")
    (capture / "run.started.utc").write_text("2026-08-28T19:08:28Z\n", encoding="utf-8")
    (capture / "run.finished.utc").write_text("2026-08-28T20:00:00Z\n", encoding="utf-8")
    (capture / "run.stderr").write_bytes(b"")
    if result_kind in {"COMPLETE", "FAIL"}:
        passed = result_kind == "COMPLETE"
        record = {
            "schema": EXPECTED_SCHEMA, "status": "PASS" if passed else "FAIL",
            "phase": "h2_extend", "origin_h2_passed": True,
            "scalar_extension_passed": True, "diagnostic_serialized": True,
            "detail": "NONE" if passed else "C08-010_VOLTERRA_CONVOLUTION_OR_U_REFINEMENT_EXHAUSTION",
            "diagnostic": {"schema": "nhm2.g2h_e_s5.c08_h2_p8b_parent_diagnostic.v1"},
            "candidate_evaluations": 0, "positive_parameter_samples": 0,
            "candidate_roots_created": False, "scientific_handler_linked": False,
            "authority_promoted": False,
        }
        (capture / "run.stdout").write_text(json.dumps(record, separators=(",", ":")) + "\n", encoding="utf-8")
    ...
    (capture / "capture-files.sha256").write_text(
        "".join(f"{sha256(capture / name)}  {name}\n" for name in files), encoding="utf-8"
    )
    return capture


def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="nhm2-h2-p8c-audit-") as temp:
        root = Path(temp)
        expected = {
            "COMPLETE": "P8C_DIAGNOSTIC_PASS",
            "FAIL": "P8C_DIAGNOSTIC_NUMERICAL_FAIL",
            "PARTIAL": "P8C_DIAGNOSTIC_TIMEOUT_OR_PARTIAL",
        }
        checks: list[bool] = []
        ...
        checks.append(result["audit_status"] == "FAIL" and result["diagnostic_h2_pass"] is False)
        passed = sum(checks)
