#!/usr/bin/env python3
"""Independent runtime/source audit for candidate-neutral S4 P10/R10 fixtures."""
...
BINDING = ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-p10-r10-build-binding.v4.json"
...
record("P10_algorithm_tokens", all(token in primary_source for token in (
    "panels = 1024", "nodes_per_panel = 32", "mesh_intervals = 8192", "rational_bisections_per_root = 600",
    "interval_newton_steps_per_root = 16", "validation_degree = 63", "legendre_fmpq", "arb_intersection", "arb_const_pi",
)), digest(ROOT / binding["primary_cpp_fixture"]["changed_sources"][1]["path"]))
record("R10_algorithm_tokens", all(token in rust_source for token in (
    "PANELS: usize = 1536", "NODES_PER_PANEL: usize = 24", "PI_SERIES_TERMS: usize = 160",
    "TRANSCENDENTAL_TAYLOR_DEGREE: usize = 256", "VALIDATION_DEGREE: usize = 47",
    "machin_pi", "build_ts24", "sinh_cosh_256", "TAIL_MAJORANT_ITERATIONS: usize = 12",
)), digest(ROOT / binding["independent_rust_fixture"]["changed_sources"][1]["path"]))
record("R10_pure_rust", "unsafe" not in rust_source and all(token not in rust_source.lower() for token in ('extern "c"', "#[link", "gmp", "mpfr", "flint", "arb.h")), "no unsafe, FFI, C ABI or primary arithmetic lineage")
record("algorithm_lineages_disjoint", "legendre_fmpq" in primary_source and "legendre_fmpq" not in rust_source and "machin_pi" in rust_source and "machin_pi" not in primary_source, binding["disjointness"]["algorithms"])
...
with tempfile.TemporaryDirectory(prefix="nhm2-s4-p10-audit-") as temp_name:
    temp = Path(temp_name)
...
with tempfile.TemporaryDirectory(prefix="nhm2-s4-p10-corrupt-") as temp_name:
