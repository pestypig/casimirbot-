#!/usr/bin/env python3
"""Verify the versioned P01/R01 receipt without changing its bound audit."""
...
RECEIPT = ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-arithmetic-runtime-verification-receipt.v4.json"
...
check("receipt_status", receipt["status"] == "PASS_P01_R01_ARITHMETIC_KERNELS_REMAINING_SCIENTIFIC_PRODUCERS_INCOMPLETE", receipt["status"])
paths = {
...
arithmetic = json.loads(subprocess.run([sys.executable, str(paths["arithmetic_audit"])], cwd=ROOT, check=True, capture_output=True, text=True).stdout)
check("arithmetic_audit", arithmetic["status"] == "PASS" and arithmetic["checks_passed"] == 15 == arithmetic["checks_total"], [arithmetic["status"], arithmetic["checks_passed"], arithmetic["checks_total"]])
matrix = json.loads(paths["completion_matrix"].read_bytes())
check("fail_closed", matrix["promotion_summary"]["S4_implementation_closure"] is False and matrix["promotion_summary"]["inert_future_primary_proposal_allowed"] is False, matrix["promotion_summary"])
...
report = {
    "schema": "nhm2.g2h_e_s4.arithmetic_receipt_audit.v1",
    ...
    "next_role": "P02/P03 then R02/R03 inverse builders",
}
