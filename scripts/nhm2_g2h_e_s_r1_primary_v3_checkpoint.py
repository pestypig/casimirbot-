#!/usr/bin/env python3
"""One-shot offline G2H-E primary-v3 checkpoint; inert without a new exact AUTHORIZED record."""
...
BINDING_REL = Path(
    "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s-r1-build-binding.v1.json"
)
...
TOKEN_SHA256 = "39ee95896fbd9d39d13c2762c42cefcf5284a2cb66e78dddadcf0006530dd3c3"
CONTRACT_SHA256 = "30de966d41d6342e8a047ee655a33e02f68d32a6ba49efcb39b0bbd7981c343d"

LANES = {
    "primary": {
        "image": "nhm2-g2h-primary-proof:v3",
        "image_id": "sha256:5a790295492d607fc9c3ede7a527a86922e09608cce1eaa9af464d33a1617ada",
        "executable_sha256": "1764b6bec075061eea815a2a9c44cba3ff459fb8cc0709c813762863225246e1",
        "output_root": Path("artifacts/research/nhm2/g2h/tolman-vii-primary-v3"),
        "authorization": Path(
            "artifacts/research/nhm2/g2h-authorizations/g2h-e-primary-v3.txt"
        ),
        "container": "nhm2-g2h-e-primary-v3",
    },
    "independent": {
...
def verify_chronology(lane: str) -> str | None:
    ...
    exhausted_primary_root_v1 = ROOT / "artifacts/research/nhm2/g2h/tolman-vii-primary-v1"
    exhausted_primary_root_v2 = ROOT / "artifacts/research/nhm2/g2h/tolman-vii-primary-v2"
    independent_root = ROOT / LANES["independent"]["output_root"]
    if lane == "primary":
        if primary_root.exists() or exhausted_primary_root_v1.exists() or exhausted_primary_root_v2.exists() or independent_root.exists():
...
def execute_once(lane: str, token: str) -> int:
    if lane != "primary":
        raise RuntimeError("G2H-E-S-R1 primary-v3 checkpoint is primary-only")
    ...
    invocation = EXECUTION_PARENT / f"g2h-e-{lane}-v3-invocation.json"
    stdout_path = EXECUTION_PARENT / f"g2h-e-{lane}-v3.stdout.log"
    stderr_path = EXECUTION_PARENT / f"g2h-e-{lane}-v3.stderr.log"
    result_path = EXECUTION_PARENT / f"g2h-e-{lane}-v3-result.json"
    ...
    write_exclusive(
        invocation,
        {
            "schema": "nhm2.g2h_e_successor.invocation.v3",
            "lane": lane,
    ...
    write_exclusive(
        result_path,
        {
            "schema": "nhm2.g2h_e_successor.process_result.v3",
            "lane": lane,
...
def main() -> int:
    ...
    try:
        ...
        print(f"G2H-E-S-R1 primary-v3 checkpoint rejected: {error}", file=sys.stderr)
        return 64
