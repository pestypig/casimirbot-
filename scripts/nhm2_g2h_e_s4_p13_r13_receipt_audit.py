#!/usr/bin/env python3
"""Audit the fixture-only P13/R13 verification receipt."""
from __future__ import annotations
import hashlib, json, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
RECEIPT = ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-p13-r13-runtime-verification-receipt.v13.json"
PATHS = {
    "quantum_builder": ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-r2-total-quantum-builder-algorithms.v2.json",
    "p12_r12_build": ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-p12-r12-build-binding.v6.json",
    "p13_r13_build": ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s4-p13-r13-build-binding.v7.json",
...
record("receipt_sidecar", digest(RECEIPT) == RECEIPT.with_suffix(".sha256").read_text("ascii").split()[0], digest(RECEIPT))
record("status_fixture_only", receipt["status"] == "PASS_FIXTURE_ONLY_13_OF_13_PER_LANE_FINAL_S4_CLOSURE_PENDING_NO_SCIENTIFIC_EXECUTION_AUTHORITY", receipt["status"])
...
for name, image_name in (("primary", "nhm2-g2h-s4-primary-fixture:v7"), ("independent", "nhm2-g2h-s4-independent-fixture:v7")):
...
record("promotion_waits_for_closure", promotion["primary_complete"] == 13 and promotion["independent_complete"] == 13 and promotion["all_required_roles_complete"] and promotion["final_closure_audit_required"] and not promotion["S4_complete"] and not promotion["inert_future_primary_proposal_allowed"], promotion)
...
report = {"schema": "nhm2.g2h_e_s4.p13_r13_receipt_audit.v1", "status": "PASS" if passed == len(checks) else "FAIL", "checks_passed": passed, "checks_total": len(checks), "checks": checks, "candidate_evaluations": 0, "candidate_roots_created": False, "execution_authorized": False, "authority_promoted": False, "S4_implementation_closure": False}
