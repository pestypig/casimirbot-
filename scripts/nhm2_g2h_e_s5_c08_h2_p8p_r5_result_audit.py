#!/usr/bin/env python3
"""Independent static audit of the terminal candidate-neutral P8P-R5 result."""
...
RESULT = ROOT / "docs/research/nhm2-spherical-boson-star-v2-g2h-e-s5-a4-h2-p8p-r5-ssh-drop-preexecution-result.md"
...
checks = {
    ...
    "proposal_identity": digest(PROPOSAL) == "650806a579fdf41dbe7bd6731439bf1872644a2f15091c95388e7ec86e6e3b03",
    "r4_result_identity": digest(R4_RESULT) == "a7cefb8050fbab5518915852358813836aa7da58db88565e2f1532ba838a0c8f",
    "proposal_named": "650806a579fdf41dbe7bd6731439bf1872644a2f15091c95388e7ec86e6e3b03" in text,
    "terminal_classification": "BLOCKED_PREEXECUTION_SSH_CONNECTION_DROPPED_DURING_REMOTE_GUARD / R5 EXHAUSTED" in text,
    "proposal_audit": "29/29" in text,
    ...
    "browser_preflight": all(value in flat for value in (
        "Chrome profile `Default`", "hehggadaopoacecdllhhajmbjkdcmajg",
        "1.26.827.12125_0", "`newAllowFileAccess=true`",
    ...
    "guard_submitted_once": "remote absence-guard command was entered and submitted once" in flat,
    "connection_failure": all(value in flat for value in (
        "`Connection Failed`", "unexpected connection error")),
    "guard_unauthenticated": all(value in flat for value in (
        "Neither `R4_REMOTE_ABSENT_16`", "guard outcome and the remote destination-path state therefore remain unauthenticated",
        "does not claim that the paths were absent")),
    "first_failure_terminal": "first terminal R5 failure" in flat,
    "no_retry": all(value in flat for value in (
        "Retry, Troubleshoot, a second SSH surface and alternate transport were not used",
        "R5 is exhausted")),
    "zero_picker_upload": all(value in flat for value in (
        "Upload control was not activated", "no native file-open dialog appeared",
        "no file was selected", "zero bytes were uploaded")),
    "zero_guest_execution": "No Docker presence check or installation" in flat and "guest-ledger invocation" in flat,
    "zero_build": all(value in flat for value in (
        "manifest replay, offline build, executable invocation",
        "turnaround-result audit ran")),
    "zero_numerical_process": "P=1024 calibration" in flat and "turnaround-result audit ran" in flat,
    ...
    "successor_narrow": "separately versioned successor" in flat and "stable authenticated SSH command round trip" in flat,
