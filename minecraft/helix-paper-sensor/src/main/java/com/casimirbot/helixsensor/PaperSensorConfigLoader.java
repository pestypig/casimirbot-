...
public final class PaperSensorConfigLoader {
    private PaperSensorConfigLoader() {}
    ...
    public static HelixSensorConfig from(FileConfiguration config) {
        ...
        return new HelixSensorConfig(
            config.getBoolean("helix.enabled", HelixSensorConfig.DEFAULT_ENABLED),
            HelixSensorConfig.stripTrailingSlash(
                config.getString("helix.endpoint", HelixSensorConfig.INACTIVE_ENDPOINT)
            ),
            ...
            scopePolicy,
            config.getBoolean(
                "helix.read_only_probes_enabled",
                HelixSensorConfig.DEFAULT_READ_ONLY_PROBES_ENABLED
            ),
            positive(config.getInt("helix.max_pending_probes_per_poll", 8), 8),
            config.getBoolean(
                "helix.execution_enabled",
                HelixSensorConfig.DEFAULT_EXECUTION_ENABLED
            ),
            ...
            new HelixSensorConfig.SeedMapOptions(
                ...
                positive(
                    config.getInt("helix.seed_map.repeat_every_location_samples", 1),
                    1
                ),
    ...
    private static int positive(int value, int fallback) {
        return HelixSensorConfig.positive(value, fallback);
    }
}
