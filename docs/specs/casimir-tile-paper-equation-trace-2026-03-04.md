...
| SRC-041 | EQT-041-01 | SEM calibration statistics using SRM-484 and uncertainty expression | Reported: `1000x-20000x` range, `2000` edge pairs, `48 s` run, `25 ms` interval, `U=2*uc`, plus numeric uncertainty anchors `0.01 um (3sigma)` line-scale and `0.1 um (3sigma)` SEM-vs-line-scale differences, max difference `0.046 um` | `EXP-SE-003..EXP-SE-009`, `EXP-SE-016..EXP-SE-020` | `CH-SE-001` | `d_sem_corr_nm`, `u_sem_nm`, `delta_se_nm`, SEM repeatability components | partial | Full paired SEM+ellips raw datasets and covariance terms are still missing |
...
| SRC-050 | EQT-050-01 | Ellipsometry reference-procedure acceptance anchor for thickness and expanded uncertainty | Reported from PDF procedure card: thickness range `0.1 nm to 10 um`, expanded uncertainty `0.5 nm absolute (0.1 nm best case) to 10% relative` with `k=2` | `EXP-SE-012..EXP-SE-014` | `CH-SE-001` | `d_ellip_nm`, `u_ellip_nm`, repeatability lane | partial | Reference-procedure anchor is extracted; paired SEM+ellips run covariance datasets are still required |
