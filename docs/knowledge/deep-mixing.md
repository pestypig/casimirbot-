...
Definition: Deep mixing is a reduced-order planning hypothesis for testing whether conservative composition transport could extend a solar-mass star's main-sequence lifetime. It is not a solved solar intervention.

In this repo, the plan is captured in the Deep Mixing autopilot preset and the tachocline setpoint helpers.

Key parameters:
- epsilon: requested gross-circulation hypothesis relative to the hydrogen-burning reference ledger; it is not a lifetime-extension result.
- areaFraction: fraction of the tachocline belt engaged.
- targetDeltaT_Myr: requested optimization threshold, not a predicted consequence of epsilon.
- transportEfficiency: diagnostic fraction of gross circulation that reaches the declared receiving region.
- alpha: cumulative net hydrogen delivery divided by the envelope hydrogen reservoir; alpha is derived and is not equal to epsilon.

Key outputs:
- vrSetpoint_mps: radial downflow setpoint at the tachocline.
- duty, cadenceDays: synthetic planning trim outputs. Synthetic telemetry cannot authorize actuation.
