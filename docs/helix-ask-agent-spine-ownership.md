...
| Terminal candidate materialization | PARTIAL_SERVICE_OWNER | materializer services, final-answer composer, `receipt-framing-suppression.ts` post-observation draft cleanup | `server/routes/agi.plan.ts` | family-specific terminal candidates and repair glue | final draft, route/product contract, support refs | terminal candidate artifacts | final answer draft, materialized terminal candidate, cleaned draft text | terminal authority single writer | finalizer, post-tool bridge | terminal authority | terminal materializer tests, post-observation draft text boundary test | candidate creation and selection not fully separated |
...
| `runtime_composer_fallback_text` | `runtime/runtime-composer-fallback-text.ts` | service via route import | no inline implementation | scholarly and internet-search observation payloads plus support-ref collectors | yes, candidate text can be used as draft input | yes for draft support only | runtime-composer-fallback-text boundary test, scholarly research tool test | SERVICE_OWNED |
...
| S136 | runtime composer fallback text | `server/services/helix-ask/runtime/runtime-composer-fallback-text.ts` | SERVICE_OWNED for deterministic scholarly and internet-search fallback text construction only |
