...
| `note_arg_boundaries` | `note-arg-boundaries.ts` | service factory via route import | no inline implementation | supplied note/action argument text plus route-owned bounded-note-args flag getter | yes, trimmed strings, basic text/title/create-note title arg reads, create-note intent/masking results, deictic note predicate results, and artifact destination predicate results can feed workspace note/clipboard action planning | yes for prompt argument trimming, basic text/title/create-note title readers, create-note intent/masking, deictic/invalid-title predicates, and artifact destination predicates only | note-arg-boundaries boundary test | SERVICE_OWNED |
...
- Note/workspace action argument boundary trimming is service-owned after S153
  ...
  helpers are service-owned after S170; route-owned note-target resolution,
