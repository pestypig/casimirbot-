...
| `note_arg_boundaries` | `note-arg-boundaries.ts` | service factory via route import | no inline implementation | supplied note/action argument text plus route-owned bounded-note-args flag getter | yes, trimmed strings, basic text/title arg reads, and deictic note predicate results can feed workspace note action planning | yes for prompt argument trimming, basic text/title readers, and deictic/invalid-title predicates only | note-arg-boundaries boundary test | SERVICE_OWNED |
...
- Note/workspace action argument boundary trimming is service-owned after S153
  ...
  predicates are service-owned after S168; route-owned note-target resolution,
  workspace note matching, action planning, execution, terminal materialization,
