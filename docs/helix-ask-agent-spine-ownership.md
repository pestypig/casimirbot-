...
| `doc_args` | `doc-args.ts` | service and service factory via route import | no inline implementation | supplied transcript text plus route-owned structured-docs prompt guard | yes, explicit doc-path args and latest-doc/open-doc topic reader results can feed doc context/planning | yes for doc prompt argument and latest-doc prompt interpretation only | doc-args boundary test | SERVICE_OWNED |
...
| S152 | doc path args | `server/services/helix-ask/doc-args.ts` | SERVICE_OWNED for explicit doc-path argument extraction only |
...
- Artifact text normalization and artifact-store text lookup are service-owned
  ...
  terminal materialization, terminal authority, and projection remain unresolved.
- Explicit doc-path argument extraction is service-owned after S152, and
  latest-doc topic/open-doc prompt readers are service-owned after S175;
