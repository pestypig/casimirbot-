...
The extraction wave now continues through S248. Recent slices moved small,
...
| S245-S247 | live-source mail observation readers | `server/services/helix-ask/live-source/mail-observation-readers.ts` | SERVICE_OWNED for already-materialized live-source observation, processed mail packet reading, and current-batch mail-id collection only |
| S248 | live-source mail progress refs | `server/services/helix-ask/live-source/mail-progress-refs.ts` | SERVICE_OWNED for mail-loop progress-ref prefix matching and recursive collection only |
