...
| `composite_followup_helpers` | `composite-followup-helpers.ts` | service via route import | no inline implementation for extracted helpers | supplied composite artifacts, subgoal/status records, and follow-up transcript text | yes, helper outputs feed route-owned composite receipt/handoff and pending request paths | yes for latest artifact lookup, summary rendering, handoff hint construction, and subgoal-reference intent classification only | composite-followup-helpers boundary test | SERVICE_OWNED |
| `composite_subgoal_detector` | `composite-subgoal-detector.ts` | service via route import | no inline implementation for extracted detector | supplied turn id, transcript text, and shared workspace action registry | yes, detected subgoals feed route-owned composite receipt/execution planning | yes for compound prompt normalization, prompt-span lookup, and deterministic composite subgoal detection only | composite-subgoal-detector boundary test | SERVICE_OWNED |
...
| `stage_play_fallback_text` | `live-source/stage-play-fallback-text.ts` | service via route import | no inline implementation | already-materialized Stage Play checkpoint request, job-plan, and reflection tool observations | yes, deterministic Stage Play support text feeds route-owned final-answer composer fallback paths | yes for deterministic checkpoint request, job-plan, and reflection fallback text rendering plus pure source/ref/missing-evidence/projection-label helpers only | Stage Play fallback text boundary test | SERVICE_OWNED |
| `stage_play_checkpoint_receipt_readers` | `live-source/stage-play-checkpoint-receipt.ts` | service via route import | no inline implementation for extracted readers/ref collectors | already-materialized Stage Play reflection tool observations, graph records, source-window records, and live-answer projection records | yes, read results and refs feed route-owned checkpoint receipt recording and ledger mutation | yes for reflection observation lookup, graph/source-window/projection readers, checkpoint evidence-ref collection, and source-window receipt-ref collection only | Stage Play checkpoint receipt boundary test | SERVICE_OWNED_READERS_ONLY |
...
The extraction wave now continues through S274. Recent slices moved small,
...
| S271 | Stage Play fallback text | `server/services/helix-ask/live-source/stage-play-fallback-text.ts` | SERVICE_OWNED for deterministic Stage Play checkpoint request, job-plan, and reflection fallback text rendering only; checkpoint receipt recording, queue completion, terminal materialization, authority, projection, and live behavior remain route-owned |
| S272 | Stage Play checkpoint receipt readers | `server/services/helix-ask/live-source/stage-play-checkpoint-receipt.ts` | SERVICE_OWNED_READERS_ONLY for Stage Play reflection observation lookup, graph/source-window/live-answer projection record reads, checkpoint evidence-ref collection, and source-window receipt-ref collection only; checkpoint receipt recording, queue completion, payload/ledger mutation, terminal materialization, authority, projection, and live behavior remain route-owned |
