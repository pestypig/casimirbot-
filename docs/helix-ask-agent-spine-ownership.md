...
| `artifact_text` | `artifact-text.ts` | service via route import | no inline implementation | supplied artifact-like values, artifact store, or current-turn artifact ledger | yes, normalized text and payload records can feed draft/support text construction and ledger scans | yes for pure text, merge, lookup, and payload-record extraction only | artifact-text boundary test | SERVICE_OWNED |
...
| S151/S231 | artifact text and ledger reader helpers | `server/services/helix-ask/artifact-text.ts` | SERVICE_OWNED for pure artifact text normalization, artifact-store text lookup, ledger artifact merge/lookup, and payload-record reading only |
