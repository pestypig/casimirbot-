...
| `value_readers` | `value-readers.ts` | service via route import | no inline implementation | supplied primitive values, action-like args records, and payload workspace snapshots | yes, trimmed strings can feed every downstream Helix Ask field reader without owning semantics | yes for primitive non-empty string trim/null reading, action-argument string reading, and workspace snapshot active-doc path reading only | value-readers boundary test | SERVICE_OWNED |
...
| S234/S236 | primitive and payload value readers | `server/services/helix-ask/value-readers.ts` | SERVICE_OWNED for non-empty string trim/null reading, action-argument string reading, and workspace snapshot active-doc path reading only |
