...
| Continuation and solver handoff | PARTIAL_SERVICE_OWNER | `runtime/observation-decision.ts`, `runtime/runtime-intent-packet.ts`, solver-controller payload adapter | `server/routes/agi.plan.ts` | runtime-loop continuation/handoff order | observation decision, goal satisfaction, canonical goal frame, available capabilities | continue/finalize/typed failure candidate, runtime intent packet | decision records, pending requirements, runtime intent packet, runtime audit packet ref | final selected terminal unless authority passes | runtime loop, debug export wrapper | finalizer, hard gates, runtime authority audit | observation characterization, solver-controller tests, runtime-intent-packet boundary | final continuation still route-orchestrated |
