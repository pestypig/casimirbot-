...
| `voice_output_intent` | `voice-output-intent.ts` | service via route import | no inline implementation | supplied transcript text | yes, Dottie voice readout intent result can feed route-owned voice/doc-readout routing | yes for Dottie voice readout prompt predicate only | voice-output-intent boundary test | SERVICE_OWNED |
...
- Explicit doc-path argument extraction is service-owned after S152,
  ...
  open-doc goal detection is service-owned after S191, and Dottie voice
  readout detection is service-owned after S192;
