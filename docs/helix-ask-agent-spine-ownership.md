...
| `note_arg_boundaries` | `note-arg-boundaries.ts` | service factory via route import | no inline implementation | supplied note/action argument text plus route-owned bounded-note-args flag getter | yes, trimmed strings can feed workspace note action planning | yes for prompt argument trimming only | note-arg-boundaries boundary test | SERVICE_OWNED |
...
- Note/workspace action argument boundary trimming is service-owned after S153
  behind a route-owned feature-flag getter; route-owned note-target resolution,
