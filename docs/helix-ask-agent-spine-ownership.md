...
| `runtime_composer_fallback_text` | `runtime/runtime-composer-fallback-text.ts` | service via route import | no inline implementation | scholarly, internet-search, and workspace OS status observation payloads plus support-ref collectors | yes, candidate text can be used as draft input | yes for draft support only | runtime-composer-fallback-text boundary test, scholarly research tool test | SERVICE_OWNED |
...
| S136/S143 | runtime composer fallback text | `server/services/helix-ask/runtime/runtime-composer-fallback-text.ts` | SERVICE_OWNED for deterministic scholarly, internet-search, and workspace OS status fallback text construction only |
