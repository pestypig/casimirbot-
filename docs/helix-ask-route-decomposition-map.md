...
Pinned source state described: S140 runtime-civilization-bounds-composer-guard extraction.
...
| `runtime-composer-fallback-text` | service-owned | `EVIDENCE_REENTRY` / `FINAL_ANSWER_COMPOSITION_SUPPORT` | `MEDIUM_LOW` | `EXTRACTED` | `server/services/helix-ask/runtime/runtime-composer-fallback-text.ts` | S136 moved deterministic scholarly and internet-search fallback text builders into a dedicated runtime service. It does not move final-answer draft selection, LLM invocation, terminal materialization, terminal authority, or projection behavior. |
...
| `runtime-composer-fallback-text` | final-answer composition / evidence re-entry | service-owned | Build deterministic scholarly and internet-search fallback text from already-materialized observation artifacts. | Extracted by S136 as pure text builders backed by the support-ref collectors. It does not choose authority, invoke an LLM, materialize terminal products, or mutate payload/debug state. | `EXTRACTED` |
...
10. `runtime-composer-fallback-text` - `EXTRACTED_S136`
