...
Pinned source HEAD described: `45719f648012f9a3ec80b6446298e0179063f4cc`
...
| Lines | 179,526 |
| Bytes | 8,057,629 |
| Top-level helper estimate | 317 helper blocks |
...
| `objective-scoped-recovery` | objective loop / evidence re-entry | partially service-owned | Apply objective scoped-recovery gate bypass, target selection, max-attempt calculation, escalation hints, target expansion, and variant scoring. | S123 routed the behavior-identical helper subset through `server/services/helix-ask/retrieval/objective-scoped-recovery.ts`. Query-variant construction and scoped-retrieval requirement enforcement stay route-owned until service query merging and optional-slot filtering differences are resolved. | `PARTIAL_EXTRACTED` |
