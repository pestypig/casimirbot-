...
Pinned source HEAD described: `4dfe686471aaeae35abb41c7331aa0e5c30d13a8`
...
| Lines | 179,368 |
| Bytes | 8,052,581 |
| Top-level helper estimate | 314 helper blocks |
...
| `objective-llm-contracts` | objective loop / evidence re-entry | partially service-owned | Render deterministic objective retrieve-proposal prompts, parse retrieve-proposal JSON packets, and parse mini-synth/mini-critic outputs. | S125 routed retrieve-proposal prompt rendering and parser helpers through `server/services/helix-ask/objectives/objective-llm-contracts.ts`. S127 removed the route-local mini-synth/mini-critic parser block and routes those parser calls through the same LLM-contract owner. LLM invocation, repair loops, and objective assembly policy remain route-owned. | `PARTIAL_EXTRACTED` |
