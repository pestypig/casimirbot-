...
Pinned source HEAD described: `6d016d401` plus pending S132 runtime-continuation-hints extraction.
...
| `runtime-continuation-hints` | `legacy-private-runtime-loop` / solver handoff | service-owned | Build non-authoritative continuation hints, append them to payload/ledger/debug, collect agent decisions, match accepted hints, and mark migration status. | Extracted by S132 behind a seven-callback dependency interface for capability keys, decision args, string reads, artifact payloads, hashing, ledger merge, and time. | `EXTRACTED` |
