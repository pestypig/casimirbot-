...
magnetized plasma coherence (flux ropes / current channels) — inspired by solar surface
organization — while keeping nuclear tunneling and S-factor physics unchanged.
...
4) How to translate the Sun’s quasi-periodic coherence into device-agnostic control parameters.
...
- [x] Phase 0 — Inventory + contracts
  - [x] Baseline curvature-unit tool exists: `server/skills/physics.curvature.ts` (2D Gaussian sources → Poisson → Essence envelope).
  - [x] Baseline schema exists: `shared/essence-physics.ts` (`CurvatureUnitInput`, `CurvatureUnit`).
  - [x] Solar ingestion / SunPy “bridge” already exists: `server/routes/star-watcher.ts` (`/sunpy-export`, `/solar-coherence`, `/solar-events`) + `server/services/essence/sunpy-coherence-bridge.ts` (300 s bins) + `tools/hek_movie_export.py` (SunPy `Fido` exporter).
  - [x] Prediction-safe “observables vs labels” guardrails exist (use these for forecasting runs):
  ...
  - [x] Add an “information boundary” contract for all derived artifacts:
  ...
  - [x] Define a shared “curvature proxy” contract in one place and make all code paths use it (same constants, same prefactors, same units):
    ...
    - Optional diagnostic: `kappa_u(u) = (8*pi*G/(c^4)) * u` (energy-density curvature yardstick; used in the ledger’s EM section).
  ...
  - [x] Add a shared “collapse benchmark” schema + math contract (benchmark/proxy, not a claim of confirmed wavefunction-collapse physics):
    ...
    - Discrete-time helper: per-step hazard `p(dt)=1-exp(-dt/tau)` so voxel sims can sample “collapse events” without superluminal effects.
  ...
  - [x] Define where “results of record” live (recommended: Essence envelopes + stable hashes + dataset manifest). (Decision: Essence envelopes + stable hashes; manifests define repeatable runs.)
...
- [ ] Phase 1.5 — Collapse benchmark engine (unit-consistent)
...
- [x] Phase 1 — Curvature unit v2 (accept real energy fields)
  ...
  - [x] Add tests: decode/encode invariance, residual thresholds, determinism (same input → same hashes): `tests/curvature-unit.v2.spec.ts`.
  - DoD: `npm test` passes and a 256×256 raster field produces φ + residuals deterministically. (Covered by `tests/curvature-unit.v2.spec.ts`.)
...
- [x] Phase 3 — Coherence metrics + ridge tracking (K0–K3)
  - [x] Implement K0–K3 from φ (K0 = normalized |∇²φ| percentile; K1 = ridge score on |∇φ|; K2 = residual fraction; K3(t) = phase-lock score over time).
  - [x] Implement ridge extraction (NMS + hysteresis thresholding is enough for v1) and a stable ridge “spine” representation.
  - [x] Implement ridge tracking across time bins (IDs, lifetime, fragmentation rate).
  - [x] Add tests on synthetic fields (two Gaussians, translated/rotated ridge) for determinism + stability.
  - DoD: Experiment A outputs a “rope coherence index” time series + event-aligned summary.

- [ ] Phase 4 - SPARC-class tokamak adapter (R-Z equilibrium grid + USD export hooks)
  - [x] Define a tokamak snapshot schema for an R-Z cross-section: grid extents, spacing, coordinate frame, and a plasma-domain mask (inside separatrix).
  - [x] Require equilibrium reconstruction inputs (B_eq, p_eq) plus time-aligned perturbation channels.
  - [x] Add explicit channels and weights for `u_total`:
    - deltaB energy: `u_deltaB = |B - B_eq|^2 / (2 * mu0)`
    - pedestal proxy: `u_gradp = |grad p|` or `p - p_eq`
    - current proxy: `u_J = |J|^2` (structure channel, not literal energy)
    - radiation/impurity proxy: `u_rad` from bolometry/SXR emissivity
  - [x] Persist the channel manifest (weights + normalizations + constants) alongside every run for reproducibility.
  - [x] Add a small synthetic R-Z dataset under `datasets/` with a known onset label (edge crash / tearing onset) and a reference mask.
  - [x] Run baseline precursor curves (u_total p95) and compute ROC/AUC.
  - [ ] Export Omniverse/USD artifacts: phi, |grad phi|, residual fields as textures on the R-Z plane; ridge spines as USD curves; K-metrics as time series.
  - [x] Add an offline CLI to generate a report JSON (and optional plots) for Experiment B.
  - DoD: Experiment B produces precursor curves, USD overlays, and a baseline metric (ROC/AUC or similar) with a documented manifest hash.

- [ ] Phase 5 — Tunneling do-no-harm validation (mesh adaptation)
...
- [ ] Phase 6 (optional) — 3D volumes + acceleration
  ...
  - [ ] Wire a 3D viewer overlay for φ/|∇φ| and ridge volumes.
  - DoD: a bounded 128³ case solves + visualizes with determinism and acceptable latency.
...
- Recommended “features-only” export call pattern: `/api/star-watcher/sunpy-export?...&asOf=<end>&bridgeMode=observables&includeEvents=false&bridge=off`.
- Recommended “labels-only” call pattern (after the horizon): `/api/star-watcher/solar-events?start=<t0>&end=<t1>&eventTypes=FL`.
...
- Observable “coherence windows”: quasi-periodic bands (e.g., ~5-minute p-mode range) treated as
...
## Curvature-unit workflow (data → diagnostic fields)
### Step 1: Build an energy density field u(x,y,z,t)
From simulation or inferred observation:
- u_B = B^2 / (2 μ0)
- u_E = ε0 E^2 / 2 (if relevant)
- u_th ≈ (3/2) n k T  (or appropriate EOS)
- u_kin = (1/2) ρ v^2
...
Feed (grid + constants + ρ_eq or sources) into the curvature-unit handler.
Collect:
- φ(x): Poisson potential
- ∇φ and/or ∇²φ proxies
- residual / roots / envelope outputs

### Step 4: Derive coherence metrics (examples)
Define a few device-agnostic metrics:
- K0 = normalized |∇²φ| percentile (structure intensity)
- K1 = filamentarity / ridge score on |∇φ|
...
## Minimal experiments
### Experiment A — Solar: coherence segmentation
1) Take a time series of B(x,y,t) and compute u_B.
2) Run curvature unit to get φ, |∇φ|, residual.
...
### Experiment B — Device: instability precursor index
...
### Experiment C — Tunneling: do-no-harm validation
...
## How to translate the solar “5-minute coherence” idea to lab control
Do NOT copy 3.3 mHz literally.
Instead define a *dimensionless coherence drive parameter*:

χ = f_drive * τ_char

Where τ_char might be:
- Alfvén time τ_A,
- energy confinement time τ_E,
- dominant instability e-folding time 1/γ.

Control hypothesis:
- Choose f_drive such that χ sits in a “phase-lock plateau” where K2(t) is minimized.
