...
for row in [1.,1e3,1e4,1e5,1e6]:
 ...
 def fold(floor,n):
  ...
  amp=1e-18/(4*np.pi)*(1e6+row**2)/(qs**2+row**2)
 ...
 rows.append(dict(mediator_eV=row,W_at_1keV_GeV_minus2=1.,variants=variants))
out=dict(status='conditional_covered_detector_prediction_not_exclusion',
 ...
 assumptions=['fixed mono-speed flux at XENON1T, without transport','published ER response including selections','linear response interpolation in log energy','same covered atomic K response and Yukawa Wref=1 GeV^-2 at qref=1 keV'],
