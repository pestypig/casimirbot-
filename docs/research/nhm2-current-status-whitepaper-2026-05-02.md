...
| `axis-aligned-shear-suppressed-smoke-v1/nhm2-time-dependent-source-campaign.json` | `a42d6e122b77d117093c9dc68a7997f7739b66c91c5acbca8d2506e3f7479928` |
...
| Axis-aligned shear-suppressed candidate | A separate `axis-aligned-shear-suppressed-smoke-v1` branch sets the declared source-side off-diagonal components to zero with explicit axis-aligned shear-suppression provenance. This removes the current-declared-source-model falsifier (`currentDeclaredSourceModelFalsified = false`) but does not pass the campaign. The candidate campaign first blocker moves to `hull:T01:full_tensor_residual_exceeded` in the `momentum_t0i` family, with `worstRelResidual = 1.0011044101591493`. | This is useful frontier movement, not physical progress by itself: the next source-side problem becomes momentum-density closure, while observer robustness and stability evidence remain non-pass. |
