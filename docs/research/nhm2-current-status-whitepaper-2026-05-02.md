...
| `profile-search/nhm2-campaign-profile-run-manifest-latest.json` | `3f957a642ee7c2323928539ac38e1c3aa83d44917ded4aface0bc8a57e44824f` |
...
| Campaign profile run manifest | The new `nhm2_campaign_profile_run_manifest/v1` sidecar queues only screen-passing candidates for frozen campaign evidence generation. It queues `stage1_centerline_alpha_0p9000_combined_metric_redesign_campaign_screen_v1` first. The current generated candidate row now provides the reduced-order projected momentum-demand audit and remediation target artifacts: all screened `T0i` rows are inside the causal ratio bound, the worst row is `hull:T02` at about `0.044888`, and `remediationRequired = false` under the scaled screen. It also provides a blocked metric-required full-regional-tensor screen: `T0i` is projected from the reduced-order screen, but `T00` and `Tij` are inherited from the parent profile, so all regions carry `candidate_metric_required_full_tensor_screen_not_full_adm_route`. The manifest still marks source/tile compatibility, regional full tensor residuals, switching conservation, frequency convergence, dynamic/effective geometry agreement, QEI dossier, observer-family checks, horizon/blueshift/particle/stability, and the final time-dependent campaign artifact as required/missing. | This is run scheduling and evidence governance only. The new candidate rows clear the reduced-order projected-momentum screen for this candidate and expose the next metric-side blocker, but they do not supply a full ADM tetrad, frozen campaign, profile validation, or any physical / transport / route / propulsion claim. |
