...
| `nhm2-time-dependent-source-campaign.json` | `ecb241e233aa68162f89be8f7c7ee2263d95173ccf582bf2a2083234b2f27cb3` |
...
| Full regional tensor closure | `fullRegionalTensorClosurePass = false`; the current campaign first blocker is `hull:T13:full_tensor_residual_exceeded`, and the residual sidecar now classifies that frontier as `off_diagonal_tij`. Same-run metric-required full-tensor components are now represented, but the full-tensor residual gate fails; hull, wall, and exterior `T00` are under the reduced-order `0.1` threshold while global `T00`, momentum density terms, and off-diagonal spatial stresses remain out of tolerance. | This is a quantified source-side full-tensor mismatch, not a missing-component blocker and not a physical closure pass. |
...
Current static smoke-chain evidence cannot pass that campaign by itself. The June 18 dynamic-evidence sidecars first moved the campaign blocker from `switching_conservation_evidence_missing` to `dynamic_effective_geometry_evidence_missing`, then to `dynamic_geometry_ref_missing`, then to `dynamic_geometry_samples_missing`, then to `effective_geometry_ref_missing`, then to `averaged_source_tensor_ref_missing`, then to `backreaction_residual_not_bounded`, then to `regional_full_tensor_components_missing`, and then to quantified full-tensor residuals. The newest frontier receipts consume a same-run metric-required regional full-tensor source and identify the dominant current blocker as `hull:T13:full_tensor_residual_exceeded` in the `off_diagonal_tij` family: switching conservation, frequency convergence, runtime geometry references, averaged source tensor provenance, bounded dynamic/effective channel residuals, and metric-side full tensor components can now be represented as diagnostic receipts, but the campaign still fails source-side off-diagonal spatial-stress closure, observer robustness, and stability evidence. This is a falsification ledger for the next frozen dynamic run, not a physical viability claim, route ETA claim, propulsion claim, or transport certificate.
