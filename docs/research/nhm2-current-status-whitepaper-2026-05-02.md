...
| `axis-aligned-shear-suppressed-smoke-v1/nhm2-time-dependent-source-campaign.json` | `e8fbc9e7bcab18cd7fd62ce673b2d4116dc533451b0f50c148b48a53ba078ea1` |
...
| Axis-aligned shear-suppressed candidate | A separate `axis-aligned-shear-suppressed-smoke-v1` branch sets the declared source-side off-diagonal components to zero with explicit axis-aligned shear-suppression provenance. This removes the off-diagonal current-declared-source-model shear falsifier (`currentDeclaredSourceModelFalsified = false` in the shear audit) but does not pass the campaign. With the momentum-density audit attached, the candidate campaign first blocker becomes `momentum_density_causal_bound_frame_projection_missing`. The raw residual symptom remains in the `momentum_t0i` family, including `hull:T01:full_tensor_residual_exceeded` with `worstRelResidual = 1.0011044101591493`. | This is useful frontier movement, not physical progress by itself: the next problem is to provide local orthonormal / projection evidence before interpreting the raw metric-required `T0i/T00` chart-component ratio as a causal-material bound, while still addressing source-side momentum mechanism evidence. Observer robustness and stability evidence remain non-pass. |
...
A new diagnostic branch, `axis-aligned-shear-suppressed-smoke-v1`, tests the opposite source-side hypothesis: explicit zero off-diagonal shear with axis-aligned suppression provenance. That branch removes the tuned-source shear falsifier and no longer reports a missing shear mechanism, but it still fails full regional tensor closure because the metric-required same-chart tensor has nonzero momentum-density and off-diagonal entries. With the momentum-density sidecar attached, the first campaign blocker becomes `momentum_density_causal_bound_frame_projection_missing`, with the raw residual family reported as `momentum_t0i`. The sidecar shows that the declared candidate carries uniform fractional `T0i` rows but lacks a flux / current / Poynting / constitutive momentum mechanism, while the required amplification to enter the reduced-order pass window reaches about `2.02e22` on `T02`. The metric-required side also shows raw momentum-to-energy ratios up to about `2.24e15` in this same-chart comparison, but the current atlas is chart-basis, so that ratio is not yet promoted to a causal-material no-go without a local orthonormal / observer-frame projection receipt. This is a better localized failure, not a pass: it says the next source-side model must address momentum-density mechanism evidence and the metric/source projection basis before causal-material momentum-bound language is admissible, while observer robustness and horizon/blueshift/particle/stability evidence remain non-pass. Switching conservation, frequency convergence, runtime geometry references, averaged source tensor provenance, bounded dynamic/effective channel residuals, metric-side full tensor components, and regional sampling metadata can now be represented as diagnostic receipts, but the campaign still fails source-side full tensor closure, observer robustness, and stability evidence. This is a falsification ledger for the next frozen dynamic run, not a physical viability claim, route ETA claim, propulsion claim, or transport certificate.
