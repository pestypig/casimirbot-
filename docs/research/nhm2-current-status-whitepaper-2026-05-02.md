...
The June pass-path / smoke-chain state should be read as pinned local evidence, not as a broad publication-ready closure result. In the `qei-worldline-receipted-smoke-v1` artifact set, the source component authority ledger can mark required source components authoritative and the wall `T00` residual can fall below the reduced-order `0.1` tolerance. That is narrower than "wall closure passed": coupled closure and the regional pass-path harness still return false, observer robustness still fails or remains incomplete, and physical / transport claims remain forbidden. The newer `axis-aligned-shear-suppressed-smoke-v1` branch sharpens the frontier further: once off-diagonal shear is suppressed and reduced-order local-frame projection evidence is attached, the current selected metric profile is flagged non-resolvable under the declared projected momentum-demand audit, with a worst required metric-side momentum suppression of about `2.244e15`. The follow-on `nhm2_campaign_profile_search/v1` sidecar begins the next step: it rejects alpha-only profile changes as insufficient for the projected `T0i` blocker and identifies combined metric-redesign candidates that must still be run through a full frozen campaign. The paired `nhm2_campaign_profile_run_manifest/v1` sidecar then turns those screened candidates into a frozen-run evidence checklist. In the current generated manifest, the `0p9000` combined metric-redesign candidate is still queued first and the reduced-order projected momentum-demand audit/remediation rows are now provided for that candidate: the worst screened row is `hull:T02`, with projected metric-required momentum-to-energy ratio about `0.044888`, and the remediation target screen reports `remediationRequired = false`. The candidate now also has a `nhm2_candidate_metric_profile_spec/v1` row that records diagnostic clocking, profile levers, and ADM-route entry conditions. That row is `provided_blocked`: `tau = 0.9 T_coordinate` is replayable as a diagnostic clocking calculation, but executable candidate shift-field geometry, regional support atlas, and grid refs are still missing. The campaign first blocker is therefore `candidate_executable_shift_field_evaluator_missing`. The older metric-required full-regional-tensor screen remains `provided_blocked` as secondary evidence because `T0i` is projected from the reduced-order momentum screen while `T00` and `Tij` remain inherited parent-profile placeholders. This only retires the old projected-momentum screen blocker for that candidate; it does not provide a fresh ADM route, and the remaining frozen campaign rows are still required/missing.
...
| Full regional tensor closure | `fullRegionalTensorClosurePass = false`; the current campaign first blocker is now `source_off_diagonal_current_declared_model_falsified`. The underlying numeric symptom remains `hull:T13:full_tensor_residual_exceeded` in the `off_diagonal_tij` family. The current source-side hull `T13` magnitude is about `1.49e15×` outside the pass-window magnitude implied by the reduced-order residual tolerance. Same-run metric-required full-tensor components and source-side regional sample metadata are now represented, but off-diagonal spatial-stress residuals still fail. | This is a scoped falsifier for the current declared source model, not a permission to copy the metric-required tensor, not a universal no-go theorem for all possible sources, and not a physical closure pass. |
...
| Campaign profile search | The new `nhm2_campaign_profile_search/v1` sidecar consumes the remediation target and frontier disposition. It rejects alpha-only `0p9900`, `0p9500`, `0p9000`, and `0p7000` candidates because lowering `alpha` alone does not reduce the projected metric-required `T0i` blocker. In the initial screen, the fastest candidate that clears the reduced-order frontier screen is `stage1_centerline_alpha_0p9000_combined_metric_redesign_campaign_screen_v1`, with `firstBlocker = full_frozen_campaign_run_required`. | This is profile-search steering only. A screen pass is not a campaign pass, not a profile validation, not a route ETA, not a transport claim, and not physical viability. |
| Candidate metric profile spec | The new `nhm2_candidate_metric_profile_spec/v1` sidecar records the `0p9000` combined metric-redesign profile definition, diagnostic clocking replay, and ADM-route entry requirements. It reports `tau = 0.9 T_coordinate`, `shipProperTimeSeconds = 123980369.3253` for the Alpha-Centauri-scale coordinate-time input, and `clockSavingSeconds = 13775596.5917`, but `admRouteReady = false`. The first blocker is `candidate_executable_shift_field_evaluator_missing`, with missing regional atlas and grid refs also listed. | This is profile-definition and ADM-entry governance only. Trip clocking is replayable but not a route ETA, speed, transport, propulsion, or physical viability claim. |
| Campaign profile run manifest | The new `nhm2_campaign_profile_run_manifest/v1` sidecar queues only screen-passing candidates for frozen campaign evidence generation. It queues `stage1_centerline_alpha_0p9000_combined_metric_redesign_campaign_screen_v1` first. The current generated candidate row now provides the candidate metric profile spec as `provided_blocked`, plus the reduced-order projected momentum-demand audit and remediation target artifacts: all screened `T0i` rows are inside the causal ratio bound, the worst row is `hull:T02` at about `0.044888`, and `remediationRequired = false` under the scaled screen. It also provides a blocked metric-required full-regional-tensor screen: `T0i` is projected from the reduced-order screen, but `T00` and `Tij` are inherited from the parent profile, so all regions carry `candidate_metric_required_full_tensor_screen_not_full_adm_route`. The manifest still marks source/tile compatibility, regional full tensor residuals, switching conservation, frequency convergence, dynamic/effective geometry agreement, QEI dossier, observer-family checks, horizon/blueshift/particle/stability, and the final time-dependent campaign artifact as required/missing. | This is run scheduling and evidence governance only. The new candidate rows clear the reduced-order projected-momentum screen for this candidate and expose the next metric-profile blocker, but they do not supply executable candidate geometry, a full ADM tetrad, frozen campaign, profile validation, or any physical / transport / route / propulsion claim. |
...
| Coupled closure | `passCandidate = false`; first blocker is currently `global_source_side_authority_incomplete` in the smoke artifact. | No diagnostic closure candidate yet. |
| Regional tensor pass-path harness | `numericalPassPathReady = false`; first blocker is `regional_material_source_tensor_model_missing` in this harness artifact, while the campaign and residual sidecars carry the newer source-model and off-diagonal residual evidence. | Harness still blocks pass-path readiness and needs source-model wiring synchronized with the regenerated campaign chain. |
...
Current static smoke-chain evidence cannot pass that campaign by itself. The June 18 dynamic-evidence sidecars first moved the campaign blocker from `switching_conservation_evidence_missing` to `dynamic_effective_geometry_evidence_missing`, then to `dynamic_geometry_ref_missing`, then to `dynamic_geometry_samples_missing`, then to `effective_geometry_ref_missing`, then to `averaged_source_tensor_ref_missing`, then to `backreaction_residual_not_bounded`, then to `regional_full_tensor_components_missing`, then to quantified full-tensor residuals, then from missing global metric aggregation metadata to `global:source_evidence:sample_count_mismatch`, then to the explicit tensor residual `hull:T13:full_tensor_residual_exceeded`, then to `source_off_diagonal_shear_falsifier_candidate`, and now to `source_off_diagonal_current_declared_model_falsified`. The newest frontier receipts consume a same-run metric-required regional full-tensor source and use the generated metric-required regional receipt as the source-side sample plan, so the campaign is no longer blocked by sample-count metadata in this smoke chain. The full-tensor gate still identifies the dominant numeric residual as `hull:T13` in the `off_diagonal_tij` family, with a current-to-allowed magnitude ratio of about `1.49e15`. The paired shear-audit sidecar adds that the current source tensor has declared off-diagonal components but lacks documented shear/anisotropic/constitutive mechanism evidence. It further shows the declared source is a uniform fractional-shear ansatz with `T13` fixed at `0.0005|T00|`, while the metric-required off-diagonal fractions are far smaller; the strongest required suppression/reorientation factor is about `5.83e15`. This is now a current-declared-source-model falsifier at the full-tensor residual layer, rather than a universal source impossibility or an evidence-intake blocker.

A new diagnostic branch, `axis-aligned-shear-suppressed-smoke-v1`, tests the opposite source-side hypothesis: explicit zero off-diagonal shear with axis-aligned suppression provenance. That branch removes the tuned-source shear falsifier and no longer reports a missing shear mechanism, but it still fails full regional tensor closure because the metric-required same-chart tensor has nonzero momentum-density and off-diagonal entries. With the momentum-density sidecar, reduced-order frame-projection receipt, metric-required momentum demand audit, remediation-target sidecar, and campaign frontier disposition attached, the first campaign blocker is now `metric_momentum_current_profile_rejected_for_campaign`, with the raw residual family reported as `momentum_t0i`. The source-side sidecar shows that the declared candidate carries uniform fractional `T0i` rows but lacks a flux / current / Poynting / constitutive momentum mechanism, while the required amplification to enter the reduced-order pass window reaches about `2.02e22` on `T02`. The projection evidence admits the same-chart momentum-to-energy fractions as reduced-order local-frame ratios, so the metric-required side exposes a projected momentum-to-energy ratio up to about `2.2444057604285525e15`; the source side remains about `1e-6`. The remediation-target artifact converts that falsifier into a design boundary: the worst row is `hull:T02`, with `requiredSuppressionFactor = 2.2444057604285525e15`, `requiredFractionalReduction = 0.9999999999999996`, and `nonResolvableForCurrentProfile = true`. The campaign frontier disposition converts that design boundary into a typed current-profile rejection under the declared reduced-order projected momentum-demand evidence. This is a better localized failure, not a pass: it says the current metric profile is rejected for this campaign unless metric-required projected `T0i` is reduced by profile redesign, a full ADM tetrad / observer-frame calculation changes the ratio, or a different profile is pursued. Observer robustness and horizon/blueshift/particle/stability evidence remain non-pass. Switching conservation, frequency convergence, runtime geometry references, averaged source tensor provenance, bounded dynamic/effective channel residuals, metric-side full tensor components, and regional sampling metadata can now be represented as diagnostic receipts, but the campaign still fails source-side full tensor closure, observer robustness, and stability evidence. This is a falsification ledger for the next frozen dynamic run, not a physical viability claim, route ETA claim, propulsion claim, or transport certificate.
...
### 7.5 Frontier bisection strategy
...
| `nhm2_candidate_metric_profile_spec/v1` | candidate metric profile spec sidecar | records profile levers, diagnostic clocking replay, and executable geometry refs required before a candidate can enter the full ADM tensor route | profile-definition and ADM-entry governance only; trip clocking is not route ETA, speed, transport, propulsion, or physical viability |
| `nhm2_campaign_profile_run_manifest/v1` | campaign profile run-manifest sidecar | turns screen-passing candidate profiles into required frozen-campaign evidence rows and next-run roots | evidence scheduling only; queued candidate does not validate the profile or pass the campaign |
