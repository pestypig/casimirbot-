...
| `profile-search/nhm2-campaign-profile-run-manifest-latest.json` | `7de5b07492e865922862c25d9f604790422f3984ffff34a85dd4473552a7c5a4` |
...
| Campaign profile run manifest | The new `nhm2_campaign_profile_run_manifest/v1` sidecar queues only screen-passing candidates for frozen campaign evidence generation. It queues `stage1_centerline_alpha_0p9000_combined_metric_redesign_campaign_screen_v1` first. The current generated candidate row now provides the reduced-order projected momentum-demand audit and remediation target artifacts: all screened `T0i` rows are inside the causal ratio bound, the worst row is `hull:T02` at about `0.044888`, and `remediationRequired = false` under the scaled screen. The manifest still marks metric-required full tensor, source/tile compatibility, regional full tensor residuals, switching conservation, frequency convergence, dynamic/effective geometry agreement, QEI dossier, observer-family checks, horizon/blueshift/particle/stability, and the final time-dependent campaign artifact as required/missing. | This is run scheduling and evidence governance only. The new candidate rows clear the reduced-order projected-momentum screen for this candidate, not the full ADM tetrad, not the frozen campaign, not profile validation, and not any physical / transport / route / propulsion claim. |
