...
models=json.loads(p.read_text())['rows'];v=776/299792.458;m=1e11;hc=1.973269804e-5
...
for label,h in [('withLFE','467ad77324dec54c4000c10405d8b1de958e6e935ed8368d6906a390c048d229'),('noLFE','cce62a651d8d624c428555e08dd2796a25dac2f73a1979db1e9bc562416f8630')]:
 ...
 for model in models:
  masses=np.array(model['masses_GeV'])*1e9;c=np.array(model['effective_alpha_products'])
  vals=[]
  for n in [512,1024]:
   ...
   amp=np.sum(c/(qs[:,None]**2+masses**2),axis=1)
  ...
  rows.append(dict(response=label,masses_GeV=model['masses_GeV'],covered_D=vals[-1],refinement_relative=drift))
out=dict(status='covered_bulk_electronic_hold_exponent_not_total_coherence',rows=rows,
 formula='D=2 n c M t/(pi alpha_EM v hbarc rho) integral dq q^3 A(q)^2 [1-sinc(qd/hbar)] integral dOmega ELF',
 assumptions=['bulk isotropic zero-temperature electronic response','fixed separated branches during hold','same incoming density and speed and fixed products'],
