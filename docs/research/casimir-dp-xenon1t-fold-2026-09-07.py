...
for row in common['rows']:
 ...
 rows.append(dict(masses_GeV=row['masses_GeV'],variants=variants))
out=dict(status='conditional_covered_detector_prediction_not_exclusion',
 ...
 assumptions=['fixed mono-speed flux at XENON1T, without transport','published ER response including selections','linear response interpolation in log energy','same covered atomic K response and fixed mediator products'],
...
for r in rows:print(r['masses_GeV'],[{k:v for k,v in x.items() if k!='selected_bin_counts'} for x in r['variants']])
