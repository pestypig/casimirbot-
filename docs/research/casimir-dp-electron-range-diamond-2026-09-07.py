...
models=[1.,1e3,1e4,1e5,1e6];v=776/299792.458;m=1e11;hc=1.973269804e-5
...
for label,h in [('withLFE','467ad77324dec54c4000c10405d8b1de958e6e935ed8368d6906a390c048d229'),('noLFE','cce62a651d8d624c428555e08dd2796a25dac2f73a1979db1e9bc562416f8630')]:
 ...
 for model in models:
  ...
  for n in [512,1024]:
   ...
   amp=1e-18/(4*math.pi)*(1e6+model**2)/(qs**2+model**2)
  ...
  rows.append(dict(mediator_eV=model,response=label,D_per_Wref_squared=vals[-1],formal_Wref_for_DP_GeV_minus2=math.sqrt(.029511464722144533/vals[-1]),refinement_relative=drift))
out=dict(status='single_yukawa_electron_density_response_scan',rows=rows,
 formula='D=2 n c M t/(pi alpha_EM v hbarc rho) integral dq q^3 A(q)^2 [1-sinc(qd/hbar)] integral dOmega ELF',
 assumptions=['bulk isotropic zero-temperature electronic response','fixed separated branches during hold','W(q)=Wref*(qref^2+mA^2)/(q^2+mA^2), qref=1 keV, Wref=1 GeV^-2'],
