...
| Provider-neutral environment time and receding-horizon action planning | specified | `docs/architecture/helix-environment-time-action-planning-v1.md`; `docs/work-packets/eh-g8-et-environment-time-receding-horizon-v1.md`; `docs/architecture/helix-environment-agent-reasoning-v1.md`; `docs/architecture/helix-minecraft-dual-plane-adapter-v1.md` | Build ET0–ET8 in order. Minecraft supplies the maximum-capacity tick profile by compiling to the existing finite sequence and concurrent guardian; the shared contract remains strategy-free and adapter-neutral. Measure runway, stalls, reaction latency, evidence volume and duplicate effects before tuning. A second adapter must conform without inheriting Minecraft authority. |
...
Environment Time planning checkpoint (2026-09-03): PNA3.6 exposes the next
capacity boundary—continuous useful motion through model latency while
preserving rapid environment and user interruption. ET0 specifies a
...
progressive affordance frontiers, ordered interruption receipts, separated
feedback-latency spans, Minecraft capacity metrics and second-adapter
conformance. Minecraft remains the maximum-capacity reference and compiles to
the existing `survival_tas` and concurrent guardian engines; it does not define
the shared contract. ET remains `specified`; implementation begins with shared
schemas and adversarial fixtures, not a longer hardcoded Minecraft macro. See
