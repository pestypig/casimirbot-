...
| `com.casimirbot.minecraft.spatial_region.inspect` | `target` (`current_actor`), optional `freshness_requirement_ms` | `helix.environment_connector.probe_observation.v1` | Read-only bounded block-volume evidence around the bound actor for planning and later action verification; it does not mutate or pathfind. |
...
| `com.casimirbot.minecraft.registry.fact.read` | `registry_kind`, `resource_id` | `helix.environment_connector.probe_observation.v1` | Read-only exact membership check against one bounded live Fabric registry. It reports the paired server version and never lists a registry or changes the game. |
| `com.casimirbot.minecraft.recipe.fact.read` | `query_kind`, `resource_id`; optional `max_results` | `helix.environment_connector.probe_observation.v1` | Read-only exact recipe-ID or bounded output-item lookup against the live Fabric recipe manager. Dynamic result displays are explicitly marked incomplete rather than guessed. |
...
| `com.casimirbot.minecraft.player.navigate` | `destination`, `arrival_radius`, `allow_sprint`, `allow_dig`, `allow_place`, `engine_preference`; optional `environment_label` | `helix.environment_action.observation.v1` | Separately paired Player Embodiment action. Exact room/player/world authority, fresh manifest and heartbeat, manual override, cancellation, postcondition verification, and emergency stop are mandatory. |
| `com.casimirbot.minecraft.player.look` | `target_kind`, `max_turn_degrees_per_tick`; target-specific fields and optional `environment_label` | `helix.environment_action.observation.v1` | Turns the bound player's view through the admitted Fabric client; success requires action-specific postcondition evidence rather than dispatch alone. |
| `com.casimirbot.minecraft.player.camera.track` | `target_kind`, `aim_point`, `prediction_ticks`, `max_acquisition_distance`, `max_turn_degrees_per_tick`, `max_angular_acceleration_degrees_per_tick_squared`, `deadband_degrees`, `reacquire_ticks`, `max_duration_ms`, `require_line_of_sight`, `stop_below_health`; target-specific fields and optional `environment_label` | `helix.environment_action.observation.v1` | Predictively tracks one admitted live target for a bounded duration with rate and acceleration limits, manual override, interruption, and postcondition evidence. |
| `com.casimirbot.minecraft.player.walk` | `direction`, `duration_ms`, `sprint`; optional `environment_label` | `helix.environment_action.observation.v1` | Bounded directional movement with manual-input cancellation and a verified motion outcome. |
| `com.casimirbot.minecraft.player.jump` | `count`; optional `environment_label` | `helix.environment_action.observation.v1` | Bounded jump sequence with typed progress, interruption, and settled-result evidence. |
| `com.casimirbot.minecraft.player.interact` | `target`, `hand`, `interaction`; optional `environment_label` | `helix.environment_action.observation.v1` | Uses or interacts with the exact current focus through the paired client; a dispatch receipt alone is not proof that the intended world state changed. |
| `com.casimirbot.minecraft.player.hotbar.select` | `slot`; optional `environment_label` | `helix.environment_action.observation.v1` | Selects one zero-based hotbar slot and verifies the selected-slot state. |
| `com.casimirbot.minecraft.player.equipment.equip` | `item_id`, `destination`; optional `environment_label` | `helix.environment_action.observation.v1` | Moves a matching inventory item to an admitted equipment destination and verifies the resulting equipment state. |
| `com.casimirbot.minecraft.player.follow` | `subject_ref`, `distance`, `max_duration_ms`, `stop_below_health`; optional `environment_label` | `helix.environment_action.observation.v1` | Follows one exact Helix-resolved room player for a bounded interval; the model never receives the native player identity. |
| `com.casimirbot.minecraft.player.collect` | `item_or_block_id`, `count`, `search_radius`; optional `environment_label` | `helix.environment_action.observation.v1` | Collects matching dropped item entities in loaded client range and verifies the resulting inventory increase. |
| `com.casimirbot.minecraft.player.mine` | `block_id`, `count`, `search_radius`; optional `environment_label` | `helix.environment_action.observation.v1` | Mines matching loaded blocks within the admitted radius and verifies each removed block under the mutation ceiling. |
| `com.casimirbot.minecraft.player.place` | `block_id`, exactly one of `positions` or `position_binding`; optional `environment_label` | `helix.environment_action.observation.v1` | Places held blocks at exact admitted positions, or resolves one bounded `predicted_collision_cell` from current measured trajectory inside the admitted mutation region. Support, reach, replaceability, resulting block state, and resolved target identity are verified. |
| `com.casimirbot.minecraft.player.craft` | `output_item_id`, `count`; optional `recipe_id`, `environment_label` | `helix.environment_action.observation.v1` | Uses the current player grid or open crafting table and verifies the crafted player-inventory delta. |
...
| `helix.environment_action.observation.v1`                                       | Player Embodiment action outcome with exact room/participant/player/world/authority, request/workflow/execution identity, progress and postcondition evidence refs, provenance validity, and current-turn re-entry eligibility. It is never an assistant answer.                                      |
| `helix.environment_action.control_observation.v1`                               | Status, resume, or cancel outcome for one exact player workflow, including release state, affected workflow refs, provenance validity, and current-turn re-entry eligibility. It is never an assistant answer.                                                                    |
...
| `com.casimirbot.minecraft.player.navigate`              | `destination`, `arrival_radius`, `allow_sprint`, `allow_dig`, `allow_place`, `engine_preference`                                                                                                                                                                          |
| `com.casimirbot.minecraft.player.look`                  | `target_kind`, `max_turn_degrees_per_tick`                                                                                                                                                                                                                                |
| `com.casimirbot.minecraft.player.walk`                  | `direction`, `duration_ms`, `sprint`                                                                                                                                                                                                                                      |
| `com.casimirbot.minecraft.player.jump`                  | `count`                                                                                                                                                                                                                                                                   |
| `com.casimirbot.minecraft.player.interact`              | `target`, `hand`, `interaction`                                                                                                                                                                                                                                           |
| `com.casimirbot.minecraft.player.hotbar.select`         | `slot`                                                                                                                                                                                                                                                                    |
| `com.casimirbot.minecraft.player.equipment.equip`       | `item_id`, `destination`                                                                                                                                                                                                                                                  |
| `com.casimirbot.minecraft.player.follow`                | `subject_ref`, `distance`, `max_duration_ms`, `stop_below_health`                                                                                                                                                                                                         |
| `com.casimirbot.minecraft.player.collect`               | `item_or_block_id`, `count`, `search_radius`                                                                                                                                                                                                                               |
| `com.casimirbot.minecraft.player.mine`                  | `block_id`, `count`, `search_radius`                                                                                                                                                                                                                                       |
...
| `com.casimirbot.minecraft.player.emergency_stop`        | `workflow_ref`                                                                                                                                                                                                                                                            |
| `com.casimirbot.minecraft.situation_digest.read`        | optional `environment_label`, `freshness_requirement_ms`, `producer_plane`                                                                                                                                                                                                |
