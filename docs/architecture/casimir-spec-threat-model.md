...
Status: implementation candidate. Claim IR, deterministic canonical source
packet parsing, snapshot-bound semantic-admission receipts, provider-neutral
formal and numerical contracts, pinned Lean replay, developer-only agent-tool
gates, pinned Lanyon admission, and one bounded independent numerical fixture
exist. The tracked benchmark policy and rubric remain explicitly unfrozen;
restricted benchmark custody, real external timestamp receipts, broad
operating-system-hermetic execution, and the sealed benchmark remain
unimplemented.
...
| Forged identity or graph relation                                      | Matching hashes and type/unit/frame signatures are structural prerequisites, never authority; semantic-admission receipts verify declared identities and registered bridges against caller-supplied server-owned snapshots                    |
| Unit, dimension, type, frame, or domain confusion                      | Typed bindings and validity domains; source IR cannot self-declare executable; semantic admission rejects mismatched signatures, core-operator arity, dimensionally inconsistent core equations, and incompatible bound-frame combinations      |
...
| Producer output promoted as proof, code correctness, or physical truth | Provider-neutral generation receipts remain non-terminal and non-authoritative; formal replay and independent numerical comparison use separate hash-bound certificate families. See `casimir-artifact-generation-and-numerical-evidence.md`. |
