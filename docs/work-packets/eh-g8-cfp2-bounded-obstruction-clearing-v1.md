...
Workstream: CFP-2.CAPABILITY candidate implementation contract
Capability or component: Exact reachable target removal and truthful effect evidence
Lifecycle stage: tool admission; evidence normalization; evidence re-entry
...
# CFP-2.CAPABILITY — Exact reachable target removal and truthful effect evidence
...
Stage and task ID: CFP-2.CAPABILITY.
Change classification: tool admission; evidence normalization; evidence re-entry.
...
Owner and reviewer: Minecraft action/adapter implementer; independent effect and evidence reviewer.

Allowed files, with exact paths and one writer frozen at dispatch: `shared/helix-minecraft-player-capabilities.ts`; `server/services/environment-connectors/actions/{authority-store,action-broker}.ts` and selected action-admission tests; Minecraft player-agent native workflow engine and mining geometry/affordance tests; applicable connector-core bounds and exact adapter documentation after reading its instructions. CFP-2.PUBLIC owns the immutable personal/hosted context kind and room/source storage; this packet consumes that reviewed interface and rechecks kind, owner, source, subject and current lease at enqueue and native effect admission. A personal context must never acquire a hosted member/effect grant, and a hosted expiry cannot disable independent personal stop/revoke. Coordinate schema/MCP ownership with PUBLIC; neither packet is accepted until the joined positive and bypass cases pass.

Implement and qualify the two-target stationary/no-approach envelope in the controlling acceptance proposal. Enforce exact cumulative target/effect limits at trusted admission and native execution, not only count=1 request syntax. Retain fresh re-entry before the second Codex-selected action. Prevent external target removal from becoming agent-attributed success. No collect workflow, automatic approach, wider NAV or ET compiler is admitted. A missing required native boundary is implementation work, not an assumed source guarantee.
...
BND-01 through BND-08, BND-12; exact normal/adverse trial denominator, deadline, freshness and stop criteria in CFP1 acceptance. Retain original adapter/temporal dependencies actually used.
...
Unapproved third target, autonomous movement, unbounded task extension, stale or ambiguous attribution, duplicate effect, weakened local stop or assumed NAV acceptance fails the packet. Restore no world fixture during measured work.
