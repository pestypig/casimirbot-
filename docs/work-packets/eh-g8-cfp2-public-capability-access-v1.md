...
Candidate owned files, to freeze as an exact non-overlapping manifest at dispatch: `shared/helix-account-session.ts`, `shared/helix-shared-realtime-room.ts` and context-kind contracts; a new reviewed `server/db/migrations/*` context-kind migration; `server/services/helix-ask/realtime-room/room-store/{rooms,invites,repository,types}.ts` and `server/services/helix-ask/realtime-room/source-link-store.ts`; `server/services/shared-live-room-control/service.ts`; `server/routes/agi.realtime-room/http-context.ts` and exact lifecycle/participant/environment/pairing routes in the CFP-1 route addendum; `server/routes/helix-shared-live-rooms.ts`; selected MCP/workstation-gateway handlers and targeted tests. The CFP-2.CAPABILITY owner, not this packet's writer, owns `server/services/environment-connectors/actions/{authority-store,action-broker}.ts` and the native connector changes required to recheck personal kind/source/subject at effect time. Freeze the shared context-kind interface and sequential file ownership with that owner before dispatch; neither packet is accepted without both room and effect checks.

Implement the parent-defined free-personal authorization predicates using existing account, target and consent policy. Consume the [CFP-1 personal-context architecture decision](../evidence/eh-g8-codex-first-paid-product-delivery-v1/cfp-1/2026-09-19-personal-context-architecture-decision-31.md): a service-created immutable `personal_environment` kind backed by the existing room/source identity chain, with no customer-facing shared-room setup. Freeze its schema and stable owner+trusted installed node/connector installation or device key, then create and bind the source to that context with exact owner/node checks; source ID is not available when the context is created. Freeze a distinct hosted-room kind before writing routes. Prove type guards at create/recover, list/inspect, invite issue/redeem, member/source share, floor/media, run/chat binding, every hosted operation, enqueue and native effect admission. A personal ID or paid term cannot convert the kind or admit a second member. If these guards cannot be complete, return to CFP-1 for the distinct-binding decision; do not use a hidden ordinary one-member shared room. Make own consent/pairing/player binding, selected narrow capability reads/actions and native lifecycle reachable to ordinary users. Reuse one trusted decision at browser/API/MCP/service boundaries. Keep hosted invitations, other members, admin commands, unrestricted room controls and unrelated actions denied by the personal path. Preserve developer superset and no-session user semantics. Never expose raw connector credential routes through the wizard.
...
Any generic purchaser room unlock, role promotion, forged evaluation grant, cross-owner access, personal-context invite/join/source-share or hosted conversion, secret exposure or caller-selected adapter is a failure. Coordinate shared MCP/room files with ONBOARD/CAPABILITY.
