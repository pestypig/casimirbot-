...
/*
 * This file intentionally does not vendor upstream code. The build must add
 * the admitted pinned Lanyon implementations directory to the include path.
 */
...
static int solve_level(FILE *stream, uint32_t cells) {
  ...
  while (time < CASIMIR_FINAL_TIME) {
    double dt_advection = 0.4 * dx / fabs(CASIMIR_A);
    double dt_diffusion = 0.2 * dx * dx / CASIMIR_D;
    ...
    for (uint32_t index = 0; index < cells; ++index) {
      ...
      advection_diffusion_full_1d_centered_diffusion_interface_gradient gradient;
