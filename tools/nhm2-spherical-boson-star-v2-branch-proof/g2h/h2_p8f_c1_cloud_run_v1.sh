...
ROOT=/home/pestypig/nhm2-h2-p8f-c1-source-v1
EVIDENCE=/home/pestypig/nhm2-h2-p8f-c1-evidence-v1
EXPORT=/home/pestypig/nhm2-h2-p8f-c1-evidence-export-v1.tgz
IMAGE=nhm2-g2h-e-s5-c08-h2-p8f-c1-cloud-representative:build-v1
CONTAINER=nhm2-h2-p8f-c1-cloud-representative-20260831
EXECUTABLE=/usr/local/bin/mini-boson-star-primary-c08-h2-p8f-c1-cloud-representative-v1
...
fail() {
  printf 'P8F_C1_CONTROLLER_FAIL phase=%s\n' "$1"
  shutdown -h now || true
  exit 1
}
...
docker load -i "$BASE_ARCHIVE" >/tmp/p8f-c1-docker-load.txt 2>&1 || fail base_image_load
docker build --network=none --pull=false \
  -f "$ROOT/tools/nhm2-spherical-boson-star-v2-branch-proof/g2h/Dockerfile.primary.mini-boson-c08-h2-p8f-c1-cloud-representative.v1" \
  -t "$IMAGE" "$ROOT" >/tmp/p8f-c1-docker-build.txt 2>&1 || fail offline_build
...
cp /tmp/p8f-c1-docker-load.txt "$EVIDENCE/docker-load.txt"
cp /tmp/p8f-c1-docker-build.txt "$EVIDENCE/docker-build.txt"
...
printf 'P8F_C1_EVIDENCE bytes=%s sha256=%s\n' "$EXPORT_BYTES" "$EXPORT_SHA"
printf 'P8F_C1_EVIDENCE_BASE64_BEGIN\n'
base64 -w0 "$EXPORT"
printf '\nP8F_C1_EVIDENCE_BASE64_END\n'
sync
shutdown -h now
