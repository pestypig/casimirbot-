...
export const productionPaths = Object.freeze({
  shortRoot: 'C:/NHM2-R42',
  evidenceRoot: join(repo, 'artifacts/nhm2/g2h-e-s5/candidate-neutral/h2-p8p-r42-retrieval-v1-20260905'),
  volumeRoot: 'C:/',
...
export function validateInstances(original, helper) {
  if (String(original.id) !== resource.originalId || original.name !== resource.original || original.status !== 'TERMINATED') throw new Error('original_identity');
  if (String(helper.id) !== resource.helperId || helper.name !== resource.helper || helper.status !== 'TERMINATED') throw new Error('helper_identity');
  if (!helper.machineType?.endsWith('/machineTypes/e2-small') || helper.disks?.length !== 2) throw new Error('helper_configuration');
  const clones = helper.disks.filter(d => d.deviceName === resource.clone);
  if (clones.length !== 1 || clones[0].mode !== 'READ_ONLY' || !clones[0].source?.endsWith(`/disks/${resource.clone}`)) throw new Error('clone_configuration');
  const boot = helper.disks.filter(d => d.boot === true);
  if (boot.length !== 1 || Number(boot[0].diskSizeGb) !== 10 || boot[0].mode !== 'READ_WRITE') throw new Error('boot_configuration');
}
