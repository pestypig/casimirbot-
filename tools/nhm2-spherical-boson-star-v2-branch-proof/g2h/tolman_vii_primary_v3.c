...
static fixture_result run_fixture_v3(const char *name) {
    if (strcmp(name, "exact_rational_positive") == 0) {
        ...
        const int exact_identity =
            fmpz_equal_si(fmpq_numref(exact), 1)
            && fmpz_equal_si(fmpq_denref(exact), 5);
        arb_set_fmpq(enclosure, exact, PRECISION_BITS);
        const int accepted = exact_identity
            && fmpq_sgn(exact) > 0
...
int main(int argc, char **argv) {
    if (argc == 3 && strcmp(argv[1], "--fixture") == 0) {
        fixture_result result = run_fixture_v3(argv[2]);
        emit_fixture(&result);
        flint_cleanup();
