...
try {
    ...
    if (Test-Path -LiteralPath $Evidence) { throw 'R29 evidence root already exists' }
    New-Item -ItemType Directory -Path $Evidence | Out-Null
    $KnownHosts = 'C:\Users\dan\AppData\Local\NHM2\p8p-r29-known-hosts'
    ...
    Save-Text -Path (Join-Path $Evidence 'preexecution.pass.txt') -Text 'R29_PREEXECUTION_PASS'
    ...
    $remoteArchive = '/home/dan/h2-p8p-r29-upload-v1.tar'
    ...
    $knownHostsSha = (Get-FileHash -Algorithm SHA256 -LiteralPath $KnownHosts).Hash.ToLowerInvariant()
    Save-Text -Path (Join-Path $Evidence 'known-hosts.sha256.txt') -Text $knownHostsSha
    $knownHostsCopy = Join-Path $Evidence 'known_hosts.r29.copy'
    Copy-Item -LiteralPath $KnownHosts -Destination $knownHostsCopy
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $knownHostsCopy).Hash.ToLowerInvariant() -ne $knownHostsSha) { throw 'known_hosts evidence copy hash mismatch' }

    $remoteSetup = @'
set -Eeuo pipefail
ARCHIVE=/home/dan/h2-p8p-r29-upload-v1.tar
STAGE=/home/dan/nhm2-h2-p8p-r29-ingress-v1
[[ "$(id -un)" == dan ]]
...
sudo tee /etc/systemd/system/nhm2-h2-p8p-r29.service >/dev/null <<'UNIT'
[Unit]
Description=NHM2 candidate-neutral H2 P8P R29 turnaround calibration
...
sudo systemctl start --no-block nhm2-h2-p8p-r29.service
sleep 5
sudo systemctl is-active --quiet nhm2-h2-p8p-r29.service
printf 'R29_REMOTE_CONTROLLER_ACTIVE\n'
'@
    ...
    if ($sshText -notmatch 'R29_REMOTE_CONTROLLER_ACTIVE') { throw 'remote controller active marker absent' }
