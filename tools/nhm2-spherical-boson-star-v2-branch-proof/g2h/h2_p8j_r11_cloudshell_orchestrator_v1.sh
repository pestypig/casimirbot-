...
REGION=us-east1
VM=nhm2-h2-p8j-r11-c2d-32-20260901
...
EVIDENCE=/home/pestypig/nhm2-h2-p8j-r11-cloudshell-evidence-v1
...
[[ ! -e "$EVIDENCE" ]] || { printf 'R11_FAIL phase=evidence_root_absence\n'; exit 1; }
mkdir "$EVIDENCE"
...
assert len(rows) == 1
row = rows[0]
assert row["name"] == "nhm2-h2-p8j-r11-c2d-32-20260901"
...
PY
printf 'R11_ALLOCATION_PASS zone=%s disk=%s\n' "$ZONE" "$DISK"
...
sudo tee /etc/systemd/system/nhm2-h2-p8j-r11.service >/dev/null <<UNIT
[Unit]
Description=NHM2 candidate-neutral H2 P8J R11 representative attribution
...
sudo systemctl start --no-block nhm2-h2-p8j-r11.service
sleep 5
sudo systemctl is-active --quiet nhm2-h2-p8j-r11.service
printf 'R11_REMOTE_CONTROLLER_ACTIVE\n'
REMOTE
)"
...
tar --sort=name --mtime='UTC 2026-09-01' --owner=0 --group=0 --numeric-owner \
  -czf /home/pestypig/nhm2-h2-p8j-r11-cloudshell-evidence-export-v1.tgz \
  -C /home/pestypig "$(basename "$EVIDENCE")"
printf 'R11_CONTROLLER_LAUNCHED vm=%s zone=%s\n' "$VM" "$ZONE"
trap - EXIT
