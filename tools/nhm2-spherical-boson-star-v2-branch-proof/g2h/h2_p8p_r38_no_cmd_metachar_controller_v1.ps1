...
$Evidence = Join-Path $Root 'artifacts\nhm2\g2h-e-s5\candidate-neutral\h2-p8p-r38-no-cmd-metachar-cloud-fixture-v1-20260904'
...
$guard = 'set -eu; fixture=/home/pestypig/h2_p8p_r31_local_image_binding_fixture_v1.sh; wrapper=/home/pestypig/h2_p8p_r32_fresh_vm_binding_guest_v1.sh; archive=/home/pestypig/h2-p8p-r16-regional-bulk-upload-v1.tar; test -f "$fixture"; test ! -L "$fixture"; test "$(stat -c %s "$fixture")" = 4024; test "$(sha256sum "$fixture")" = "97c0209284fa67ee33e259a75abfef7947f1f8ce8971af3e7f39a3421ce07c79  /home/pestypig/h2_p8p_r31_local_image_binding_fixture_v1.sh"; test -f "$wrapper"; test ! -L "$wrapper"; test "$(stat -c %s "$wrapper")" = 3129; test "$(sha256sum "$wrapper")" = "f66d2f72649c36f88c3e03134150967aadfba639f59781facfaa3ed6ccde9a19  /home/pestypig/h2_p8p_r32_fresh_vm_binding_guest_v1.sh"; test ! -e "$archive"; echo R38_REMOTE_GUARD_PASS'
$handoff = 'set -eu; archive=/home/pestypig/h2-p8p-r16-regional-bulk-upload-v1.tar; test -f "$archive"; test ! -L "$archive"; test "$(stat -c %s "$archive")" = 236640768; test "$(sha256sum "$archive")" = "3c697fa3e238398d3b4c30a6c379fea8d6d545b2228d0c699dd66594a23670b5  /home/pestypig/h2-p8p-r16-regional-bulk-upload-v1.tar"; exec bash /home/pestypig/h2_p8p_r32_fresh_vm_binding_guest_v1.sh'
foreach ($command in @($guard, $handoff)) {
    if ($command.Contains("`r") -or $command.Contains("`n") -or $command.Contains('|') -or $command.Contains('&') -or $command.Contains('<') -or $command.Contains('>')) { throw 'remote command contains forbidden cmd metacharacter' }
}

try {
    if (Test-Path -LiteralPath $Evidence) { throw 'R38 evidence root already exists' }
    ...
    if ($guardResult.ExitCode -ne 0 -or $guardResult.Text -notmatch 'R38_REMOTE_GUARD_PASS') { throw 'read-only remote guard failed' }
