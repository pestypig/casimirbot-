...
try {
    ...
    if (Test-Path -LiteralPath $Evidence) { throw 'R27 evidence root already exists' }
    New-Item -ItemType Directory -Path $Evidence | Out-Null
    $KnownHosts = Join-Path $Evidence 'known_hosts.r27'
    ...
    Save-Text -Path (Join-Path $Evidence 'preexecution.pass.txt') -Text 'R27_PREEXECUTION_PASS'
    ...
    $sshCommon = @('-i',$PrivateKey,'-o','BatchMode=yes','-o','IdentitiesOnly=yes','-o',"UserKnownHostsFile=$KnownHosts",'-o','ConnectTimeout=30')
    $remoteArchive = '/home/dan/h2-p8p-r27-upload-v1.tar'
    $scpArgs = @('-i',$PrivateKey,'-o','BatchMode=yes','-o','IdentitiesOnly=yes','-o',"UserKnownHostsFile=$KnownHosts",'-o','StrictHostKeyChecking=accept-new','-o','ConnectTimeout=30',$Archive,"dan@${ip}:$remoteArchive")
    ...
    $remoteSetup = @'
set -Eeuo pipefail
ARCHIVE=/home/dan/h2-p8p-r27-upload-v1.tar
STAGE=/home/dan/nhm2-h2-p8p-r27-ingress-v1
[[ "$(id -un)" == dan ]]
...
sudo tee /etc/systemd/system/nhm2-h2-p8p-r27.service >/dev/null <<'UNIT'
[Unit]
Description=NHM2 candidate-neutral H2 P8P R27 turnaround calibration
...
sudo systemctl start --no-block nhm2-h2-p8p-r27.service
sleep 5
sudo systemctl is-active --quiet nhm2-h2-p8p-r27.service
printf 'R27_REMOTE_CONTROLLER_ACTIVE\n'
'@
    ...
    if ($sshText -notmatch 'R27_REMOTE_CONTROLLER_ACTIVE') { throw 'remote controller active marker absent' }
