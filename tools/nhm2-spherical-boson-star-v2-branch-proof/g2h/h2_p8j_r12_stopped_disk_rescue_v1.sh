...
DEV=/dev/disk/by-id/google-nhm2-h2-p8j-r11-evidence-clone
MNT=/mnt/nhm2-p8j-r11-rescue
CAP=/home/pestypig/nhm2-h2-p8j-r11-rescue-capture-v1
OUT=/home/pestypig/nhm2-h2-p8j-r11-stopped-disk-evidence-v1.tgz
...
mkdir "$CAP"
{
  echo SCHEMA=nhm2.h2_p8j_r11.stopped_disk_rescue.v1
  echo DEVICE="$DEV"
...
if [[ -d "$MNT/var/log/journal" ]]; then
  journalctl --directory="$MNT/var/log/journal" \
    -u nhm2-h2-p8j-r11.service --no-pager \
...
umount "$MNT"
trap - EXIT
printf 'P8J_R11_RESCUE_READY bytes=%s sha256=%s\n' "$BYTES" "$SHA"
