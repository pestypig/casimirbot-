...
DEV=/dev/disk/by-id/google-nhm2-h2-p8f-c2-r1-evidence-clone
MNT=/mnt/nhm2-p8f-c2-r1-rescue
CAP=/home/pestypig/nhm2-h2-p8f-c2-r1-rescue-capture-v1
OUT=/home/pestypig/nhm2-h2-p8f-c2-r1-stopped-disk-evidence-v1.tgz
...
mkdir "$CAP"
{
  echo SCHEMA=nhm2.h2_p8f_c2_r1.stopped_disk_rescue.v1
  echo DEVICE="$DEV"
...
for REL in \
  home/pestypig/nhm2-h2-p8f-c2-r1-evidence-v1 \
  home/pestypig/nhm2-h2-p8f-c2-r1-evidence-export-v1.tgz \
  tmp/p8f-c2-r1-docker-load.txt \
  tmp/p8f-c2-r1-docker-build.txt; do
  if [[ -e "$MNT/$REL" ]]; then
    cp -a -- "$MNT/$REL" "$CAP/"
  else
...
CTRL="$MNT/home/pestypig/nhm2-h2-p8f-c2-r1-source-v1/tools/nhm2-spherical-boson-star-v2-branch-proof/g2h/h2_p8f_c2_r1_cloud_run_v1.sh"
...
if [[ -d "$MNT/var/log/journal" ]]; then
  journalctl --directory="$MNT/var/log/journal" \
    -u nhm2-h2-p8f-c2-r1-controller --no-pager \
...
find "$MNT/home/pestypig" "$MNT/tmp" -maxdepth 2 -xdev \
  -printf '%y %s %T@ %p\n' 2>/dev/null \
  | grep -E 'p8f-c2-r1|nhm2-h2-p8f-c2-r1' \
...
umount "$MNT"
trap - EXIT
printf 'P8F_C2_R1_RESCUE_READY bytes=%s sha256=%s\n' "$BYTES" "$SHA"
