...
base_archive=${1:?usage: h2_p8c_diagnostic_offline_build_guard_v1.sh BASE_ARCHIVE [IMAGE_TAG]}
image_tag=${2:-nhm2-g2h-e-s5-h2-p8c-diagnostic:v1}
...
binary=/usr/local/bin/mini-boson-star-primary-c08-h2-p8c-diagnostic-run-v1
required_binary=7e7d78393f933ac103208476f6e8c5beefb5de66b58d93a6b2a080bdf80deb25

if docker image inspect "$builder_tag" >/dev/null 2>&1 \
   || docker image inspect "$runtime_tag" >/dev/null 2>&1; then
...
DOCKER_BUILDKIT=0 docker build --pull=false --network=none \
  -f tools/nhm2-spherical-boson-star-v2-branch-proof/g2h/Dockerfile.primary.mini-boson-c08-h2-p8c-diagnostic-run.v1 \
  -t "$image_tag" .
