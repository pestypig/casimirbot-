...
export async function runRetrieval(testOnly = {}) {
  const store = testOnly.store ?? localStore('C:/NHM2-CV2-R2',resolve(repo,'artifacts/nhm2/g2h-e-s5/candidate-neutral/charter-v2-retrieval-2'),'C:/');
  ...
  async function command(exe,args,timeoutMs=30000,signal,onStdout) {
    ...
    try {const r=await run(exe,args,{timeoutMs,env,signal,onStdout});if(args.includes('project-info')) { const p=JSON.parse(r.stdout); r.stdout=JSON.stringify({name:p.name,commonInstanceMetadata:p.commonInstanceMetadata?{fingerprint:p.commonInstanceMetadata.fingerprint,items:p.commonInstanceMetadata.items?.map(x=>({key:x.key}))}:undefined}); } record('return',r);return r.stdout.trim();}
    catch(e){if(e.terminationConfirmed===false)unresolvedProcesses.push({pid:e.pid,message:e.message});record('failure',{message:e.message,pid:e.pid,terminationConfirmed:e.terminationConfirmed,stdout:args.includes('project-info')?'[metadata omitted]':e.stdout,stderr:e.stderr});throw e;}
  }
  ...
  try {
    ...
    authenticateFile(resolve(repo,'docs/research/nhm2-p8p-charter-v2-ledger.md'),'1729705a8248c75ca02015ca3251a87458ac74e6a2a79cc63ab030aacb6d58de');
    ...
    // Read the actual object; command() suppresses metadata values locally.
    const p=JSON.parse(await gc(['compute','project-info','describe',project,'--format=json']));
...
if(process.argv[1] && import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
  ...
  const manifest=JSON.parse(readFileSync(resolve(import.meta.dirname,'h2_p8p_charter_v2_retrieval_r2_manifest.json'),'utf8'));
