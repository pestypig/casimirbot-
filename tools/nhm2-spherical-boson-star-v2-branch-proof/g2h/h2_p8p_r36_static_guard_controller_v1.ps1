...
$Evidence = Join-Path $Root 'artifacts\nhm2\g2h-e-s5\candidate-neutral\h2-p8p-r36-static-guard-cloud-fixture-v1-20260904'
...
test "$(stat -c %s "$fixture")" = 4024
printf '%s  %s\n' 97c0209284fa67ee33e259a75abfef7947f1f8ce8971af3e7f39a3421ce07c79 "$fixture" | sha256sum -c -
test -f "$wrapper" && test ! -L "$wrapper"
test "$(stat -c %s "$wrapper")" = 3129
printf '%s  %s\n' f66d2f72649c36f88c3e03134150967aadfba639f59781facfaa3ed6ccde9a19 "$wrapper" | sha256sum -c -
test ! -e "$archive"
printf '%s\n' R36_REMOTE_GUARD_PASS
'@
...
test "$(stat -c %s "$archive")" = 236640768
printf '%s  %s\n' 3c697fa3e238398d3b4c30a6c379fea8d6d545b2228d0c699dd66594a23670b5 "$archive" | sha256sum -c -
exec bash /home/pestypig/h2_p8p_r32_fresh_vm_binding_guest_v1.sh
'@

try {
    if (Test-Path -LiteralPath $Evidence) { throw 'R36 evidence root already exists' }
    ...
    if ($guardResult.ExitCode -ne 0 -or $guardResult.Text -notmatch 'R36_REMOTE_GUARD_PASS') { throw 'read-only remote guard failed' }
