...
void fill(Storage &storage, std::size_t jet, unsigned multiplier) {
    ...
    for (std::size_t degree = 0U; degree < degree_count; ++degree) {
        ...
        arb_add_error_2exp_si(value, -220L - static_cast<slong>(degree % 5U));
    }
}
...
bool positive(arb_srcptr value) {
    return arb_is_finite(value) && arb_is_positive(value);
}
...
int main() {
    ...
    fill(f0, 0U, 1U); fill(f1, 0U, 2U); fill(f2, 0U, 3U);
    fill(g0, 9U, 2U); fill(g1, 9U, 3U); fill(g2, 9U, 4U);
    ...
    bivariate::CoefficientAttribution attribution, repeated_attribution;
    const bool ordinary_ok = prepared_ok && bivariate::evaluate_prepared(
        input, prepared, &ordinary, &ordinary_result);
    const bool observed_ok = prepared_ok
        && bivariate::evaluate_prepared_attributed(
            input, prepared, 3U, &observed, &observed_result, &attribution);
    const bool repeated_ok = prepared_ok
        && bivariate::evaluate_prepared_attributed(
    ...
    checks.push_back(attribution.evaluated
                     && attribution.target_degree == 3U
                     && attribution.observation_only
                     && attribution.final_reconstruction_equal);
    checks.push_back(arb_equal(attribution.reconstructed_coefficient,
                               observed.coefficient(3U)));
    checks.push_back(positive(attribution.direct_integrated_radius_sum)
                     && positive(attribution.boundary_radius_sum));
    checks.push_back(positive(attribution.f_source_hull_radius_bound)
                     && positive(attribution.gprime_source_hull_radius_bound));
    checks.push_back(attribution.integrated_terms_observed > 0U
                     && attribution.boundary_terms_observed > 0U);
    checks.push_back(arb_equal(attribution.integrated_centered_component,
                               repeated_attribution.integrated_centered_component)
                     && arb_equal(attribution.boundary_centered_component,
                                  repeated_attribution.boundary_centered_component)
                     && arb_equal(attribution.direct_integrated_radius_sum,
                                  repeated_attribution.direct_integrated_radius_sum)
                     && arb_equal(attribution.boundary_radius_sum,
                                  repeated_attribution.boundary_radius_sum));
    ...
    std::cout
        << "{\"schema\":\"nhm2.g2h_e_s5.c08_h2_p8h_slot3_attribution_fixture.v1\","
        << "\"status\":\"" << (passed == checks.size() ? "PASS" : "FAIL")
        << "\",\"checks_passed\":" << passed
        << ",\"checks_total\":" << checks.size()
        << ",\"target_degree\":3,\"f_jet\":0,\"gprime_jet\":9,"
        << "\"integrated_terms_observed\":"
        ...
        << (attribution.final_reconstruction_equal ? "true" : "false")
