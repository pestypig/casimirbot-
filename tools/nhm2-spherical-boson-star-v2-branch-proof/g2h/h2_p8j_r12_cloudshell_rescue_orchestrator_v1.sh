...
ZONE=us-east1-c
ORIGINAL=nhm2-h2-p8j-r11-c2d-32-20260901
SNAP=nhm2-h2-p8j-r11-evidence-snapshot-20260901
CLONE=nhm2-h2-p8j-r11-evidence-clone-20260901
HELPER=nhm2-h2-p8j-r12-rescue-e2-small-20260901
DEVICE=nhm2-h2-p8j-r11-evidence-clone
EVIDENCE=/home/pestypig/nhm2-h2-p8j-r12-cloudshell-evidence-v1
RESCUE=/home/pestypig/h2_p8j_r12_stopped_disk_rescue_v1.sh
REMOTE_ARCHIVE=/home/pestypig/nhm2-h2-p8j-r11-stopped-disk-evidence-v1.tgz
LOCAL_ARCHIVE=/home/pestypig/nhm2-h2-p8j-r11-stopped-disk-evidence-v1.tgz
OUTER_ARCHIVE=/home/pestypig/nhm2-h2-p8j-r12-cloudshell-evidence-export-v1.tgz
RESCUE_BYTES=2682
RESCUE_SHA=4e60f6c55bb68fc8da96036fe77aafeb9fcd9004443fe7f805834f8f2c38444c
...
cleanup() {
  ...
  if [[ "$code" -ne 0 ]]; then printf 'R12_FAIL exit=%s\n' "$code"; fi
}
trap cleanup EXIT
...
assert x['name']=='nhm2-h2-p8j-r11-c2d-32-20260901' and x['status']=='TERMINATED'
...
printf 'R12_PREEXECUTION_PASS source_disk=%s\n' "$SOURCE_DISK"
...
m=re.search(r'P8J_R11_RESCUE_READY bytes=(\d+) sha256=([0-9a-f]{64})', t)
assert m
...
printf 'R12_RECOVERY_PASS bytes=%s sha256=%s outer_bytes=%s outer_sha256=%s\n' "$BYTES" "$SHA" "$(stat -c %s "$OUTER_ARCHIVE")" "$(sha256sum "$OUTER_ARCHIVE" | awk '{print $1}')"
trap - EXIT
