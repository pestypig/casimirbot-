...
There was also a documentation and theory-language pass. Commit `b09f171c1` expanded `shared/theory/nhm2-full-solve-theory-badges.ts`, updated `shared/theory/warp-gr-nhm2-map.ts`, and refreshed `docs/research/nhm2-current-status-whitepaper-2026-05-02.md`. The effect is to make the lane's public grammar match the new contracts: regional atlas, source authority, wall `T00` trace, coupled closure candidate, regional pass-path harness, and QEI dossier all appear as named diagnostic surfaces rather than loose ideas. That is not a new solve. It is a cleaner promise about what the solve must still prove.
