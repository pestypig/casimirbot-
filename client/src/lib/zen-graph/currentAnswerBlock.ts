export type ZenGraphCurrentAnswerTraceStep = {
  step: string;
  nodeIds: string[];
  badgeIds: string[];
  reason: string;
...
export type ZenGraphCurrentAnswerBlock = {
  ...
  pathToRoot: string[];
  trace: ZenGraphCurrentAnswerTraceStep[];
...
const collectTrace = (binding: Record<string, unknown> | null, reflection: Record<string, unknown> | null): ZenGraphCurrentAnswerTraceStep[] => {
  const bindingTrace = Array.isArray(binding?.trace) ? binding?.trace : [];
  const trace = bindingTrace
    .map((entry): ZenGraphCurrentAnswerTraceStep | null => {
      const record = asRecord(entry);
    ...
    .filter((entry): entry is ZenGraphCurrentAnswerTraceStep => Boolean(entry));
  ...
  return exact
    ...
    .map((entry): ZenGraphCurrentAnswerTraceStep | null => {
      ...
      return {
        ...
        badgeIds: [],
        reason: readString(match?.reason) ?? readString(match?.label) ?? "ZenGraph activated this lens from the prompt.",
      };
    })
    .filter((entry): entry is ZenGraphCurrentAnswerTraceStep => Boolean(entry));
};

export function buildZenGraphCurrentAnswerBlockFromDebugExport(
  value: unknown,
  options: { nowMs?: number } = {},
): ZenGraphCurrentAnswerBlock | null {
  const debugExport = parseDebugExport(value);
  if (!debugExport) return null;
  const ledger = ledgerFromDebugExport(debugExport);
  const toolPayload =
    payloadForKind(ledger, "helix_zen_graph_reflection_tool_result") ??
    asRecord(debugExport.zen_graph_reflection_tool_result) ??
    null;
  const hasZenGraphTool = Boolean(toolPayload);
  ...
  const route =
    ...
    "unknown";
  if (!hasZenGraphTool && route !== "zen_graph_reflection") return null;
  ...
  return {
    schema: "zen_graph_current_answer_block/v1",
    blockId: `zen-current-answer:${readString(debugExport.active_turn_id) ?? "unknown-turn"}`,
    ...
    terminalArtifactKind:
      ...
      "unknown",
    route,
    toolReceiptRef: artifactRefForKind(ledger, "helix_zen_graph_reflection_tool_result"),
