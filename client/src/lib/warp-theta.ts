...
// ===== Canonical physics (mirror of engine) — for UI display/tests only =====
export function thetaCanonical(params: {
  gammaGeo: number; qSpoilingFactor: number; gammaVanDenBroeck_mass: number;
  dutyLocal: number; sectorsConcurrent: number; sectorsTotal: number;
  viewAveraged?: boolean; mode?: 'standby'|'hover'|'cruise';
}): number {
  const {
