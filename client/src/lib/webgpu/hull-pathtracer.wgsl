...
struct ViewParams {
    ...
    // WGSL alignment: assume vec4 padding
    eye_pos: float4,
    volume_scale: float4,
    light_dir: float4,
    frame_id: u32,
...
fn sample_density(val: f32) -> f32 {
    return clamp(abs(val) * params.density_scale, 0.0, 1.0);
}
...
fn sample_woodcock(
    ...
    loop {
        ...
        *t -= log(1.0 - samples.x) / params.sigma_t_scale;
        ...
        var density = sample_density(val);
        if (density > samples.y) {
            result.scattering_event = true;
            let color_u = clamp(val * 0.5 + 0.5, 0.0, 1.0);
            result.color = textureSampleLevel(colormap, tex_sampler, float2(color_u, 0.5), 0.0).rgb;
...
fn delta_tracking_transmittance(
    orig: float3,
    dir: float3,
    interval: float2,
    rng: ptr<function, LCGRand>
    ...
    loop {
        ...
        var p = orig + t * dir;
        var val = textureSampleLevel(volume, tex_sampler, p, 0.0).r;
        if (sample_density(val) > samples.y) {
            return 0.0;
        }
    }
    return 1.0;
...
fn ratio_tracking_transmittance(
    orig: float3,
    dir: float3,
    interval: float2,
    rng: ptr<function, LCGRand>
    ...
    loop {
        ...
        var p = orig + t * dir;
        var val = textureSampleLevel(volume, tex_sampler, p, 0.0).r;
        transmittance *= (1.0 - sample_density(val));
    }
...
fn fragment_main(in: VertexOutput) -> @location(0) float4 {
    ...
    // Image assumed <= 1280x720 for RNG determinism
