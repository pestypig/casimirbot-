...
| `ask-doc-viewer-context.ts` | Deterministic docs-viewer path normalization, explicit path extraction from prompt text, and debug path comparison normalization. | Active-doc handoff, docs-viewer store/URL reads, workspace snapshots, request envelope construction, source admission, or route metadata. |
...
| `ask-output-cleanup.ts` | Deterministic answer-boundary, stage-tag, prompt-line, and echoed-question cleanup primitives used by legacy fallback presentation. | Fallback answer selection, route authority, terminal authority, solver output admission, or model response rewriting. |
...
| `ask-value-normalization.ts` | Shared deterministic value normalization helpers for number parsing, numeric clamping, text clipping, safe text coercion, non-empty string trimming, plain-record checks, string-array cleanup, and case-insensitive string dedupe. | Env policy, route/request admission, terminal authority, debug export authority, mutation, or side effects. |
...
| `ask-voice-diagnostics-export.ts` | Deterministic voice diagnostics export projection: segment count summaries, bounded playback receipts, bounded voice calls, bounded timeline rows, and monitor rounding. | Diagnostics snapshot construction/publication, refs/effects, mic/STT runtime, playback runtime, TTS transport, terminal authority, or debug export authority. |
...
| `ask-voice-capture-display.ts` | Deterministic voice-capture signal display predicates: level smoothing, flat-signal/stall checks, loopback label matching, container-header priming decisions, noise profile thresholds, low-quality signal flags, and media error debug labels. | Mic recorder lifecycle, browser capability probing, `MediaRecorder` MIME selection, stream capture, STT dispatch, voice confirmation, barge-in state transitions. |
...
| Pending input and cancellation terminal visibility | `readHelixPendingInputRecord`, `normalizeHelixPendingTransitionMarker`, `readHelixPendingTransitionTrace`, `hasHelixPendingCancellationMarker`, `isHelixCanceledPendingTurn`, `resolveHelixPendingInputRecord`, `resolveHelixVisibleTerminalKind` | Pending request resolution, cancellation markers, and visible terminal-kind selection affect latest-turn lifecycle and terminal projection; behavior-goal only. |
| Workstation command lexicon and panel resolution | `normalizePanelQuery`, `resolvePanelIdFromText`, `resolvePanelIdFromPath`, `parseOpenPanelCommand`, `restateWorkstationSubgoal`, `normalizeWorkstationCommandText`, `resolveCapabilityAliasPanelAction`, `parseWorkstationLexiconAction` | Natural-language workstation command parsing, panel capability matching, and `HelixWorkstationAction` construction; behavior-goal only. |
...
| Voice capture, STT, confirmation, continuation, and auto-dispatch | `resolveTranscriptConfirmPolicy`, `evaluateVoiceAutoDispatchGovernance`, `shouldMergeVoiceContinuationTurn`, `buildVoiceReasoningDispatchPrompt`, `deriveVoiceTimelineSuppressionMeta` | Mic/STT/runtime control, transcript lifecycle, and dispatch policy; do not move as display. Deterministic transcript confidence/threshold helpers live in `ask-voice-transcript-confidence.ts`; deterministic confirmation command parsing lives in `ask-voice-confirmation-command.ts`; deterministic active-turn steering transcript/request/event predicates live in `ask-voice-steering-client.ts`; deterministic barge/preempt predicates live in `ask-voice-barge-policy.ts`, while reservation, queue mutation, and tool recording stay behavior-sensitive. |
...
| Docs-viewer and workspace context snapshots | `normalizeDocViewerPathForAskSnapshot`, `readDocViewerDebugSnapshot`, `resolveAskTurnDocViewerSnapshotPath`, `readDocViewerPathFromDesktopUrlForAskSnapshot`, `rememberDocViewerPathForAskSnapshot`, `resolveDocsViewerAnchorPathForQuestion`, `readWorkstationLayoutDebugSnapshot`, `buildAskTurnWorkspaceContextSnapshot` | Active-doc handoff, URL/store reads, layout state reads, and request-envelope context; deterministic path normalization/extraction lives in `ask-doc-viewer-context.ts`, but HASK-BSQ-001 handoff behavior remains quarantined. |
...
| Backend entrypoint and runtime authority guards | `requiresHelixAskBackendEntrypoint`, `buildHelixAskBackendEntrypointRuntimeFingerprint`, `buildHelixActionEnvelopeRuntimeAuthority`, `attachHelixActionEnvelopeRuntimeAuthorityDebug`, `readAgentLoopAuditRecord`, `extractAskLevelTheoryReflection`, `normalizeHelixRuntimeActionKey`, `readHelixDecisionCapabilityKeys`, `readHelixGatewayCapabilityKeys`, `collectHelixAgentSelectedCapabilities`, `readHelixWorkstationActionRuntimeKeys` | Backend route guardrails, selected-capability admission, runtime/action-envelope authority, theory-reflection handoff, debug authority attachment, and shortcut prevention; behavior-goal only. |
...
| Debug export and clipboard authority | `base64FromText`, `sha256TextHex`, `buildReplyMasterEventClockExport`, `extractHelixRenderedTurnDebugFromButton`, `debugPayloadMatchesRenderedReply`, `debugPayloadMatchesRenderedTurnPayload`, `buildHelixDebugExportEnvelopeFromMasterPayload`, `normalizeReplyMasterDebugPayload`, `buildClientProjectionDebugFields`, `summarizeHelixDebugObservationForCopy`, `summarizeHelixDebugArtifactsForCopy`, `summarizeHelixAgentRuntimeLoopForCopy`, `copyHelixRailCriticalDebugFieldsForUi`, `boundHelixDebugExportTextForUi`, `buildReplyScopedDebugExportFromRenderedReply`, `buildReplyScopedDebugExportFromRenderedButton`, `resolveAuthoritativeDebugExportPayload`, `copyDebugPayloadToClipboard` | Reply-scoped debug binding, rendered-button turn matching, backend debug export materialization, event-clock export, payload hashing/encoding, fallback bounding, and clipboard readback; pure drawer fallback result formatting lives in `../agi/debugExport.ts`. |
