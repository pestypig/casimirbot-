...
| `ask-procedural-display.ts` | Procedural timeline/debug-copy action label formatting, workstation intent-stage detail copy, and workstation lifecycle status/receipt copy. | Timeline rendering, route selection, router fail IDs, calculator fast-path replies, workstation dispatch, tool execution. |
...
| `ask-terminal-projection.ts` | Typed visible final-answer projection helpers, pending-input request reading, live-answer context-pack answer copy, and terminal answer text normalization/invalid-placeholder detection. | Model sampling, terminal completion, server-side authority, JSX timeline rendering, reply lifecycle state. |
| `ask-turn-transcript.ts` | Runtime transcript, runtime Ask live-event projection, structured workstation gateway transcript projection, and causal trace row building. | Tool execution, live source materialization, terminal decisions, JSX timeline rendering. |
