...
import type { GrEvolveBrickDecoded } from "@/lib/gr-evolve-brick";
import type { LapseBrickDecoded } from "@/lib/lapse-brick";
import type {
...
const NATARIO_GEOM_WARP_SCALE = 0;
...
const resolveWarpMode = (pipeline: EnergyPipelineState | null): TimeDilationRenderMode => {
  const raw =
...
const resolveChannelRange = (
  brick: GrEvolveBrickDecoded | LapseBrickDecoded | null | undefined,
  key: string,
...
export const computeTimeDilationRenderPlan = (
  pipelineState: EnergyPipelineState | null,
  grBrick: GrEvolveBrickDecoded | null | undefined,
  lapseBrick: LapseBrickDecoded | null | undefined,
  ...
  const allowGeometryWarp = mode === "alcubierre" || cinematicOverride;
  ...
  const geomWarpScale = enableGeometryWarp ? ALCUBIERRE_GEOM_WARP_SCALE : 0;
