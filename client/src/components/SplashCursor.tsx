...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas) {
      ...
      const hasFloatSupport = formatRGBA && formatRG && formatR && halfFloatTexType;
      const resolvedFormatRGBA = formatRGBA || fallbackFormat;
      const resolvedFormatRG = formatRG || resolvedFormatRGBA;
      const resolvedFormatR = formatR || resolvedFormatRGBA;
      const resolvedTexType = hasFloatSupport ? halfFloatTexType : gl.UNSIGNED_BYTE;
      const resolvedLinearFiltering = hasFloatSupport ? !!supportLinearFiltering : true;

      if (!hasFloatSupport) {
        console.warn('[SplashCursor] Falling back to UNSIGNED_BYTE framebuffers due to missing float texture support.');
      }

      return {
        gl,
        ext: {
          formatRGBA: resolvedFormatRGBA,
          formatRG: resolvedFormatRG,
          formatR: resolvedFormatR,
          halfFloatTexType: resolvedTexType,
          supportLinearFiltering: resolvedLinearFiltering
        }
      };
    }
