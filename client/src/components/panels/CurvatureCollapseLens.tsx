...
import type { CurvatureCollapseGroup, CurvatureCollapseGroupId } from "@shared/theory/curvature-collapse-map";

function bandLabel(band: CurvatureCollapseGroup["band"]) {
  return band.replace(/_/g, " ");
}

function bandClass(band: CurvatureCollapseGroup["band"], selected: boolean) {
  const ring = selected ? " ring-2 ring-cyan-100" : "";
  switch (band) {
    case "curvature":
      return `border-emerald-300 bg-emerald-950/70 text-emerald-50${ring}`;
    case "collapse":
      return `border-teal-300 bg-teal-950/70 text-teal-50${ring}`;
    case "uncertainty":
...
function uniqueBands(groups: CurvatureCollapseGroup[]) {
  return groups.reduce<CurvatureCollapseGroup["band"][]>((acc, group) => {
    if (!acc.includes(group.band)) acc.push(group.band);
    return acc;
  }, []);
}

export default function CurvatureCollapseLens({
  graph,
  groups,
  selectedGroupId,
  ...
  groups: CurvatureCollapseGroup[];
  selectedGroupId: CurvatureCollapseGroupId | null;
  selectedObjectBindingId: string | null;
  translateText?: (text: string) => string;
  onSelectGroup: (group: CurvatureCollapseGroup) => void;
  onSelectObjectBinding: (group: CurvatureCollapseGroup, bindingId: string) => void;
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-950 bg-zinc-950/95 text-zinc-100">
      <div className="border-b border-zinc-800 px-3 py-2">
        <div className="text-xs font-bold uppercase tracking-wide text-emerald-200">{tx("Curvature")}</div>
        <div className="text-sm font-semibold">{tx("Collapse")}</div>
      </div>
            ...
            <div className="space-y-1">
              {groups
                ...
                .map((group) => {
                  ...
                  return (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.curvature-collapse-lens.on-select-group-group" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
                      key={group.id}
                      type="button"
          ...
          <div className="space-y-2">
            ...
            {selectedGroup.objectBindings.length > 0 ? (
              <div className="space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">{tx("Benchmark Binding")}</div>
                  {selectedBinding ? (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.curvature-collapse-lens.on-clear-object-binding" data-helix-interaction-kind="act" data-helix-authority-state="client_local"
                      type="button"
                ...
                {selectedGroup.objectBindings.map((binding) => {
                  ...
                  return (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.curvature-collapse-lens.on-select-object-binding-selected-group-binding-id" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
                      key={binding.id}
                      type="button"
            ...
            <div className="space-y-1">
              {selectedGroup.calculatorPayloadRefs.slice(0, 6).map((ref) => {
                ...
                return (
                  <button data-helix-control-id="workstation.panel.theory-badge-graph.curvature-collapse-lens.on-load-payload-ref-badge-id-ref-payload-id" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
            ...
            {selectedGroup.calculatorPayloadRefs.length === 0 ? (
              <div className="border border-zinc-800 bg-zinc-900 px-2 py-2 text-[11px] text-zinc-500">
                {tx("Runtime/benchmark context. No scalar calculator payload.")}
              </div>
            ) : null}
          </div>
        ) : (
          <div className="text-[11px] text-zinc-500">{tx("Pick a curvature/collapse group to light benchmark badges.")}</div>
        )}
      </div>
    </aside>
  );
