...
function nodeClasses(args: { node: MoralGraphNode; selected: boolean; highlighted: boolean; dimmed: boolean }): string {
  const classes = [
...
function nodeGlyph(node: MoralGraphNode): string {
...
function termProcedureExpression(node: MoralGraphNode, term: FruitionProcedureTermV1): string {
...
function addNode(nodes: Map<string, MoralGraphNode>, node: MoralGraphNode): void {
  if (!nodes.has(node.id)) nodes.set(node.id, node);
}

function addPathNodes(params: {
  nodes: Map<string, MoralGraphNode>;
...
function findProcedureTermForNode(node: MoralGraphNode, fruition: FruitionProcedureExpressionV1): FruitionProcedureTermV1 | undefined {
...
function decorateProcedureNode(node: MoralGraphNode, fruition: FruitionProcedureExpressionV1): MoralGraphNode {
...
function buildGraph(
  ...
  const nodes = new Map<string, MoralGraphNode>();
  ...
  addNode(nodes, {
    ...
    x: reflection.graph.rootId === MORAL_WISDOM_ROOT_ID ? 900 : 90,
    y: 210,
    tags: reflection.graph.rootId === MORAL_WISDOM_ROOT_ID ? ["objective_binding", "preset"] : ["root"],
    summary: reflection.graph.rootId === MORAL_WISDOM_ROOT_ID ? "Objective binding assembled from primitive wisdom badges." : "Root principle",
    proceduralExpression: rootProcedureExpression(reflection.graph.rootId),
  });

  if (reflection.graph.rootId === MORAL_WISDOM_ROOT_ID) {
    MORAL_WISDOM_PRINCIPLES.forEach((principle, index) => {
  ...
  reflection.matches.exact.forEach((match) => addMatch(match, "lens", "cyan", "Exact MoralGraph lens match"));
  reflection.matches.likely.forEach((match) => addMatch(match, "lens", "slate", "Likely MoralGraph lens match"));
  ...
  if (reflection.graph.rootId === MORAL_WISDOM_ROOT_ID && nodes.has("mission-ethos")) {
          ...
          >
            ...
            >
              ...
              <svg className="pointer-events-none absolute inset-0" width={graph.width} height={graph.height}>
                <defs>
                  <marker id="moral-graph-arrow" markerHeight="7" markerWidth="7" orient="auto" refX="6" refY="3.5">
                  ...
                  return (
                    <g key={edge.id}>
                      <path
                        ...
                        markerEnd="url(#moral-graph-arrow)"
                      />
                    </g>
                  );
                })}
