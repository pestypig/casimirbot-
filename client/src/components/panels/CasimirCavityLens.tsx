...
import type { CasimirCavityGroup, CasimirCavityGroupId } from "@shared/theory/casimir-cavity-map";

function bandLabel(band: CasimirCavityGroup["band"]) {
  return band.replace(/_/g, " ");
}

function bandClass(band: CasimirCavityGroup["band"], selected: boolean) {
...
function uniqueBands(groups: CasimirCavityGroup[]) {
  return groups.reduce<CasimirCavityGroup["band"][]>((acc, group) => {
    if (!acc.includes(group.band)) acc.push(group.band);
    return acc;
  }, []);
}

export default function CasimirCavityLens({
  graph,
  groups,
  selectedGroupId,
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-950 bg-zinc-950/95 text-zinc-100">
      <div className="border-b border-zinc-800 px-3 py-2">
        <div className="text-xs font-bold uppercase tracking-wide text-cyan-200">Casimir</div>
            ...
            {selectedGroup.objectBindings.length > 0 ? (
              <div className="space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Cavity Binding</div>
                  {selectedBinding ? (
                    <button
                      type="button"
