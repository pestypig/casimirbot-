...
import ZenGraphNode from "./ZenGraphNode";
...
export function ZenGraphOverlay({ reflection }: { reflection: IdeologyContextReflectionV1 }) {
  ...
  return (
    <section aria-labelledby="zengraph-overlay-title" className="rounded border border-zinc-800 bg-zinc-950/80 p-3">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 id="zengraph-overlay-title" className="text-sm font-semibold text-zinc-100">
            {reflection.overlay?.title ?? "ZenGraph reflection"}
          </h3>
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        <ZenGraphNode title={reflection.graph.rootId} subtitle="Root principle" tone="root" />
        {highlightedNodeIds.map((nodeId) => (
          <ZenGraphNode key={nodeId} title={labelForNodeId(reflection, nodeId)} subtitle={nodeId} tone="lens" />
        ))}
      </div>
    </section>
  );
...
export default ZenGraphOverlay;
