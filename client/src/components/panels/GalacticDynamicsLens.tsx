...
import type { GalacticDynamicsGroup, GalacticDynamicsGroupId } from "@shared/theory/galactic-dynamics-map";

function bandLabel(band: GalacticDynamicsGroup["band"]) {
  return band.replace(/_/g, " ");
}

function bandClass(band: GalacticDynamicsGroup["band"], selected: boolean) {
...
function uniqueBands(groups: GalacticDynamicsGroup[]) {
  return groups.reduce<GalacticDynamicsGroup["band"][]>((acc, group) => {
    if (!acc.includes(group.band)) acc.push(group.band);
    return acc;
  }, []);
}

export default function GalacticDynamicsLens({
  graph,
  groups,
  selectedGroupId,
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-950 bg-zinc-950/95 text-zinc-100">
      <div className="border-b border-zinc-800 px-3 py-2">
        <div className="text-xs font-bold uppercase tracking-wide text-sky-200">{tx("Galactic")}</div>
        <div className="text-sm font-semibold">{tx("Dynamics")}</div>
      </div>
            ...
            <div className="space-y-1">
              {groups
                ...
                .map((group) => {
                  ...
                  return (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.galactic-dynamics-lens.on-select-group-group" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
                      key={group.id}
                      type="button"
          ...
          <div className="space-y-2">
            ...
            {selectedGroup.objectBindings.length > 0 ? (
              <div className="space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">{tx("Dynamics Binding")}</div>
                  {selectedBinding ? (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.galactic-dynamics-lens.on-clear-object-binding" data-helix-interaction-kind="act" data-helix-authority-state="client_local"
                      type="button"
                ...
                {selectedGroup.objectBindings.map((binding) => {
                  ...
                  return (
                    <button data-helix-control-id="workstation.panel.theory-badge-graph.galactic-dynamics-lens.on-select-object-binding-selected-group-binding-id" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
                      key={binding.id}
                      type="button"
            ...
            <div className="space-y-1">
              {selectedGroup.calculatorPayloadRefs.slice(0, 6).map((ref) => {
                ...
                return (
                  <button data-helix-control-id="workstation.panel.theory-badge-graph.galactic-dynamics-lens.on-load-payload-ref-badge-id-ref-payload-id" data-helix-interaction-kind="configure" data-helix-authority-state="client_local"
