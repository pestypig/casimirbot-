...
export function AgentConnectionSetup() {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${AGENT_CONNECTION_READINESS_ENDPOINT}?client_profile=${encodeURIComponent(setup.selectedProfile)}`,
      ...
      } else if (setup.viewedStep === "account" || setup.viewedStep === "authorize") {
  ...
  useEffect(() => {
    if (setup.selectedProfile) void refresh();
    ...
    switch (setup.viewedStep) {
      ...
      case "check":
        return status?.catalog_reenumeration_required
          ? { title: "Refresh the AI app connection", body: "CasimirBot authorization changed after this app last loaded its tools. Reconnect the server or start a fresh chat, then ask: “Connect this chat to CasimirBot and check Agent Connections.”" }
    ...
    <section className="rounded-xl border border-cyan-300/20 bg-slate-950/85 p-4 text-slate-100" aria-labelledby="agent-connection-setup-title">
          ...
          <ol className="list-decimal space-y-2 pl-5 text-xs leading-5 text-slate-300">
            {selected.profile_id === "codex_app" ? (
              <>
                ...
                <li>After sign-in succeeds, restart Codex only when it requests it, then begin a new chat so the current tool catalog is loaded.</li>
              </>
            ) : (
              <>
                ...
                <li>Reconnect the server or start a fresh task so it discovers the current catalog.</li>
              </>
            )}
          </ol>
      ...
      {setup.viewedStep === "check" ? (
        <div className="mt-4 rounded-lg border border-amber-300/20 bg-amber-400/5 p-3 text-xs leading-5 text-amber-50/80">
          The AI app should use CasimirBot's connection-check tool automatically. The check passes only after this profile, current node, client session, and chat thread are authenticated. Opening an app or tunnel alone does not pass it. If sign-in succeeds but the app immediately asks again, stop repeating the loop: the connection needs administrator repair before Retry can pass.
        </div>
      ) : null}
