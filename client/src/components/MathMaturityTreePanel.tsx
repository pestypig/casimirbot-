          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <Badge className={cn("bg-emerald-500/20", STATUS_TEXT.pass)}>
                  PASS {summary?.status.pass ?? 0}
                </Badge>
                <Badge className={cn("bg-amber-500/20", STATUS_TEXT.warn)}>
                  WARN {summary?.status.warn ?? 0}
                </Badge>
                <Badge className={cn("bg-rose-500/20", STATUS_TEXT.fail)}>
                  FAIL {summary?.status.fail ?? 0}
                </Badge>
                <Badge variant="secondary">Unstaged {summary?.unstagedCount ?? 0}</Badge>
              ...
              {summary && (
                <div className="text-xs text-slate-400">
                  S0 {summary.stages.exploratory} S1 {summary.stages["reduced-order"]} S2{" "}
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
                ...
                {report?.path ? <span className="text-slate-500">{report.path}</span> : null}
              </div>
              {!report?.available && (
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(360px,1fr)_320px]">
                  ...
                  <CardContent className="space-y-4">
                    {!selected ? (
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge className={cn("border", STATUS_TEXT[selected.status])}>
                            {formatStatusLabel(selected.status)}
                          </Badge>
                          {selected.stage && (
                            <Badge variant="outline" className={STAGE_BADGE[selected.stage]}>
                              {STAGE_LABELS[selected.stage]} {formatStageName(selected.stage)}
                            </Badge>
                          )}
                          {selected.tag && (
