...
export type RepoEvidenceZenGraphInput = {
  repoRef: string;
  commitRef?: string;
  fileRefs?: string[];
...
export type RepoEvidenceZenGraphResult = {
...
function buildRepoRecommendations(reflection: IdeologyContextReflectionV1): IdeologyContextReflectionRecommendedActionV1[] {
  ...
  if (hasLens(reflection, /integrity/i)) {
    recommendations.push({
      id: "zen-graph.show_integrity_lens",
  ...
  if (hasLens(reflection, /capture|resistance/i)) {
    recommendations.push({
      id: "zen-graph.show_capture_resistance_warning",
  ...
  if (asksForTestEvidence) {
    recommendations.push({
      id: "zen-graph.ask_for_test_evidence",
  ...
  if ((reflection.action_gate_warnings ?? []).length > 0 || hasLens(reflection, /gate|approval|review/i)) {
    recommendations.push({
      id: "zen-graph.suggest_review_gate",
...
function mapRepoRecommendation(
  ...
  if (REPO_MUTATION_ACTION_TYPES.has(recommendation.type)) {
    return {
      ...
      reason: "Repo Evidence ZenGraph cannot trigger repo mutation, command execution, commits, or code edits.",
      reasonCode: "unknown_action_not_allowlisted",
      display_policy: "hidden",
      reasonCodes: ["zen_graph_reflection", "repo_mutation_blocked", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "suggest_review_gate") {
    return {
      ...
      requiresConfirmation: true,
      agentExecutable: false,
      reason: "Repo Evidence ZenGraph review gate suggestions are claim-sensitive and require user confirmation.",
      reasonCode: "claim_sensitive_language",
      display_policy: "actionable",
      reasonCodes: ["zen_graph_reflection", "review_gate_requires_confirmation", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "ask_for_test_evidence") {
    return {
      ...
      reasonCodes: ["zen_graph_reflection", "ask_for_test_evidence", "diagnostic_overlay_only", "evidence_only_authority"],
    };
  }
  ...
  if (
    ...
    return {
      ...
      requiresConfirmation: false,
      agentExecutable: false,
      reason: "Repo Evidence ZenGraph display recommendation is diagnostic-only.",
      reasonCode: "read_only_allowlisted",
      display_policy: "diagnostic_only",
  ...
  return {
    ...
    requiresConfirmation: true,
    agentExecutable: false,
    reason: "Repo Evidence ZenGraph recommendation type is not allowlisted.",
    reasonCode: "unknown_action_not_allowlisted",
    display_policy: "hidden",
    reasonCodes: ["zen_graph_reflection", "unknown_repo_zen_graph_action", "evidence_only_authority"],
  };
}
...
function buildRepoAdmission(reflection: IdeologyContextReflectionV1): HelixRecommendedActionAdmissionV1 {
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    reasonCodes: ["zen_graph_reflection", "repo_evidence_adapter", "evidence_only_authority"],
    actions: recommendations.map((recommendation) => mapRepoRecommendation(reflection, recommendation)),
  });
}

export function reflectRepoEvidenceWithZenGraph(
  graph: IdeologyGraph,
  input: RepoEvidenceZenGraphInput,
): RepoEvidenceZenGraphResult {
