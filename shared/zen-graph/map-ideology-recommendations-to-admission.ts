...
function actionIdForRecommendation(recommendation: IdeologyContextReflectionRecommendedActionV1): string {
  return recommendation.id.startsWith("zen-graph.") ? recommendation.id : `zen-graph.${recommendation.type}`;
}
...
function mapRecommendation(
  ...
  if (BLOCKED_ACTION_TYPES.has(recommendation.type)) {
    return {
      ...
      reason: "ZenGraph recommendations cannot admit operational actions in this phase.",
      reasonCode: "unknown_action_not_allowlisted",
      display_policy: "hidden",
      reasonCodes: ["zen_graph_reflection", "blocked_operational_action", "evidence_only_authority"],
    };
  }
  ...
  if (MUTATING_ACTION_TYPES.has(recommendation.type)) {
    return {
      ...
      requiresConfirmation: true,
      agentExecutable: false,
      reason: "ZenGraph recommendation may mutate workstation state and requires user confirmation.",
      reasonCode: "workspace_mutation_requires_confirmation",
      display_policy: "actionable",
      reasonCodes: ["zen_graph_reflection", "mutating_requires_confirmation", "evidence_only_authority"],
    };
  }
  ...
  if (DISPLAY_ONLY_ACTION_TYPES.has(recommendation.type)) {
    return {
      ...
      agentExecutable: false,
      reason: "ZenGraph display-only recommendation is auto-admitted to diagnostic evidence stream only.",
  ...
  return {
    ...
    requiresConfirmation: true,
    agentExecutable: false,
    reason: "ZenGraph recommendation type is not allowlisted for admission.",
    reasonCode: "unknown_action_not_allowlisted",
    display_policy: "hidden",
    reasonCodes: ["zen_graph_reflection", "unknown_zen_graph_action", "evidence_only_authority"],
  };
}
