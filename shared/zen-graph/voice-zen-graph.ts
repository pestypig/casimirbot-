...
export type VoiceZenGraphInput = {
  voiceEventId: string;
  transcriptOrCalloutDraft: string;
};

export type VoiceZenGraphResult = {
...
function buildVoiceRecommendations(reflection: IdeologyContextReflectionV1): IdeologyContextReflectionRecommendedActionV1[] {
  ...
  if (matchedNode || rightSpeech || isClaimSensitive(summary)) {
    recommendations.push({
      id: "zen-graph.suggest_tone_adjustment",
  ...
  if (rightSpeech) {
    recommendations.push({
      id: "zen-graph.show_right_speech_warning",
  ...
  if (needsClarification(summary, reflection)) {
    recommendations.push({
      id: "zen-graph.ask_for_clarification",
  ...
  if (isClaimSensitive(summary)) {
    recommendations.push({
      id: "zen-graph.suggest_less_claim_sensitive_wording",
...
function mapVoiceRecommendation(
  ...
  if (VOICE_SEND_ACTION_TYPES.has(recommendation.type)) {
    return {
      ...
      reason: "Voice ZenGraph cannot auto-speak, auto-send, or call external voice tools.",
      reasonCode: "unknown_action_not_allowlisted",
      display_policy: "hidden",
      reasonCodes: ["zen_graph_reflection", "voice_send_blocked", "evidence_only_authority"],
    };
  }
  ...
  if (
    ...
    return {
      ...
      requiresConfirmation: true,
      agentExecutable: false,
      reason: "Voice ZenGraph suggestion is claim-sensitive and requires user confirmation before any voice action.",
      ...
      ...(recommendation.type === "ask_for_clarification" && missing.length > 0
        ...
        : {}),
      reasonCodes: ["zen_graph_reflection", "voice_suggestion_requires_confirmation", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "show_right_speech_warning") {
    return {
      ...
      reasonCodes: ["zen_graph_reflection", "right_speech_warning", "diagnostic_overlay_only", "evidence_only_authority"],
    };
  }
  ...
  return {
    ...
    requiresConfirmation: true,
    agentExecutable: false,
    reason: "Voice ZenGraph recommendation type is not allowlisted.",
    reasonCode: "unknown_action_not_allowlisted",
    display_policy: "hidden",
    reasonCodes: ["zen_graph_reflection", "unknown_voice_zen_graph_action", "evidence_only_authority"],
  };
}
...
function buildVoiceAdmission(reflection: IdeologyContextReflectionV1): HelixRecommendedActionAdmissionV1 {
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    reasonCodes: ["zen_graph_reflection", "voice_adapter", "evidence_only_authority"],
    actions: recommendations.map((recommendation) => mapVoiceRecommendation(reflection, recommendation)),
  });
}

export function reflectVoiceEventWithZenGraph(
  graph: IdeologyGraph,
  input: VoiceZenGraphInput,
): VoiceZenGraphResult {
