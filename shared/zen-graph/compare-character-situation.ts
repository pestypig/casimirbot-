...
import { buildFruitionFromZenBadgeComparisonSeed } from "./build-fruition-from-zen-badge-comparison-seed";
...
function posturePriority(posture: ZenBadgeComparisonPostureV1): number {
...
function dominantPosture(rules: Array<CharacterDecisionRuleV1 & { confidence: number }>): ZenBadgeComparisonPostureV1 {
...
function mapPostureToFruition(posture: ZenBadgeComparisonPostureV1): FruitionResultPostureV1 {
...
function profileWeightsById(profile: CharacterIdealProfileV1): Map<string, CharacterIdealProfileBadgeWeightV1> {
  return new Map(
    [...profile.zenBadgeWeights, ...profile.characterSpecificBadges].map((weight) => [weight.nodeId, weight]),
  );
}

function allLocatedLocations(locator: ZenBadgeLocatorV1): ZenBadgeLocationV1[] {
...
function enrichedLocatorText(situationText: string, rules: Array<CharacterDecisionRuleV1 & { confidence: number }>): string {
  ...
  return `${situationText}\nActivated Zen badge ids: ${activatedGenericNodes.join(", ")}.`;
}
...
export function compareCharacterSituation(input: CompareCharacterSituationInput): CharacterSituationComparisonV1 {
  ...
  const locator = overrideLocatorPosture(
    locateZenBadges(input.graph, {
      kind: input.inputKind ?? "user_prompt",
      text: enrichedLocatorText(input.situationText, rules),
      summary: input.situationText,
      refs: input.refs,
      generatedAt: input.generatedAt,
      locatorId: input.comparisonId ? `zen-badge-locator:${input.comparisonId}` : undefined,
    }),
    rules,
  );
  const fruition = buildFruitionFromZenBadgeComparisonSeed({
    locator,
