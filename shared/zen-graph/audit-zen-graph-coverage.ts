import {
  ZEN_GRAPH_COVERAGE_AUDIT_ARTIFACT_ID,
  ZEN_GRAPH_COVERAGE_AUDIT_SCHEMA_VERSION,
  type ZenGraphCoverageAuditNodeV1,
  type ZenGraphCoverageAuditSummaryV1,
  type ZenGraphCoverageAuditV1,
  type ZenGraphCoverageRecommendedPatchTypeV1,
  type ZenGraphCoverageStatusV1,
} from "../contracts/zen-graph-coverage-audit.v1";
...
import { getZenWisdomPrinciple } from "./wisdom-principles";
...
function graphFromInput(input: IdeologyGraph | IdeologyGraphDocument): IdeologyGraph {
  if ("nodeById" in input && input.nodeById instanceof Map) return input;
  return buildIdeologyGraph(input);
}
...
function actionIdsForNode(node: IdeologyGraphNode, isActionGate: boolean): string[] {
  ...
  if (isActionGate) actionIds.push(`zen-graph.action_gate.${node.id}`);
...
function missingPiecesForStatus(params: {
  status: ZenGraphCoverageStatusV1;
  hasPrinciple: boolean;
  isActionGate: boolean;
  hasActions: boolean;
...
function recommendedPatchType(params: {
  ...
  hasActions: boolean;
  hasReferences: boolean;
}): ZenGraphCoverageRecommendedPatchTypeV1 {
...
function notesForNode(params: {
  ...
  const notes: string[] = [];
  if (params.hasPrinciple) notes.push("Mapped by ZEN_WISDOM_PRINCIPLES procedural role/operator metadata.");
...
export function auditZenGraphCoverage(input: IdeologyGraph | IdeologyGraphDocument): ZenGraphCoverageAuditV1 {
  const graph = graphFromInput(input);
  const actionGateIds = new Set(findIdeologyActionGates(graph).map((node) => node.id));
  const nodes: ZenGraphCoverageAuditNodeV1[] = orderedNodes(graph).map((node) => {
    const principle = getZenWisdomPrinciple(node.id);
  ...
  const summary: ZenGraphCoverageAuditSummaryV1 = {
    mapped: 0,
    partial: 0,
    conceptual_only: 0,
    unmapped: 0,
  ...
  return {
    artifactId: ZEN_GRAPH_COVERAGE_AUDIT_ARTIFACT_ID,
    schemaVersion: ZEN_GRAPH_COVERAGE_AUDIT_SCHEMA_VERSION,
