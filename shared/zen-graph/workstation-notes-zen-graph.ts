...
export type WorkstationNoteZenGraphInput = {
  id: string;
  content: string;
  title?: string;
};

export type WorkstationNoteZenGraphResult = {
...
function mapNoteRecommendation(
  ...
  if (recommendation.type === "suggest_note_tag" || recommendation.type === "link_ethos_node") {
    return {
      ...
      reason: "Notes ZenGraph recommendation changes note metadata or links and requires user confirmation.",
      reasonCode: "workspace_mutation_requires_confirmation",
      display_policy: "actionable",
      reasonCodes: ["zen_graph_reflection", "notes_mutation_requires_confirmation", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "ask_for_missing_evidence") {
    return {
      ...
      reasonCodes: ["zen_graph_reflection", "missing_evidence", "diagnostic_overlay_only", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "suggest_review" || recommendation.type === "show_path_to_root") {
    return {
      ...
      requiresConfirmation: false,
      agentExecutable: false,
      reason: "Notes ZenGraph display recommendation is diagnostic-only.",
      reasonCode: "diagnostic_only_not_executable",
      display_policy: "diagnostic_only",
  ...
  return {
    ...
    requiresConfirmation: true,
    agentExecutable: false,
    reason: "Notes ZenGraph recommendation type is not allowlisted.",
    reasonCode: "unknown_action_not_allowlisted",
    display_policy: "hidden",
    reasonCodes: ["zen_graph_reflection", "unknown_notes_zen_graph_action", "evidence_only_authority"],
  };
}
...
function buildNoteAdmission(reflection: IdeologyContextReflectionV1): HelixRecommendedActionAdmissionV1 {
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    reasonCodes: ["zen_graph_reflection", "workstation_notes_adapter", "evidence_only_authority"],
    actions: recommendations.map((recommendation) => mapNoteRecommendation(reflection, recommendation)),
  });
}

export function reflectWorkstationNoteWithZenGraph(
  graph: IdeologyGraph,
  note: WorkstationNoteZenGraphInput,
): WorkstationNoteZenGraphResult {
