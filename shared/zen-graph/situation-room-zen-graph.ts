...
export type SituationRoomZenGraphInput = {
  situationId: string;
  actionId: string;
  missionActionSummary: string;
};

export type SituationRoomZenGraphResult = {
...
function buildSituationRoomRecommendations(
  ...
  if (hasLens(reflection, /two.?key|approval|review/i) || highImpact) {
    recommendations.push({
      id: "zen-graph.require_two_key_review",
  ...
  if (missingAuthority || highImpact) {
    recommendations.push({
      id: "zen-graph.ask_for_missing_authority",
  ...
  if (hasLens(reflection, /lawful|jurisdiction|interface/i) || highImpact) {
    recommendations.push({
      id: "zen-graph.show_lawful_interface_check",
  ...
  if (hasLens(reflection, /non.?harm|harm|safety|restraint/i) || highImpact) {
    recommendations.push({
      id: "zen-graph.show_non_harm_check",
  ...
  if (highImpact || missingAuthority) {
    recommendations.push({
      id: "zen-graph.block_policy_sensitive_action",
      type: "block_policy_sensitive_action",
      label: "Block policy-sensitive action",
      description: "Block unclear or high-impact operational action from being approved by ZenGraph alone.",
...
function mapSituationRoomRecommendation(
  ...
  if (recommendation.type === "block_policy_sensitive_action") {
    return {
      ...
      reason: "Situation Room ZenGraph cannot approve, execute, or unlock high-impact mission actions.",
      reasonCode: "unknown_action_not_allowlisted",
      display_policy: "hidden",
      reasonCodes: ["zen_graph_reflection", "policy_sensitive_action_blocked", "evidence_only_authority"],
    };
  }
  ...
  if (recommendation.type === "require_two_key_review" || recommendation.type === "ask_for_missing_authority") {
    return {
      ...
      requiresConfirmation: true,
      agentExecutable: false,
      reason: "Situation Room ZenGraph review and authority requirements need operator confirmation.",
      ...
      ...(recommendation.type === "ask_for_missing_authority"
        ...
        : {}),
      reasonCodes: ["zen_graph_reflection", "situation_room_review_required", "evidence_only_authority"],
    };
  }
  ...
  if (
    ...
    return {
      ...
      agentExecutable: false,
      reason: "Situation Room ZenGraph warning is diagnostic-only and cannot execute mission actions.",
  ...
  return {
    ...
    requiresConfirmation: true,
    agentExecutable: false,
    reason: "Situation Room ZenGraph recommendation type is not allowlisted.",
    reasonCode: "unknown_action_not_allowlisted",
    display_policy: "hidden",
    reasonCodes: ["zen_graph_reflection", "unknown_situation_room_zen_graph_action", "evidence_only_authority"],
  };
}
...
function buildSituationRoomAdmission(reflection: IdeologyContextReflectionV1): HelixRecommendedActionAdmissionV1 {
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    reasonCodes: ["zen_graph_reflection", "situation_room_adapter", "evidence_only_authority"],
    actions: recommendations.map((recommendation) => mapSituationRoomRecommendation(reflection, recommendation)),
  });
}

export function reflectSituationRoomEventWithZenGraph(
  graph: IdeologyGraph,
  input: SituationRoomZenGraphInput,
): SituationRoomZenGraphResult {
