...
export type BuildFruitionFromZenBadgeComparisonSeedInput = {
...
function uniqueLocations(locator: ZenBadgeLocatorV1): ZenBadgeLocationV1[] {
  ...
  const result: ZenBadgeLocationV1[] = [];
  const ordered = [
...
function mapSeedPostureToFruition(posture: ZenBadgeComparisonPostureV1): FruitionResultPostureV1 {
  if (posture === "requires_check") return "requires_review";
  if (posture === "blocked_or_missing_check") return "ask_for_clarification";
  return "diagnostic_only";
}

function polarityFromSeedPosture(posture: ZenBadgeComparisonPostureV1): FruitionTermPolarityV1 {
...
function operatorFromSeedPosture(posture: ZenBadgeComparisonPostureV1): FruitionProceduralOperatorV1 {
  if (posture === "requires_check" || posture === "blocked_or_missing_check") return "requires";
  if (posture === "constrained_action_posture") return "constrains";
  return "supports";
}

function operatorKindFromSeedPosture(posture: ZenBadgeComparisonPostureV1): FruitionOperatorKindV1 {
  if (posture === "requires_check" || posture === "blocked_or_missing_check") return "requires";
  if (posture === "constrained_action_posture") return "constrains";
  return "supports";
}

function roleFromLocation(location: ZenBadgeLocationV1, posture: ZenBadgeComparisonPostureV1): FruitionProceduralRoleV1 {
...
function kindFromLocation(location: ZenBadgeLocationV1): FruitionTermKindV1 {
...
function buildSeedAdmission(locator: ZenBadgeLocatorV1, refs: string[], generatedAt?: string): HelixRecommendedActionAdmissionV1 {
  ...
  return buildHelixRecommendedActionAdmissionV1({
    ...
    source: {
      ...
      tool: "zen-badge-locator",
      artifact_type: "zen_badge_locator",
    ...
    actions: [
      {
        ...
        reason: "Zen badge locator seed can be displayed as diagnostic Fruition evidence only.",
        ...
        reasonCodes: ["zen_badge_locator_seed", "evidence_only_authority"],
      },
    ],
    evidenceRefs: refs,
    evidenceRequirements: missing.length > 0 ? { missing } : undefined,
    reasonCodes: ["zen_badge_locator_seed", "diagnostic_overlay_only", "evidence_only_authority"],
  });
}

export function buildFruitionFromZenBadgeComparisonSeed(
  input: BuildFruitionFromZenBadgeComparisonSeedInput,
  ...
  const operators = [
    {
      ...
      rationale:
        "Located Zen badges provide evidence-only procedural context; they do not decide execution or moral finality.",
    },
  ];
