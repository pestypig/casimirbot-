...
import { reflectWorkstationNoteWithZenGraph } from "../workstation-notes-zen-graph";
...
describe("workstation notes ZenGraph adapter", () => {
  it("produces a ZenGraph reflection from note text and preserves note refs", () => {
    const result = reflectWorkstationNoteWithZenGraph(graph, {
      id: "note:ethos:1",
  ...
  it("requires ask_user for suggested note tags and does not auto-mutate notes", () => {
    ...
    const before = { ...note };
    const result = reflectWorkstationNoteWithZenGraph(graph, note);
    const admission = result.admissions[0]!;
    const tagAction = admission.actions.find((action) => action.actionId === "zen-graph.suggest_note_tag");
  ...
  it("maps missing evidence to diagnostic_only without execution", () => {
    ...
    const admission = result.admissions[0]!;
    const missingAction = admission.actions.find((action) => action.actionId === "zen-graph.ask_for_missing_evidence");
  ...
  it("requires ask_user for related ethos node links and identifies Notes as source", () => {
    ...
    const admission = result.admissions[0]!;
    const linkAction = admission.actions.find((action) => action.actionId === "zen-graph.link_ethos_node");
