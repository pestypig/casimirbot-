...
import { reflectSituationRoomEventWithZenGraph } from "../situation-room-zen-graph";
...
describe("situation room ZenGraph adapter", () => {
  it("reflects situation room events and preserves situation/action refs", () => {
    const result = reflectSituationRoomEventWithZenGraph(graph, {
  ...
  it("maps warnings and checks to auto claim-sensitive diagnostic admissions", () => {
    const admission = reflectSituationRoomEventWithZenGraph(graph, {
      situationId: "situation:alpha",
    ...
    for (const actionId of [
      "zen-graph.show_lawful_interface_check",
      "zen-graph.show_non_harm_check",
    ]) {
  ...
  it("requires ask_user for two-key review and missing authority", () => {
    ...
    expect(validateHelixRecommendedActionAdmissionV1(admission)).toEqual([]);
    for (const actionId of [
      "zen-graph.require_two_key_review",
      "zen-graph.ask_for_missing_authority",
    ]) {
      expect(admission.actions.find((action) => action.actionId === actionId)).toMatchObject({
        admission: "ask_user",
        risk: "claim_sensitive",
  ...
  it("blocks high-impact or unclear operational actions", () => {
    ...
    const blocked = admission.actions.find((action) => action.actionId === "zen-graph.block_policy_sensitive_action");
  ...
  it("cannot approve or execute mission actions by itself", () => {
    ...
    const before = { ...event };
    const admission = reflectSituationRoomEventWithZenGraph(graph, event).admissions[0]!;
