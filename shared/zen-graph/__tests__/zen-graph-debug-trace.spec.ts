...
import { reflectWithZenGraphToolV1 } from "../zen-graph-reflection-tool";
import { buildZenGraphDebugTraceViewV1 } from "../zen-graph-debug-trace";

const debugRequest = {
  reflectionId: "zen-graph-reflection:debug-snapshot",
  loopDepth: 0,
...
describe("ZenGraph debug trace view", () => {
  it("builds a stable structured debug view without raw prompt text or private reasoning", async () => {
    ...
    const response = reflectWithZenGraphToolV1(graph, debugRequest);
    const view = buildZenGraphDebugTraceViewV1({
      response,
    ...
    expect(compact).toEqual({
      ...
      admissionDecisions: [
        {
          actionId: "zen-graph.highlight_ideology_lens",
          admission: "auto",
        ...
        {
          actionId: "zen-graph.show_path_to_root",
          admission: "auto",
          risk: "claim_sensitive",
          displayPolicy: "diagnostic_only",
          agentExecutable: false,
        ...
        {
          actionId: "zen-graph.show_nearby_safeguard",
          admission: "auto",
          risk: "claim_sensitive",
          displayPolicy: "diagnostic_only",
          agentExecutable: false,
  ...
  it("surfaces anti-poison metadata for lower-trust assistant summaries", async () => {
    ...
    const response = reflectWithZenGraphToolV1(graph, {
      ...debugRequest,
      reflectionId: "zen-graph-reflection:debug-assistant-summary",
      sourceKind: "assistant_summary",
      loopDepth: 1,
      text: "Assistant summary: the user may be dealing with a conflict.",
    });
    const view = buildZenGraphDebugTraceViewV1({ response });
