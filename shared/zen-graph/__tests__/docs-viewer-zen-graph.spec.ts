...
import { reflectDocsSelectionWithZenGraph } from "../docs-viewer-zen-graph";
...
describe("docs viewer ZenGraph adapter", () => {
  it("reflects selected text and preserves document refs as evidenceRefs", () => {
    const result = reflectDocsSelectionWithZenGraph(graph, {
  ...
  it("maps highlight and show actions to auto read-only diagnostic admissions", () => {
    const admission = reflectDocsSelectionWithZenGraph(graph, {
      documentId: "doc:ethos:right-speech",
      selectionRange: "120:260",
      selectedText: "Right Speech Infrastructure should frame this selected passage.",
    }).admissions[0]!;
    ...
    for (const actionId of ["zen-graph.highlight_relevant_ethos_passage", "zen-graph.show_related_ethos_node"]) {
  ...
  it("maps action gate warnings to read-only diagnostic display", () => {
    ...
    expect(admission.actions.find((action) => action.actionId === "zen-graph.show_action_gate_warning")).toMatchObject({
      admission: "auto",
      risk: "read_only",
      display_policy: "diagnostic_only",
      agentExecutable: false,
  ...
  it("requires ask_user for document annotation suggestions", () => {
    ...
    const annotation = admission.actions.find((action) => action.actionId === "zen-graph.suggest_doc_annotation");
  ...
  it("does not allow document mutation without user confirmation", () => {
    ...
    const admission = reflectDocsSelectionWithZenGraph(graph, selected).admissions[0]!;
