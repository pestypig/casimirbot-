...
import { validateZenObjectiveBindingV1 } from "../../contracts/zen-objective-binding.v1";
...
describe("ZenGraph objective preset resolver", () => {
  ...
  it("resolves a named character perspective preset without a separate character graph", async () => {
    const graph = await loadIdeologyGraphFromFile();
    const binding = resolveZenGraphObjectivePresetV1(graph, REINHARD_CHARACTER_PERSPECTIVE_PRESET, {
    ...
    expect(binding.trace.map((entry) => entry.reason).join(" ")).toMatch(/badge weights over the Zen graph/i);
  });

  it("fails preset validation when a badge reference is missing from the ideology graph", async () => {
    const graph = await loadIdeologyGraphFromFile();
    const invalid: ZenGraphObjectivePresetV1 = {
      ...DEFAULT_ZEN_WISDOM_PRESET,
      presetId: "zen.preset.invalid",
      badgeWeights: [
        ...DEFAULT_ZEN_WISDOM_PRESET.badgeWeights,
        {
          badgeId: "missing-zen-badge",
          weight: 0.5,
        },
      ],
    };

    expect(validateZenGraphObjectivePresetV1(graph, invalid)).toEqual(
      expect.arrayContaining(["missing ideology badge reference: missing-zen-badge"]),
    );
    expect(() => resolveZenGraphObjectivePresetV1(graph, invalid)).toThrow(/missing-zen-badge/);
  });
