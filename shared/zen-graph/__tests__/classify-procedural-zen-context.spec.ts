...
import { validateProceduralZenClassificationV1 } from "../../procedural-zen-classification";
...
import { classifyProceduralZenContext } from "../classify-procedural-zen-context";
...
function classify(text: string) {
  ...
  return classifyProceduralZenContext({
    graph,
    reflection,
    text,
    generatedAt: "2026-06-09T00:00:00.000Z",
    classificationId: "procedural-zen:conversation",
  });
}

describe("procedural Zen context classifier", () => {
  ...
  it("classifies ignorance, guilt, and missing consideration as inquiry and repair posture", () => {
    ...
    expect(classification.recommendedNextMoves).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "procedural-zen-action:ask-reasonably-knowable",
        }),
        expect.objectContaining({
          id: "procedural-zen-action:separate-guilt-from-repair",
        }),
      ]),
    );
