...
describe("Zen wisdom procedural principle catalog", () => {
  it("requires every principle mapping to carry procedural source, rule, trace, and boundaries", () => {
    expect(ZEN_WISDOM_PRINCIPLES.length).toBe(26);

    for (const principle of ZEN_WISDOM_PRINCIPLES) {
  ...
  it("models safety and approval ideology nodes as concrete action gates and authority boundaries", () => {
    expect(getZenWisdomPrinciple("two-key-approval")).toMatchObject({
    ...
    expect(getZenWisdomPrinciple("two-key-approval")?.proceduralRule).toMatch(/Require both legal\/authority and ethos\/user/);
    expect(getZenWisdomPrinciple("two-key-approval")?.traceBehavior).toMatch(/required keys/i);

    expect(getZenWisdomPrinciple("no-bypass-guardrail")).toMatchObject({
      proceduralRole: "authority_boundary",
      procedureOperator: "blocks",
      refusesAuthority: ["action_bypass", "consent_override"],
    });
    expect(getZenWisdomPrinciple("no-bypass-guardrail")?.traceBehavior).toMatch(/agent_executable false/);
  });

  it("models provenance and anti-poisoning ideology nodes as evidence-sensitive procedural constraints", () => {
    expect(getZenWisdomPrinciple("provenance-protocol")).toMatchObject({
    ...
    expect(getZenWisdomPrinciple("feedback-loop-hygiene")).toMatchObject({
    ...
    expect(getZenWisdomPrinciple("feedback-loop-hygiene")?.traceBehavior).toMatch(/loop depth/);
  });

  it("keeps legal, financial, and character-sensitive mappings evidence-only rather than authoritative", () => {
    expect(getZenWisdomPrinciple("access-to-counsel-pathway")).toMatchObject({
      proceduralRole: "action_gate",
      procedureOperator: "requires",
      refusesAuthority: ["legal_advice_as_authority"],
    });

    expect(getZenWisdomPrinciple("financial-fog-warning")).toMatchObject({
      proceduralRole: "authority_boundary",
      procedureOperator: "blocks",
      refusesAuthority: ["financial_advice_as_authority"],
    });

    expect(getZenWisdomPrinciple("flattery-laundering-detection")).toMatchObject({
      proceduralRole: "constraint",
      procedureOperator: "blocks",
