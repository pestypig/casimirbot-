...
import { reflectVoiceEventWithZenGraph } from "../voice-zen-graph";
...
describe("voice ZenGraph adapter", () => {
  it("reflects voice events and preserves the voice event ref", () => {
    const result = reflectVoiceEventWithZenGraph(graph, {
  ...
  it("maps tone suggestions to ask_user claim-sensitive actionable admissions", () => {
    const admission = reflectVoiceEventWithZenGraph(graph, {
    ...
    const tone = admission.actions.find((action) => action.actionId === "zen-graph.suggest_tone_adjustment");
  ...
  it("maps right speech warnings to auto diagnostic evidence-only admissions", () => {
    ...
    const warning = admission.actions.find((action) => action.actionId === "zen-graph.show_right_speech_warning");
  ...
  it("suggests less claim-sensitive wording and clarification without executable voice authority", () => {
    ...
    for (const actionId of [
      "zen-graph.suggest_less_claim_sensitive_wording",
      "zen-graph.ask_for_clarification",
    ]) {
  ...
  it("cannot auto-speak or auto-send anything based on ZenGraph alone", () => {
    const voiceEvent = {
    ...
    const admission = reflectVoiceEventWithZenGraph(graph, voiceEvent).admissions[0]!;
