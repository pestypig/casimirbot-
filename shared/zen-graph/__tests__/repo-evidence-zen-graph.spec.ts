...
import { reflectRepoEvidenceWithZenGraph } from "../repo-evidence-zen-graph";
...
describe("repo evidence ZenGraph adapter", () => {
  it("reflects repo evidence and preserves repo, commit, and file refs", () => {
    const result = reflectRepoEvidenceWithZenGraph(graph, {
      repoRef: "repo:casimirbot",
      commitRef: "commit:abc123",
      fileRefs: ["shared/zen-graph/repo-evidence-zen-graph.ts"],
    ...
    expect(result.reflection.input.refs).toEqual([
      "repo:casimirbot",
      "commit:abc123",
      "shared/zen-graph/repo-evidence-zen-graph.ts",
    ]);
  ...
  it("maps read-only repo evidence reflection to auto diagnostic admissions", () => {
    const admission = reflectRepoEvidenceWithZenGraph(graph, {
      repoRef: "repo:casimirbot",
      commitRef: "commit:abc123",
    ...
    for (const actionId of [
      "zen-graph.show_integrity_lens",
      "zen-graph.show_capture_resistance_warning",
      "zen-graph.link_policy_node",
    ]) {
  ...
  it("asks the user for review gate suggestions", () => {
    ...
    const reviewGate = admission.actions.find((action) => action.actionId === "zen-graph.suggest_review_gate");
  ...
  it("asks for test evidence as read-only diagnostic evidence", () => {
    ...
    const testEvidence = admission.actions.find((action) => action.actionId === "zen-graph.ask_for_test_evidence");
  ...
  it("does not allow repo mutation, command execution, commits, or code edits", () => {
    ...
    const admission = reflectRepoEvidenceWithZenGraph(graph, evidence).admissions[0]!;
