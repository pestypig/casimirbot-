...
export const DEFAULT_THEORY_RUNTIME_ADAPTER_GAP_REGISTRY:
  TheoryRuntimeAdapterGapRegistryEntry[] = [
    {
    ...
    },
    {
      ...
      sourcePath: "shared/theory/runtime-traces/static-casimir-runtime-trace.ts",
    ...
    },
    {
      ...
      sourcePath: "server/services/theory/runtime-adapters/starsim-runtime-adapter.ts",
    ...
    },
    {
      ...
      sourcePath: "server/services/theory/runtime-adapters/tokamak-runtime-adapter.ts",
    ...
    },
    {
      ...
      runtimeIds: ["solar.pipeline", "solar.manifest", "casimir.verify", "physics.validate"],
    ...
    },
    {
      ...
      runtimeIds: THEORY_RUNTIME_ENTRYPOINTS.map((entrypoint) => entrypoint.runtimeId),
    ...
    },
    {
      ...
      runtimeIds: ["warp.full_solve.campaign", "nhm2.shift_lapse.alpha_sweep"],
      sourcePath: "server/services/theory/theory-runtime-run-request-manifest.ts",
      note: "Manifest/status support for long runs; no worker execution implied.",
    },
  ];

const PATCH_BY_MISSING_LEVEL: Record<TheoryRuntimeAdapterCoverageLevel, string> = {
...
function entrypointsForBlock(
  ...
  return entrypoints.filter((entrypoint) => intersects(entrypoint.ownedBadgeIds, badgeIds));
}
...
function adapterMatchesBlock(args: {
  ...
  const runtimeIds = new Set(registeredEntrypoints.map((entrypoint) => entrypoint.runtimeId));
  if (intersects(adapter.runtimeIds, runtimeIds)) return true;

  const families = new Set(registeredEntrypoints.map((entrypoint) => entrypoint.family));
...
function countPresent(ids: string[], graphBadgeIds: ReadonlySet<string>): number {
...
function recommendNextPatch(
  ...
  if (!firstMissing) return `chore(theory): keep ${block.id} runtime adapter coverage current`;
...
export function buildTheoryRuntimeAdapterGapReport(args: {
  ...
  const runtimeEntrypoints = args.runtimeEntrypoints ?? THEORY_RUNTIME_ENTRYPOINTS;
  const registeredAdapters = args.registeredAdapters ?? DEFAULT_THEORY_RUNTIME_ADAPTER_GAP_REGISTRY;
  ...
  const lanes = atlasBlocks.map((block): TheoryRuntimeAdapterGapLaneReport => {
    ...
    const matchingAdapters = registeredAdapters.filter((adapter) =>
      adapterMatchesBlock({ adapter, block, registeredEntrypoints: entrypoints }),
    );
    const staticTraceAvailable = hasCoverage(matchingAdapters, "static_reference");
    const artifactReaderAvailable = hasCoverage(matchingAdapters, "artifact_reader");
    const quickRuntimeAvailable = hasCoverage(matchingAdapters, "quick_runtime");
    const longRuntimeManifestAvailable = hasCoverage(matchingAdapters, "long_job_manifest");
    const liveRuntimeAvailable = hasCoverage(matchingAdapters, "live_runtime");
    const missingAdapterKinds = THEORY_RUNTIME_ADAPTER_COVERAGE_LEVELS.filter((level) => {
    ...
    return {
      ...
      primaryBadgeIdsPresentCount: countPresent(block.primaryBadgeIds, graphBadgeIds),
      ...
      claimBoundaryBadgeIdsPresentCount: countPresent(block.claimBoundaryBadgeIds, graphBadgeIds),
      calculatorExamplesCount: block.calculatorExamples.length,
      runtimeActionsCount: block.runtimeActions.length,
      registeredEntrypoints: entrypoints.map((entrypoint) => entrypoint.runtimeId),
  ...
  return {
    ...
    lanes,
    summary: {
      staticReferenceCount: lanes.filter((lane) => lane.staticTraceAvailable).length,
      artifactReaderCount: lanes.filter((lane) => lane.artifactReaderAvailable).length,
      quickRuntimeCount: lanes.filter((lane) => lane.quickRuntimeAvailable).length,
      longRuntimeManifestCount: lanes.filter((lane) => lane.longRuntimeManifestAvailable).length,
      liveRuntimeCount: lanes.filter((lane) => lane.liveRuntimeAvailable).length,
