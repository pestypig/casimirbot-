...
describe("curvature/collapse theory badges", () => {
  it("adds a loadable benchmark branch to the main graph", () => {
    ...
    expect(curvature.badges.some((badge) => badge.id === "collapse.objective.present_window")).toBe(true);
    expect(curvature.badges.some((badge) => badge.id === "collapse.objective.dp_hazard_probability")).toBe(true);
  ...
  it("keeps DP self-energy runtime-owned while exposing only scalar cuts to the calculator", () => {
    ...
    const presentWindow = curvature.badges.find((badge) => badge.id === "collapse.objective.present_window");
    ...
    expect(presentWindow?.calculatorPayloads.map((payload) => payload.targetVariable)).toContain("L_present_DP");
