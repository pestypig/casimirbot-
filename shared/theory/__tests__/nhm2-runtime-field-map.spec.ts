...
describe("NHM2 runtime field map", () => {
  ...
  it("distinguishes universal/model roots from runtime-bound outputs", () => {
    expect(
      getNhm2RuntimeFieldBinding("physics.gr.einstein_field_equation")?.kind,
    ).toBe("model_relation");
    expect(isNhm2RuntimeBoundBadge("physics.gr.einstein_field_equation")).toBe(
      false,
    );
    expect(
      isNhm2RuntimeBoundBadge("nhm2.closure.wall_t00_source_residual"),
    ).toBe(true);
    expect(isNhm2RuntimeBoundBadge("nhm2.source.wall_t00_trace")).toBe(true);
    expect(isNhm2RuntimeBoundBadge("nhm2.tensor.full_authority_gate")).toBe(
      true,
    );
    expect(isNhm2RuntimeBoundBadge("nhm2.tensor.same_chart_full_tensor")).toBe(
      true,
    );
    expect(
      isNhm2RuntimeBoundBadge("nhm2.source.same_basis_tensor_authority"),
    ).toBe(true);
    ...
    expect(
      isNhm2RuntimeBoundBadge("nhm2.energy_condition.observer_robust_gate"),
    ).toBe(true);
    expect(isNhm2RuntimeBoundBadge("nhm2.natario.invariant_audit")).toBe(true);
    expect(
      isNhm2RuntimeBoundBadge("nhm2.energy_condition.diagnostic_gate"),
    ).toBe(true);
    expect(
      isNhm2RuntimeBoundBadge("nhm2.meta.experiment_ready_theory_closure"),
    ).toBe(true);
  });

  it("maps experiment-ready theory closure as an independent fail-closed runtime lamp", () => {
    const binding = getNhm2RuntimeFieldBinding(
      "nhm2.meta.experiment_ready_theory_closure",
    );

    expect(binding?.kind).toBe("runtime_gate");
    expect(binding?.runtimeId).toBe("gr_nhm2.artifact_reader");
    expect(binding?.scalarCuts).toEqual([]);
    expect(binding?.artifactFields).toEqual(
      expect.arrayContaining([
    ...
    expect(binding?.artifactFields).not.toContain(
      "nhm2ExperimentReadyTheoryClosure.evidence[].checks.*.policySetSha256",
    );
    expect(
      binding?.artifactFields.some((field) =>
        field.startsWith("nhm2ExperimentReadyTheoryClosureEvaluation."),
      ),
    ).toBe(false);
    ...
    expect(binding?.claimBoundaryNotes.join(" ")).toMatch(
      /leaves physical viability, transport, propulsion, route ETA, and speed authority blocked/i,
    );
    expect(binding?.claimBoundaryNotes.join(" ")).toMatch(
      /pre-run candidate manifest.*post-run receipts.*filesystem replay/i,
    );
  });

  it("maps wall T00 and full tensor authority badges to explicit blocker fields", () => {
    const wallClosure = getNhm2RuntimeFieldBinding(
      "nhm2.closure.wall_t00_source_residual",
    );
    const wall = getNhm2RuntimeFieldBinding("nhm2.source.wall_t00_trace");
    const tensor = getNhm2RuntimeFieldBinding(
      "nhm2.tensor.full_authority_gate",
    );
    const sameChartTensor = getNhm2RuntimeFieldBinding(
    ...
    expect(wall?.gates).toEqual(
      expect.arrayContaining(["source_closure", "wall_t00_trace"]),
    );
  ...
  it("maps observer-robust energy and Natario invariant gates without scalar cuts", () => {
    const observerRobust = getNhm2RuntimeFieldBinding(
      "nhm2.energy_condition.observer_robust_gate",
    );
    const natarioAudit = getNhm2RuntimeFieldBinding(
  ...
  it("finds every binding involved in a runtime gate", () => {
    const sourceClosureBindings =
      findNhm2RuntimeFieldBindingsForGate("source_closure");
  ...
  it("keeps every runtime-bound NHM2 binding claim-bounded", () => {
    const unbounded = NHM2_RUNTIME_FIELD_BINDINGS.filter(
      (binding) =>
        ...
        (binding.claimBoundaryNotes.length === 0 ||
          binding.requiredEvidence.length === 0),
    );
  ...
  it("adds runtime-bound field metadata to compound theory runs", () => {
    ...
    const sourceRow = run.rows.find(
      (row) => row.badgeId === "nhm2.closure.source_residual",
    );

    expect(isTheoryCompoundRunV1(run)).toBe(true);
    expect(sourceRow?.warnings.join(" ")).toMatch(/Runtime-bound badge/i);
    expect(
      sourceRow?.evidenceRefs?.some((ref) => ref.kind === "runtime_field_map"),
    ).toBe(true);
    expect(
      sourceRow?.evidenceRefs?.some(
        (ref) => ref.kind === "runtime_gate" && ref.id === "source_closure",
      ),
    ).toBe(true);
    expect(
      sourceRow?.evidenceRefs?.some(
        (ref) =>
          ref.kind === "required_evidence" &&
          ref.id === "source_closure_artifact",
      ),
    ).toBe(true);
    expect(sourceRow?.claimBoundaryNotes.join(" ")).toMatch(
      /runtime-bound closure evidence/i,
    );
  });
});
