...
export const MORAL_GRAPH_REFLECTION_MAX_LOOP_DEPTH = 2 as const;

export const MORAL_GRAPH_REFLECTION_SOURCE_KINDS = [
  "user_text",
  "workstation_content",
  "document_selection",
  "repo_evidence",
...
export const MORAL_GRAPH_REFLECTION_SOURCE_TRUST_LEVELS = ["primary", "derived", "low_trust"] as const;

export type MoralGraphReflectionSourceKindV1 = (typeof MORAL_GRAPH_REFLECTION_SOURCE_KINDS)[number];
export type MoralGraphReflectionSourceTrustV1 = (typeof MORAL_GRAPH_REFLECTION_SOURCE_TRUST_LEVELS)[number];

export type MoralGraphRecommendedAction = IdeologyContextReflectionRecommendedActionV1;

export type MoralGraphReflectionToolOptionsV1 = {
...
export type MoralGraphReflectionToolRequestV1 = {
  reflectionId?: string;
  parentReflectionId?: string;
  loopDepth: number;
  sourceKind: MoralGraphReflectionSourceKindV1;
  inputKind: IdeologyContextReflectionInputKindV1;
  text: string;
  refs?: string[];
  requestedPresetIds?: string[];
  comparePresetIds?: string[];
  options?: MoralGraphReflectionToolOptionsV1;
};

export type MoralGraphReflectionProvenanceV1 = {
  reflectionId: string;
  parentReflectionId?: string;
  loopDepth: number;
  sourceKind: MoralGraphReflectionSourceKindV1;
  sourceTrust: MoralGraphReflectionSourceTrustV1;
  continuityOnly: boolean;
  confirmationEligible: boolean;
  confidenceCap: number;
};

export type MoralGraphReflectionToolResponseV1 = {
...
function validateSourceKind(value: unknown, issues: string[]): void {
  if (!includes(MORAL_GRAPH_REFLECTION_SOURCE_KINDS, value)) {
    issues.push("sourceKind is invalid");
  }
}

function validateSourceTrust(value: unknown, issues: string[]): void {
  if (!includes(MORAL_GRAPH_REFLECTION_SOURCE_TRUST_LEVELS, value)) {
...
function validateLoopDepth(value: unknown, issues: string[]): void {
  ...
  if (value < 0 || value > MORAL_GRAPH_REFLECTION_MAX_LOOP_DEPTH) {
    issues.push(`loopDepth must be between 0 and ${MORAL_GRAPH_REFLECTION_MAX_LOOP_DEPTH}`);
  }
}
...
export function validateMoralGraphReflectionToolRequestV1(value: unknown): string[] {
  const issues: string[] = [];
  if (!isRecord(value)) return ["MoralGraph reflection tool request must be an object"];
...
export function validateMoralGraphReflectionToolResponseV1(value: unknown): string[] {
  const issues: string[] = [];
  if (!isRecord(value)) return ["MoralGraph reflection tool response must be an object"];
  ...
  if (value.proceduralClassification !== undefined) {
    issues.push(
      ...validateProceduralMoralClassificationV1(value.proceduralClassification).map(
  ...
  if (Array.isArray(value.presetOverlays)) {
    value.presetOverlays.forEach((overlay, index) => {
      issues.push(...validateMoralObjectiveBindingV1(overlay).map((issue) => `presetOverlays[${index}].${issue}`));
    });
...
export function isMoralGraphReflectionToolRequestV1(value: unknown): value is MoralGraphReflectionToolRequestV1 {
  return validateMoralGraphReflectionToolRequestV1(value).length === 0;
}

export function isMoralGraphReflectionToolResponseV1(value: unknown): value is MoralGraphReflectionToolResponseV1 {
  return validateMoralGraphReflectionToolResponseV1(value).length === 0;
}
