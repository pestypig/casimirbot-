...
describe("physics_atlas/v1", () => {
  it("validates the compact physics atlas blocks", () => {
    ...
    expect(atlas.blocks.map((block) => block.id)).toEqual([...PHYSICS_ATLAS_BLOCK_IDS]);
  ...
  it("rejects forbidden overclaiming language", () => {
    ...
    expect(validatePhysicsAtlasV1(atlas).some((issue) => issue.includes("forbidden overclaiming"))).toBe(true);
  });
});
