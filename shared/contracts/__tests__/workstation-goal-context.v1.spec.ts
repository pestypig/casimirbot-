  ...
  it("rejects goal-context dispatch actions without auditable targets", () => {
    expect(validateWorkstationGoalContextUpdateV1({
      ...
      suggestedDispatch: [
        ...
        { kind: "change_preset", targetRef: "", presetId: "" },
        { kind: "bind_source", sourceRef: "", targetRef: "" },
        { kind: "unbind_source", sourceRef: "" },
        ...
        { kind: "repair_source" },
        { kind: "focus_process_graph", nodeRef: "" },
        { kind: "teleport_packet" as never },
      ...
      "suggestedDispatch[4].change_preset must include targetRef",
      "suggestedDispatch[4].change_preset must include presetId",
      "suggestedDispatch[5].bind_source must include sourceRef",
      "suggestedDispatch[5].bind_source must include targetRef",
      "suggestedDispatch[6].unbind_source must include sourceRef",
      "suggestedDispatch[7].speak_narrator mode is invalid",
      "suggestedDispatch[8].bind_narrator_stream must include sourceRef",
      "suggestedDispatch[8].bind_narrator_stream streamKind is invalid",
      "suggestedDispatch[8].bind_narrator_stream deliveryMode is invalid",
      "suggestedDispatch[9].set_loop_state must include loopRef",
      "suggestedDispatch[9].set_loop_state state is invalid",
      "suggestedDispatch[10].repair_loop must include loopRef",
      "suggestedDispatch[11].repair_source must include sourceRef or loopRef",
      "suggestedDispatch[12].focus_process_graph must include nodeRef",
      "suggestedDispatch[13].kind is invalid",
    ]));
  });

  it("accepts auditable control dispatch actions as non-terminal goal-context evidence", () => {
    expect(validateWorkstationGoalContextUpdateV1({
      ...update,
      producerKind: "route_watch",
      updateKind: "suggested_action",
      contentRef: "stage_play_workstation_control_receipt:route-watch:frog",
      evidenceRefs: [
        "stage_play_workstation_control_receipt:route-watch:frog",
        "source:visual-tab",
        "stage_play_mail_loop:desktop",
        ...
        "agent_goal_allowed_actuator:update_live_answer",
        "agent_goal_allowed_actuator:focus_process_graph",
      ],
      receiptRefs: ["stage_play_workstation_control_receipt:route-watch:frog"],
      suggestedDispatch: [
        { kind: "log_receipt", receiptRef: "stage_play_workstation_control_receipt:route-watch:frog" },
        { kind: "change_preset", targetRef: "source:visual-tab", presetId: "preset:frog-classifier" },
        { kind: "bind_source", sourceRef: "source:visual-tab", targetRef: "live-answer:visual" },
        { kind: "unbind_source", sourceRef: "source:visual-tab", targetRef: "live-answer:visual" },
        { kind: "set_loop_state", loopRef: "loop:visual-mail", state: "paused" },
        { kind: "repair_loop", loopRef: "loop:visual-mail" },
        { kind: "repair_source", sourceRef: "source:visual-tab", loopRef: "loop:visual-mail" },
        { kind: "update_live_answer", lineKey: "visual-summary" },
        { kind: "focus_process_graph", nodeRef: "stage_play_processed_mail_packet:frog" },
      ],
    })).toEqual([]);
  });
