...
export const helixEnvironmentPlanConditionSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
export const helixEnvironmentTemporalPlanNodeSchema = z.discriminatedUnion("kind", [
  actionNodeSchema,
...
const nodeTargets = (node: z.infer<typeof helixEnvironmentTemporalPlanNodeSchema>) => {
  if (node.kind === "action") {
    return [
...
const validatePlanGraph = (plan: TemporalPlanBase, context: z.RefinementCtx) => {
  const nodes = new Map(plan.nodes.map((node) => [node.node_id, node]));
  if (nodes.size !== plan.nodes.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes"], message: "Node ids must be unique." });
    return;
  }
  const lanes = new Map(plan.lanes.map((lane) => [lane.lane_id, lane]));
  if (lanes.size !== plan.lanes.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["lanes"], message: "Lane ids must be unique." });
  }
  if (!nodes.has(plan.start_node_id)) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["start_node_id"], message: "Start node is missing." });
    return;
  }
  if (plan.watermarks.committed_through_unit > plan.maximum_total_units) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["watermarks"], message: "Committed watermark exceeds plan horizon." });
  }
  if (plan.monotonic_deadline_elapsed_ms <= plan.clocks.monotonic.elapsed_ms) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["monotonic_deadline_elapsed_ms"], message: "Plan deadline must follow the starting monotonic clock." });
  }
  if (
    plan.watermarks.stabilization_node_id !== null &&
    !nodes.has(plan.watermarks.stabilization_node_id)
  ) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["watermarks", "stabilization_node_id"], message: "Stabilization node is missing." });
  }

  for (const [index, node] of plan.nodes.entries()) {
    for (const target of nodeTargets(node)) {
      if (!nodes.has(target)) {
        context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes", index], message: `Node references missing target ${target}.` });
      }
    }
    if (node.kind !== "action") continue;
    const lane = lanes.get(node.lane_id);
    if (!lane) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes", index, "lane_id"], message: "Action lane is missing." });
      continue;
    }
    if (node.required_resources.some((resource) => !lane.resource_keys.includes(resource))) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes", index, "required_resources"], message: "Action resources must be declared by its lane." });
    }
    if (node.timing.latest_start_unit + node.timing.maximum_duration_units > plan.maximum_total_units) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes", index, "timing"], message: "Action timing exceeds the plan horizon." });
    }
    for (const [effect, count] of Object.entries(node.effect_budget)) {
      if (!(effect in plan.effect_ceiling) || count > plan.effect_ceiling[effect]) {
        context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes", index, "effect_budget", effect], message: "Action effect exceeds or is absent from the plan ceiling." });
      }
    }
  }
  ...
  const visit = (nodeId: string) => {
    if (visiting.has(nodeId)) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes"], message: "Temporal plan graph must be acyclic." });
      return;
    }
  ...
  if (visited.size !== plan.nodes.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes"], message: "Every node must be reachable from the start node." });
  }
  if (!terminalReachable) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["nodes"], message: "At least one terminal must be reachable." });
  }
};
...
export const helixEnvironmentTemporalPlanSchema = temporalPlanBaseSchema
  ...
  .superRefine((plan, context) => {
    ...
    if (helixEnvironmentTimeSha256(withoutHash) !== suppliedHash) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["plan_hash"], message: "Plan hash does not match canonical semantic content." });
    }
...
export const evaluateHelixEnvironmentPlanCurrentness = (input: {
  ...
  const current = helixEnvironmentTimeIdentitySchema.parse(input.current_identity);
  const mismatches = (Object.keys(current) as Array<keyof typeof current>).filter(
    (key) => plan.identity[key] !== current[key],
  );
  ...
  if (input.monotonic_elapsed_ms >= plan.monotonic_deadline_elapsed_ms) {
    return { current: false as const, reason: "deadline_expired" as const, mismatch_fields: ["monotonic_deadline_elapsed_ms"] };
  }
...
export const reduceHelixEnvironmentTemporalPlanEvents = (
  ...
  const transitions: Record<string, Partial<Record<z.infer<typeof temporalPlanEventPayloadSchema>["kind"], typeof state>>> = {
    none: { plan_admitted: "admitted" },
    admitted: { execution_started: "running", cancel_requested: "canceling", plan_settled: "settled" },
    running: {
...
export const helixEnvironmentAffordanceFrontierSchema = z
  ...
  .superRefine((frontier, context) => {
    if (frontier.expires_at_environment_sequence <= frontier.clocks.environment.sequence) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["expires_at_environment_sequence"], message: "Affordance frontier must expire after its observation sequence." });
    }
    const keys = frontier.entries.map((entry) => `${entry.capability_id}@${entry.capability_version}:${entry.subject_id}`);
    if (new Set(keys).size !== keys.length) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["entries"], message: "Affordance entries must be unique by capability, version and subject." });
    }
  });
...
export const buildHelixEnvironmentAffordanceFrontier = (input: {
  ...
  if (previous) {
    ...
    if (identity.affordance_revision !== previous.identity.affordance_revision + 1) {
    ...
    if (
      ...
      input.clocks.environment.sequence < previous.clocks.environment.sequence ||
      input.clocks.monotonic.origin_id !== previous.clocks.monotonic.origin_id ||
      input.clocks.monotonic.elapsed_ms < previous.clocks.monotonic.elapsed_ms
    ) {
      throw new Error("Affordance frontier clocks changed domain or regressed.");
    }
  }
  const entries = input.entries
    .map((entry) => affordanceEntrySchema.parse(entry))
    .sort((left, right) => affordanceKey(left).localeCompare(affordanceKey(right)));
  const currentByKey = new Map(entries.map((entry) => [affordanceKey(entry), entry]));
  ...
  return helixEnvironmentAffordanceFrontierSchema.parse({
    ...
    expired_capability_ids: uniqueCapabilities(
      [...previousByKey].filter(([key]) => !currentByKey.has(key))
        .map(([, entry]) => entry.capability_id),
    ),
...
export const helixEnvironmentInterruptReceiptSchema = z
  ...
  .superRefine((receipt, context) => {
    if (receipt.preempted_clocks.monotonic.elapsed_ms < receipt.detected_clocks.monotonic.elapsed_ms) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["preempted_clocks"], message: "Pre-emption cannot precede detection." });
    }
    if (receipt.kind === "user_steering" && receipt.next_decision === "resume") {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["next_decision"], message: "Steering that invalidates a plan must replan, cancel or stabilize rather than auto-resume." });
    }
    const expected = ({
    ...
    } as const)[receipt.kind];
    if (
      receipt.priority !== expected[0] ||
      receipt.next_decision !== expected[1] ||
      receipt.controls_released !== expected[2]
    ) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Interrupt priority, decision and control release must match the fixed authority-reducing policy." });
    }
    if (
      ...
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["preempted_clocks"], message: "Pre-emption clocks must preserve their domains and cannot regress." });
    }
  });
...
export const buildHelixEnvironmentInterruptReceipt = (
  ...
  const [priority, nextDecision, controlsReleased] = interruptPolicy[input.kind];
...
export const buildHelixEnvironmentFeedbackLatency = (input: {
  ...
  return helixEnvironmentFeedbackLatencySchema.parse({
    ...
    spans_ms: {
      manual_input_to_release: duration(marks.manual_input, marks.controls_released),
      finalized_input_to_task_available: duration(marks.input_finalized, marks.task_available),
      pickup_to_acknowledgement: duration(marks.steering_picked_up, marks.steering_acknowledged),
      arbitration_to_plan_stop: duration(marks.arbitration_completed, marks.plan_stopped),
      observation_to_replacement_proposal: duration(marks.observation_available, marks.replacement_proposed),
      proposal_to_admission: duration(marks.replacement_proposed, marks.proposal_admitted),
      admission_to_first_execution_unit: duration(marks.proposal_admitted, marks.first_execution_unit),
      final_observation_to_presentation: duration(marks.final_observation, marks.presentation_completed),
    },
