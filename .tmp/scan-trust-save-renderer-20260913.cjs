...
const root = path.resolve('apps/desktop/release-trust-save-recovery-20260913/win-unpacked/resources/runtime/dist/public');
const hash = data => crypto.createHash('sha256').update(data).digest('hex');
const manifest = [];
const controls = [];
const forbidden = ['fixtureTrustSessions', 'fixture-trust-browser-session', 'onboarding-trust-handlers-', 'isolated-trust-account-resolver', 'fixture-private-read-failure', 'private-transport-detail', 'fixture-browser-session', 'fixture_human_route_required', 'ephemeralPairingVault', 'onboarding-real-handlers-'];
const hits = [];
function visit(dir) {
...
const serviceBytes = asar.extractFile(path.resolve('apps/desktop/release-trust-save-recovery-20260913/win-unpacked/resources/app.asar'), 'dist/service.mjs');
...
const result = {
  schema: 'casimirbot.trust_save_renderer_evidence.v1', observed_at: new Date().toISOString(),
...
fs.writeFileSync('docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-13-trust-save-renderer.json', JSON.stringify(result, null, 2) + '\n', {flag: 'wx'});
