...
async function main() {
  ...
  const payload = JSON.parse(await fs.readFile(".tmp/launcher-diagnostic-verification-request-20260913.json", "utf8"));
  const app = express();
  ...
  await fs.writeFile("docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-13-launcher-diagnostic-adapter-verification.json",
