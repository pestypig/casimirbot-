...
async function main() {
  ...
  const payload = JSON.parse(await fs.readFile(".tmp/local-server-verification-request-20260913.json", "utf8"));
  const app = express();
  ...
  await fs.writeFile("docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-13-saved-server-adapter-verification.json",
