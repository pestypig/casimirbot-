...
const root = path.resolve('apps/desktop/release-review-recovery-20260914/win-unpacked');
const markers = ['room:player-review-fixture','subject:player-review-alice','fixture:origin-provider','chat:origin-exact','pairing-origin-browser-','minecraft_launcher_native_observation_fixture.v1','fixture-selected-profile','minecraft_launcher_profile_fixture.v1','fixture_private_observation_failure','fixture-private-property','minecraft_server_lifecycle_fixture.v1','fixture_lost_start_reply','inert fixture; never executed','fixture_mutex_ready','minecraft-pending-bootstrap-fixture','pending-bootstrap-owner','fixture-browser-session','fixture_human_route_required','fixture_provider_denied','ephemeralPairingVault','onboarding-real-handlers-','STEERING_PROCESS_FIXTURE_KEY','FIXTURE_RESULT:','steering_process_fixture_failed','CASIMIR_ONBOARDING_POSTGRES_TEST_URL','onboarding_fixture','fixture-human-consent','fixture-native-human-approved-request','fixture-native-owner'];
...
const controls=Object.fromEntries(['helix_reasoning_destination_register','helix_reasoning_pairing_accept','helix_reasoning_pairing_recover','steering_revision_invalid','pairing_request_cancelled','helix.pairing_request_cancellation.v1'].map(key=>[key,service.includes(key)]));
...
fs.writeFileSync('docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-14-review-recovery-fixture-scan.json',JSON.stringify(result,null,2)+'\n',{flag:'wx'});
