...
export function buildRoot(frontier: any, planning: any, actor: any) {
  ...
  const plan = buildHelixEnvironmentTemporalPlan({
    schema:'environment.temporal_action_plan.v1', plan_id:'plan:cs3-root-motion-baseline-20260914-02',
    ...
    nodes:[
      {kind:'checkpoint',node_id:'initial',checkpoint_id:'checkpoint:cs3-root-initial-20260914-02',required_evidence_kinds:['player_pose'],condition:condition('position_within',{position:actor.position,radius:0.75}),wait_up_to_units:1,on_satisfied_node_id:'walk',on_timeout_node_id:'failed'},
      ...
      {kind:'checkpoint',node_id:'arrival',checkpoint_id:'checkpoint:cs3-root-arrival-20260914-02',required_evidence_kinds:['player_pose'],condition:condition('position_within',{position:destination,radius:1.5}),wait_up_to_units:1,on_satisfied_node_id:'success',on_timeout_node_id:'failed'},
