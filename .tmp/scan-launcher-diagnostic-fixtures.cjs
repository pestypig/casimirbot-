...
const root = path.resolve('apps/desktop/release-launcher-diagnostic-20260913/win-unpacked');
const markers = ['minecraft_launcher_profile_fixture.v1','fixture_private_observation_failure','fixture-private-property','minecraft_server_lifecycle_fixture.v1','fixture_lost_start_reply','inert fixture; never executed','fixture_mutex_ready','minecraft-pending-bootstrap-fixture','pending-bootstrap-owner','fixture-browser-session','fixture_human_route_required','fixture_provider_denied','ephemeralPairingVault','onboarding-real-handlers-','STEERING_PROCESS_FIXTURE_KEY','FIXTURE_RESULT:','steering_process_fixture_failed','CASIMIR_ONBOARDING_POSTGRES_TEST_URL','onboarding_fixture','fixture-human-consent'];
...
const controls=Object.fromEntries(['helix_reasoning_destination_register','helix_reasoning_pairing_accept','helix_reasoning_pairing_recover','steering_revision_invalid'].map(key=>[key,service.includes(key)]));
...
fs.writeFileSync('docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-13-launcher-diagnostic-fixture-scan.json',JSON.stringify(result,null,2)+'\n',{flag:'wx'});
