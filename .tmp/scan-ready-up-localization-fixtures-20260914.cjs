...
const controls=Object.fromEntries(['failure_phase','environment_session_request_invalid','durable_goal_evidence_stale','evidence_freshness','helix_reasoning_destination_register','helix_reasoning_pairing_accept','helix_reasoning_pairing_recover','steering_revision_invalid','pairing_request_cancelled','helix.pairing_request_cancellation.v1'].map(key=>[key,service.includes(key)]));
...
fs.writeFileSync('docs/evidence/eh-g8-et6-continuous-session-build-v1/2026-09-14-ready-up-localization-fixture-scan.json',JSON.stringify(result,null,2)+'\n',{flag:'wx'});
